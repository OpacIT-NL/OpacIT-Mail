...
if (!defined('X2MAIL_DEV')) {
	define('X2MAIL_DEV', '0.0.0' === APP_VERSION);
}
...
if (!defined('X2MAIL_LIBRARIES_PATH')) {
	define('X2MAIL_LIBRARIES_PATH', rtrim(realpath(__DIR__), '\\/').'/app/libraries/');

	if (false === set_include_path(X2MAIL_LIBRARIES_PATH . PATH_SEPARATOR . get_include_path())) {
		exit('set_include_path() failed. Probably due to Apache config using php_admin_value instead of php_value');
	}

	spl_autoload_extensions('.php');
	/** lowercase autoloader (for non-X2Mail libraries) */
...
//		if ($file = stream_resolve_include_path(strtr($sClassName, '\\', DIRECTORY_SEPARATOR) . '.php')) {
	...
	/** X2Mail\Engine lowercase-filename autoloader
	 *  Handles engine classes that use lowercase file/directory names
	 *  e.g. X2Mail\Engine\Crypt → X2Mail/Engine/crypt.php
	 *       X2Mail\Engine\SASL\Plain → X2Mail/Engine/sasl/Plain.php (lowercase dir, PascalCase file)
	 */
	spl_autoload_register(function($sClassName){
		if (str_starts_with($sClassName, 'X2Mail\\Engine\\')) {
			...
			// e.g. X2Mail\Engine\SASL\XOAuth2 → X2Mail/Engine/sasl/XOAuth2.php
...
if (empty($_ENV['X2MAIL_INCLUDE_AS_API'])) {
