...
require_once __DIR__ . '/app/libraries/X2Mail/Engine/integrity.php';

$result = \X2Mail\Engine\Integrity::phpVersion();
if ($result) {
	echo '<p style="color: red">[301] ' . $result . '</p>';
	exit(301);
}

$result = \X2Mail\Engine\Integrity::phpExtensions();
if ($result) {
...
//			break;
	...
	if (defined('X2MAIL_UPDATE_PLUGINS')) {
		...
		foreach ($aList as $sId) {
			\X2Mail\Engine\Repository::installPackage('plugin', $sId);
		}
		$_ENV['X2MAIL_INCLUDE_AS_API'] = $asApi;
	}
