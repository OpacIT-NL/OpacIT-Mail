...
use X2Mail\Mail\Imap\Responses\ACL as ACLResponse;
...
*/
	...
	private function FolderACLRequest(string $sFolderName, string $sCommand, array $aParams) : \X2Mail\Mail\Imap\ResponseCollection
	{
...
//			\array_unshift($aParams, $this->EscapeFolderName($sFolderName));
		...
		return new \X2Mail\Mail\Imap\ResponseCollection();
	}
	...
	public function FolderGetACL(string $sFolderName) : array
	{
		...
		if ($response->hasRight('a')) {
			...
			foreach ($oResponses as $oResponse) {
				// * ACL INBOX.shared demo@x2mail.dev akxeilprwtscd demo2@x2mail.dev lrwstipekxacd
				if (\X2Mail\Mail\Imap\Enumerations\ResponseType::UNTAGGED->value === $oResponse->ResponseType
