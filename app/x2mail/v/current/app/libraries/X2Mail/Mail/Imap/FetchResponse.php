...
class FetchResponse
{
	...
	public function GetFetchEnvelopeEmailCollection(int $iIndex, string $sParentCharset = \X2Mail\Mail\Base\Enumerations\Charset::ISO_8859_1->value) : ?\X2Mail\Mail\Mime\EmailCollection
	{
		$oResult = null;
		$aEmails = $this->GetFetchEnvelopeValue($iIndex);
		if (\is_array($aEmails) && \count($aEmails)) {
			$oResult = new \X2Mail\Mail\Mime\EmailCollection;
			foreach ($aEmails as $aEmailItem) {
				if (\is_array($aEmailItem) && 4 === \count($aEmailItem)) {
					$sDisplayName = \X2Mail\Mail\Base\Utils::DecodeHeaderValue(
						self::findEnvelopeIndex($aEmailItem, 0, ''), $sParentCharset);

//					$sRemark = \X2Mail\Mail\Base\Utils::DecodeHeaderValue(
//						self::findEnvelopeIndex($aEmailItem, 1, ''), $sParentCharset);
					...
					if (\strlen($sLocalPart) && \strlen($sDomainPart)) {
						$oResult->append(
							new \X2Mail\Mail\Mime\Email($sLocalPart.'@'.$sDomainPart, $sDisplayName)
						);
					}
				}
			}
