...
class SearchCriterias
{
	...
	*/
	...
	public static function fromString(\X2Mail\Mail\Imap\ImapClient $oImapClient, string $sFolderName, string $sSearch, bool $bHideDeleted, bool &$bUseCache = true) : self
	{
		$iTimeFilter = 0;
			...
			if (!$aLines) {
				...
				foreach ($aLines as $sName => $sRawValue) {
					...
					switch ($sName) {
						case 'FROM':
						case 'SUBJECT':
						case 'BODY': // $sValue = \trim(\X2Mail\Mail\Base\Utils::StripSpaces($sValue), '"');
						...
						case 'KEYWORD':
							...
							$aCriteriasResult[] = static::escapeSearchString(
								$oImapClient,
								\X2Mail\Mail\Base\Utils::Utf8ToUtf7Modified($sRawValue)
							);
							break;
...
//			$aCriteriasResult[] = \gmdate('j-M-Y', $iTimeFilter);
	...
	public static function escapeSearchString(\X2Mail\Mail\Imap\ImapClient $oImapClient, string $sSearch) : string
	{
//		return $oImapClient->EscapeString($sSearch);
//		return \X2Mail\Mail\Base\Utils::IsAscii($sSearch) || $oImapClient->hasCapability('QQMail'))
		return (\X2Mail\Mail\Base\Utils::IsAscii($sSearch) || !$oImapClient->hasCapability('LITERAL+'))
	...
	{
		if (\strlen($sDate)) {
			$oDateTime = \DateTime::createFromFormat('Y-m-d', $sDate, \X2Mail\Mail\Base\DateTimeHelper::GetUtcTimeZoneObject());
	...
	/**
	 * X2Mail search like: 'from=foo&to=test&is[]=unseen&is[]=flagged'
	 */
	...
	private static function parseSearchString(string $sSearch) : array
	{
		...
		$sSearch = \X2Mail\Mail\Base\Utils::StripSpaces($sSearch);
		...
		if (\preg_match_all('/".*?(?<!\\\)"/', $sSearch, $mMatch)) {
			foreach ($mMatch[0] as $sItem) {
				do {
					$sKey = \X2Mail\Mail\Base\Utils::Sha1Rand();
				}
