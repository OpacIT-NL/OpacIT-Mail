...
//			&& \is_callable($mFunctionNameOrNames);
	...
	public static function IdnToUtf8(string $sStr) : string
	{
		...
		\X2Mail\Engine\Log::warning('MailSo', "Deprecated function IdnToUtf8 called at {$trace['file']}#{$trace['line']}");
		if (\preg_match('/(^|\.|@)xn--/i', $sStr)) {
			$sStr = \str_contains($sStr, '@')
			? \X2Mail\Engine\IDN::emailToUtf8($sStr)
			: \idn_to_utf8($sStr);
		}
		return $sStr;
	}
	...
	{
		$trace = \debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 2)[1];
		\X2Mail\Engine\Log::warning('MailSo', "Deprecated function IdnToAscii called at {$trace['file']}#{$trace['line']}");
		$aParts = \explode('@', $sStr);
