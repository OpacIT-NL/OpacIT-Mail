...
abstract class NetClient
{
	...
	{
		throw new \X2Mail\Mail\RuntimeException($sErrStr, $iErrNo);
	}

	/**
	 * @throws \ValueError
	 * @throws \X2Mail\Mail\RuntimeException
	 * @throws \X2Mail\Mail\Net\Exceptions\SocketAlreadyConnectedException
	 * @throws \X2Mail\Mail\Net\Exceptions\SocketCanNotConnectToHostException
	 */
...
//			$this->Disconnect();
		...
		$this->ssl = \X2Mail\Mail\Net\Enumerations\ConnectionSecurityType::UseSSL($this->Settings->port, $this->Settings->type);
...
//			$this->sConnectedHost = ($this->ssl ? 'ssl://' : '') . $this->sConnectedHost;
		...
		if (!$this->ssl && \X2Mail\Mail\Net\Enumerations\ConnectionSecurityType::SSL->value === $this->Settings->type) {
			$this->writeLogException(
				new \X2Mail\Mail\Net\Exceptions\SocketUnsuppoterdSecureConnectionException('SSL isn\'t supported: ('.\implode(', ', \stream_get_transports()).')'),
				\LOG_ERR);
		}
		...
		if (!$this->rConnect) {
			$this->writeLogException(
				new Exceptions\SocketCanNotConnectToHostException(
					\X2Mail\Mail\Base\Locale::ConvertSystemString($sErrorStr), (int) $iErrorNo,
		...
		if ($this->rConnect && \X2Mail\Mail\Base\Utils::FunctionCallable('stream_set_timeout')) {
	...
	{
		$bSuccess = false;
		if ($this->rConnect && \X2Mail\Mail\Base\Utils::FunctionCallable('stream_socket_enable_crypto')) {
...
//			}
		...
		$bSuccess || $this->writeLogException(new \X2Mail\Mail\RuntimeException('Cannot enable STARTTLS.'), \LOG_ERR);
	}
...
//	abstract public function Login(ConnectSettings $oSettings) : self;
	...
	{
		return $this->rConnect && \X2Mail\Mail\Base\Utils::FunctionCallable('stream_context_get_options')
			? \stream_context_get_params($this->rConnect) : false;
	}

	/**
	 * @throws \X2Mail\Mail\Net\Exceptions\SocketConnectionDoesNotAvailableException
	 * @throws \X2Mail\Mail\Net\Exceptions\SocketWriteException
	 */
	...
	/**
	 * @throws \X2Mail\Mail\Net\Exceptions\SocketConnectionDoesNotAvailableException
	 * @throws \X2Mail\Mail\Net\Exceptions\SocketReadException
	 */
