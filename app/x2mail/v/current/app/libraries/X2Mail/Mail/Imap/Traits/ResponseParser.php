...
namespace X2Mail\Mail\Imap\Traits;

use X2Mail\Mail\Imap\Response;
use X2Mail\Mail\Imap\Enumerations\ResponseType;
use X2Mail\Mail\Imap\Exceptions\ResponseNotFoundException;
...
//									$this->writeLog('{'.$iLiteralLen.'} '.$sLiteral, \LOG_INFO);
									...
									\X2Mail\Engine\Log::notice('IMAP', 'Literal treated as atom and skipped');
								}
								unset($sLiteral);
							} else {
				...
				case '"':
					...
					while (true) {
						...
						case "\r":
						case "\n":
							\X2Mail\Engine\Log::notice('IMAP', 'Invalid char in quoted string: "' . \substr($this->sResponseBuffer, $iPos, $iOffset + $iLength - $iPos) . '"');
...
//				\set_time_limit(10);
	...
	private function partialResponseLiteralCallbacks(?string $sParent, ?string $sLiteralAtomUpperCase, int $iLiteralLen) : bool
	{
		...
		$rImapLiteralStream = \X2Mail\Mail\Base\StreamWrappers\Literal::CreateStream($this->ConnectionResource(), $iLiteralLen);
