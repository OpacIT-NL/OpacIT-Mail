...
*/
		...
		@$oDoc->loadHTML('<?xml version="1.0" encoding="utf-8"?>'.
			...
			'<body '.\trim($sBodyAttrs).'>'.\X2Mail\Mail\Base\Utils::Utf8Clear($sHtml).'</body></html>');

		$oDoc->normalizeDocument();

		if (\X2Mail\Mail\Base\Utils::FunctionCallable('libxml_clear_errors')) {
			\libxml_clear_errors();
		}
	...
	{
		$sText = \X2Mail\Mail\Base\Utils::StripSpaces($sText);
