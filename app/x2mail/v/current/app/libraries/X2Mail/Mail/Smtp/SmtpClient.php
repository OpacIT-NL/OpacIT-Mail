 ...
 * @package Smtp
 */
class SmtpClient extends \X2Mail\Mail\Net\NetClient
{
	/** @var Settings */
	...
	public function Login(Settings $oSettings) : self
	{
		...
		foreach ($oSettings->SASLMechanisms as $sasl_type) {
			if (\in_array(\strtoupper($sasl_type), $this->aAuthTypes) && \X2Mail\Engine\SASL::isSupported($sasl_type)) {
				$type = $sasl_type;
				break;
			}
		}
...
//			$sCmd .= ' BODY=BINARYMIME';
	...
	{
		if (!$this->bMail) {
			$this->writeLogException(new \X2Mail\Mail\RuntimeException('No sender reverse path has been supplied'), \LOG_ERR);
		}
...
//		$sTo = IDN::emailToAscii($sTo);
	...
	public function Data(string $sData) : self
	{
		...
		$rDataStream = \X2Mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString($sData);
		unset($sData);
		$this->DataWithStream($rDataStream);
		\X2Mail\Mail\Base\ResourceRegistry::CloseMemoryResource($rDataStream);
	...
	public function DataWithStream($rDataStream) : self
	{
		...
		if (!$this->bRcpt) {
			$this->writeLogException(new \X2Mail\Mail\RuntimeException('No recipient forward path has been supplied'), \LOG_ERR);
		}
...
/*
		...
		if ($this->hasCapability('CHUNKING')) {
			...
			while (!\feof($rDataStream)) {
				...
				if (false === $sBuffer) {
					if (!\feof($rDataStream)) {
						$this->writeLogException(new \X2Mail\Mail\RuntimeException('Cannot read input resource'), \LOG_ERR);
					}
					break;
				}
...
*/
	...
	{
		$sUser = \X2Mail\Mail\Base\Utils::Trim($sUser);
/*
		// RFC 6531
		if ($this->hasCapability('SMTPUTF8')) {
...
*/
	...
	/**
	 * @throws \X2Mail\Mail\Smtp\Exceptions\ResponseException
	 */
