 ...
 * @package Imap
 */
class ImapClient extends \X2Mail\Mail\Net\NetClient
{
	...
	/**
	 * Used by \X2Mail\Mail\Client\MailClient::MessageMimeStream
	 */
...
//	public function Connect(Settings $oSettings) : void
	...
	public function Login(Settings $oSettings) : self
	{
		...
		foreach ($oSettings->SASLMechanisms as $sasl_type) {
			if ($this->hasCapability("AUTH={$sasl_type}") && \X2Mail\Engine\SASL::isSupported($sasl_type)) {
				$type = $sasl_type;
				break;
			}
		}
		...
		if (!$type) {
			...
			throw new \X2Mail\Mail\RuntimeException('No supported SASL mechanism found, remote server wants: '
...
//						\X2Mail\Engine\Log::warning('IMAP', "{$oResponse->OptionalResponse[0]}: {$this->lastCommand}");
					}
...
*/
	...
	{
		return $this->EscapeString($this->UTF8 ? $sFolderName : \X2Mail\Mail\Base\Utils::Utf8ToUtf7Modified($sFolderName));
	}

	public function toUTF8(string $sText) : string
	{
		return $this->UTF8 ? $sText : \X2Mail\Mail\Base\Utils::Utf7ModifiedToUtf8($sText);
	}
}
