...
namespace X2Mail\Mail\Imap\Commands;

use X2Mail\Mail\Imap\FetchResponse;
use X2Mail\Mail\Imap\Enumerations\FetchType;
use X2Mail\Mail\Imap\ResponseCollection;
use X2Mail\Mail\Imap\SequenceSet;
use X2Mail\Mail\Imap\Enumerations\MessageFlag;
use X2Mail\Mail\Imap\Enumerations\ResponseType;
...
*/
	...
	private function writeMessageStream($rMessageStream) : ?int
	{
		...
		\X2Mail\Mail\Base\Utils::WriteStream($rMessageStream, $this->ConnectionResource());
...
*/
	 ...
	 * @throws \InvalidArgumentException
	 * @throws \X2Mail\Mail\RuntimeException
	 * $sStoreAction = \X2Mail\Mail\Imap\Enumerations\StoreAction::ADD_FLAGS_SILENT->value
	 */
	public function MessageStoreFlag(SequenceSet $oRange, array $aInputStoreItems, string $sStoreAction) : ?ResponseCollection
	{
		...
		$aInputStoreItems = \array_map('\\X2Mail\Mail\\Base\\Utils::Utf8ToUtf7Modified', $aInputStoreItems);
	...
	{
		$oSort = new \X2Mail\Mail\Imap\Requests\SORT($this);
	...
	{
		$oESearch = new \X2Mail\Mail\Imap\Requests\ESEARCH($this);
...
//		if (!$this->UTF8 && !\mb_check_encoding($sSearchCriterias, 'UTF-8')) {
		if (!$this->UTF8 && !\X2Mail\Mail\Base\Utils::IsAscii($sSearchCriterias)) {
...
//		if (!$this->UTF8 && !\mb_check_encoding($sSearchCriterias, 'UTF-8')) {
	...
	{
		$oThread = new \X2Mail\Mail\Imap\Requests\THREAD($this);
