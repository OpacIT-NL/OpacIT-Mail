...
//			, ['(USE (\Drafts \Sent))'] RFC 6154
	...
	public function FolderDelete(string $sFolderName) : void
	{
		...
		if ($oInfo->MESSAGES) {
			throw new \X2Mail\Mail\Client\Exceptions\NonEmptyFolder;
		}
				...
				if (\is_array($oResponse->OptionalResponse)) {
					...
					if (\count($oResponse->OptionalResponse) > 1) {
						if ('PERMANENTFLAGS' === $key && \is_array($oResponse->OptionalResponse[1])) {
							$oResult->PermanentFlags = \array_map('\\X2Mail\Mail\\Base\\Utils::Utf7ModifiedToUtf8', $oResponse->OptionalResponse[1]);
						}
...
//					$oResult->Flags = \array_map('\\X2Mail\Mail\\Base\\Utils::Utf7ModifiedToUtf8', $oResponse->ResponseList[2]);
				}
			}
		}
