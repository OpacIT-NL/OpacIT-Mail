...
//		'SIGSTOP',
	...
	public static function Guid() : string
	{
		static $sCache = null;
		if (null === $sCache) {
			$sCache = \substr(\X2Mail\Mail\Base\Utils::Sha1Rand(), -8);
		}
...
//		$this->bShowSecrets && $this->Write("AddSecret '{$sWord}'", \LOG_INFO);
	...
	public function __loggerShutDown() : void
	{
		if ($this->bUsed) {
			...
			$this->Write('Memory peak usage: '.\X2Mail\Mail\Base\Utils::FormatFileSize(\memory_get_peak_usage(true), 2));
	...
	public function Write(string $sDesc, int $iType = \LOG_INFO, string $sName = '', bool $bDiplayCrLf = false) : bool
	{
		...
		foreach ($this as /* @var $oLogger \X2Mail\Mail\Log\Driver */ $oLogger)
		{
