...
namespace X2Mail\Engine\Config;
...
abstract class AbstractConfig implements \ArrayAccess, \JsonSerializable
{
	...
	public function __construct(string $sFileName, string $sFileHeader = '', string $sAdditionalFileName = '')
	{
		...
		$this->bUseApcCache = defined('APP_USE_APCU_CACHE') && APP_USE_APCU_CACHE &&
			\X2Mail\Mail\Base\Utils::FunctionsCallable(array('apcu_fetch', 'apcu_store'));
	}
	...
	{
		// $salt = \basename($this->sFile) not possible due to X2Mail\Engine\Plugins\Property
		if (!empty($this->aData[$sSection][$sName][0])) try {
			return \X2Mail\Engine\Crypt::DecryptFromJSON($this->aData[$sSection][$sName][0], \APP_SALT);
		} catch (\Throwable $e) {
		}
		return $mDefault;
	}
	...
	{
		// $salt = \basename($this->sFile) not possible due to X2Mail\Engine\Plugins\Property
		$mParamValue = \X2Mail\Engine\Crypt::EncryptToJSON($mParamValue, \APP_SALT);
	...
	public function Save() : bool
	{
		...
		if (\str_ends_with($this->sFile, '.json')) {
			$this->clearCache();
			\X2Mail\Engine\Utils::saveFile($this->sFile, \json_encode($this, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES));
			// Remove old .ini file
		...
		\X2Mail\Engine\Utils::saveFile($this->sFile,
