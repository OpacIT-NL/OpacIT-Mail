...
class Identity implements \JsonSerializable
{
	...
	function toMime() : \X2Mail\Mail\Mime\Email
	{
		return new \X2Mail\Mail\Mime\Email($this->sEmail, $this->sName);
	}
	...
	public function FromJSON(array $aData, bool $bJson = false): bool
	{
		if (!empty($aData['Email'])) {
			...
			$this->sEmail = $bJson ? \X2Mail\Engine\IDN::emailToAscii($aData['Email']) : $aData['Email'];
	...
	public function jsonSerialize()
	{
		return array(
			...
			'id' => $this->sId,
			'label' => $this->sLabel,
			'email' => \X2Mail\Engine\IDN::emailToUtf8($this->sEmail),
			'name' => $this->sName,
			'replyTo' => $this->sReplyTo,
			'bcc' => $this->sBcc,
			'signature' => $this->sSignature,
