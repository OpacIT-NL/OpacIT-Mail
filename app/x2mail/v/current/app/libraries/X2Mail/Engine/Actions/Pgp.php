...
use X2Mail\Engine\PGP\Backup;
use X2Mail\Engine\PGP\Keyservers;
use X2Mail\Engine\PGP\GnuPG;
...
trait Pgp
{
	 ...
	 */
	public function GnuPG() : ?\X2Mail\Engine\PGP\PGPInterface
	{
		...
		if (80 < \strlen($homedir)) {
			...
			$tmpdir = \sys_get_temp_dir() . '/x2mail';
//			if (\X2Mail\Engine\Utils::inOpenBasedir($tmpdir) &&
...
//			$oPart = \X2Mail\Mail\Mime\Part::FromString($result);
		} else {
...
//						$oPart = \X2Mail\Mail\Mime\Part::FromStream($rStreamHandle);
					} catch (\Throwable $e) {
						$result = $e;
					}
				},
...
//			$sMicAlg = $this->GetActionParam('micAlg', '');
			...
			if ($sSigPartId) {
				...
				$decode = (new \X2Mail\Mail\Mime\HeaderCollection($sBodyMime))->ValueByName(MimeEnumHeader::CONTENT_TRANSFER_ENCODING->value);
				if ('base64' === $decode) {
