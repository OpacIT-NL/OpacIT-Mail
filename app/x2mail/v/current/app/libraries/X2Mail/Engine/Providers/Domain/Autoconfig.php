...
abstract class Autoconfig
{
	...
	{
//		$emailaddress = \X2Mail\Engine\IDN::emailToAscii($emailaddress);
		$domain = \X2Mail\Mail\Base\Utils::getEmailAddressDomain($emailaddress);
//		$domain = \X2Mail\Engine\IDN::toAscii($domain);
...
//			?: self::mx($domain, $emailaddress)
	...
	{
		$oCache = \X2Mail\Engine\Api::Actions()->Cacher();
	...
	private static function mx(string $domain, string $emailaddress) : ?array
	{
		...
		foreach (\X2Mail\Engine\DNS::MX($domain) as $hostname) {
	...
	{
		$srv = \X2Mail\Engine\DNS::SRV('_submissions._tcp.'.$domain);
		if (empty($srv[0]['target'])) {
			$srv = \X2Mail\Engine\DNS::SRV('_submission._tcp.'.$domain);
		}
		if (!empty($srv[0]['target'])) {
			$result = [
				'incomingServer' => [],
			...
			$srv = \X2Mail\Engine\DNS::SRV('_imaps._tcp.'.$domain);
			if (empty($srv[0]['target'])) {
				$srv = \X2Mail\Engine\DNS::SRV('_imap._tcp.'.$domain);
			}
			if (!empty($srv[0]['target'])) {
	...
	{
		foreach (\X2Mail\Engine\DNS::SRV("_autodiscover._tcp.{$domain}") as $record) {
			if (443 == $record['port']) {
