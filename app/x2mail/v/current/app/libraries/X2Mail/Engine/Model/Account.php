...
//				$aAccountHash['login'] = $oDomain->ImapSettings()->fixUsername($aAccountHash['login']);
				...
				$oAccount = new static;
				$oAccount->sEmail = \X2Mail\Engine\IDN::emailToAscii($aAccountHash['email']);
//				$oAccount->sImapUser = \X2Mail\Engine\IDN::emailToAscii($aAccountHash['login']);
			...
			\X2Mail\Engine\Log::debug('ACCOUNT', $e->getMessage());
	...
	public function ImapConnectAndLogin(\X2Mail\Engine\Plugins\Manager $oPlugins, \X2Mail\Mail\Imap\ImapClient $oImapClient, \X2Mail\Engine\Config\Application $oConfig) : bool
	{
...
//		$oSettings->thread_limit = \min($oSettings->thread_limit, (int) $oConfig->Get('imap', 'large_thread_limit', 50));
	...
	public function SmtpConnectAndLogin(\X2Mail\Engine\Plugins\Manager $oPlugins, \X2Mail\Mail\Smtp\SmtpClient $oSmtpClient) : bool
	{
		$oSettings = $this->Domain()->SmtpSettings();
		$oSettings->username = $this->SmtpUser();
		$oSettings->Ehlo = \X2Mail\Mail\Smtp\SmtpClient::EhloHelper();
...
*/
	...
	public function SieveConnectAndLogin(\X2Mail\Engine\Plugins\Manager $oPlugins, \X2Mail\Mail\Sieve\SieveClient $oSieveClient, \X2Mail\Engine\Config\Application $oConfig)
	{
	...
	private function netClientLogin(\X2Mail\Mail\Net\NetClient $oClient, \X2Mail\Engine\Plugins\Manager $oPlugins) : bool
	{
/*
...
/*
	...
	public function settingsLocal() : \X2Mail\Engine\Settings
	{
		return \X2Mail\Engine\Api::Actions()->SettingsProvider(true)->Load($this);
	}
*/
