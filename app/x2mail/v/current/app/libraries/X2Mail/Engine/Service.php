...
//		\header('Permissions-Policy: interest-cohort=()');
		...
		if ($oConfig->Get('security', 'force_https', false) && !$oHttp->IsSecure()) {
			\X2Mail\Mail\Base\Http::Location('https://'.$oHttp->GetHost(false).$oHttp->GetUrl());
			return true;
		}
		...
		if (\count($aPaths) && !empty($aPaths[0]) && 'index' !== \strtolower($aPaths[0])) {
			if ('mailto' !== \strtolower($aPaths[0]) && !\X2Mail\Engine\HTTP\SecFetch::matchAnyRule($oConfig->Get('security', 'secfetch_allow', ''))) {
				\X2Mail\Mail\Base\Http::StatusHeader(403);
				echo $oServiceActions->ErrorTemplates('Access Denied.',
					...
					User: " . (\X2Mail\Engine\HTTP\SecFetch::user() ? 'true' : 'false'));
				return false;
			}
...
//			if (!\X2Mail\Engine\HTTP\SecFetch::isEntering()) {
			...
			if (!\is_dir(APP_DATA_FOLDER_PATH) || !\is_writable(APP_DATA_FOLDER_PATH)) {
				echo $oServiceActions->ErrorTemplates(
					'Permission denied!',
					'X2Mail can not access the data folder "'.APP_DATA_FOLDER_PATH.'"'
				);
				return false;
			}
...
//			$oActions->verifyCacheByKey($sCacheFileName);
			...
			$sScriptNonce = \X2Mail\Engine\UUID::generate();
