 ...
 *
 * GET https://keys.openpgp.org/pks/lookup?op=get&options=mr&search=security@x2mail.dev
...
*/
	...
	private static function fetch(string $host, string $op, string $search, bool $fingerprint = false, bool $exact = false) : ?\X2Mail\Engine\HTTP\Response
	{
		...
		\X2Mail\Engine\Log::debug('PGP', $url);
		return self::HTTP()->doRequest('GET', $url);
	}

	private static $HTTP;
	private static function HTTP() : \X2Mail\Engine\HTTP\Request
	{
		if (!self::$HTTP) {
			self::$HTTP = \X2Mail\Engine\HTTP\Request::factory(/*'socket' or 'curl'*/);
...
*/
		foreach (static::$hosts as $host) {
			...
			if (!$oResponse) {
				\X2Mail\Engine\Log::info('PGP', "No response for key {$keyId} on {$host}");
				continue;
			}
			if (200 !== $oResponse->status) {
				\X2Mail\Engine\Log::debug('PGP', "{$oResponse->status} for key {$keyId} on {$host}");
				continue;
			}
	...
	public static function index(string $search, bool $fingerprint = true, bool $exact = false) : array
	{
		...
		foreach (static::$hosts as $host) {
			...
			if (!$oResponse) {
				\X2Mail\Engine\Log::info('PGP', "No response for key {$search} on {$host}");
				continue;
			}
			if (200 !== $oResponse->status) {
				\X2Mail\Engine\Log::debug('PGP', "{$oResponse->status} for search `{$search}` on {$host}");
				continue;
			}
			...
			foreach ($result as $line) {
				...
				if ('pub' === $line[0]) {
					...
					// Ignore invalid line
					if (7 !== \count($line)) {
						\X2Mail\Engine\Log::info('PGP', "Invalid pub line for search `{$search}` on {$host}");
						continue;
					}
...
//						'expired'  => \str_contains($line[6], 'e'),
				...
				// uid:<escaped uid string>:<creationdate>:<expirationdate>:<flags>
					...
					// Ignore invalid line
					if (5 !== \count($line)) {
						\X2Mail\Engine\Log::info('PGP', "Invalid uid line for search `{$search}` on {$host}");
						continue;
					}
					// Ignore flagged or expired key
