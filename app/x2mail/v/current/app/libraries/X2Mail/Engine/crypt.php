...
abstract class Crypt
{
	...
	) /* : mixed */
	{
		...
		return static::Decrypt(\array_map('X2Mail\Mail\\Base\\Utils::UrlSafeBase64Decode', $data), $key);
	}
...
*/
	...
	{
		return \implode('.', \array_map('X2Mail\Mail\\Base\\Utils::UrlSafeBase64Encode', static::Encrypt($data, $key)));
	}
	...
	) /* : mixed */
	{
		...
		return \is_callable('xxtea_decrypt')
			...
			: \X2Mail\Mail\Base\Xxtea::decrypt($data, $key);
	}
	...
	) : string
	{
		...
		$result = \is_callable('xxtea_encrypt')
			...
			: \X2Mail\Mail\Base\Xxtea::encrypt($data, $key);
		if (!$result) {
...
\X2Mail\Engine\Crypt::setCipher(\X2Mail\Engine\Api::Config()->Get('security', 'encrypt_cipher', 'aes-256-cbc-hmac-sha1'));
