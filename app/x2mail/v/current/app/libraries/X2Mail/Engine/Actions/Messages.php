...
use X2Mail\Mail\Mime\Part as MimePart;
use X2Mail\Mail\Mime\Enumerations\Header as MimeEnumHeader;
...
//			\ignore_user_abort(true);
	...
	public function DoSaveMessage() : array
	{
		...
		$mResult = false;
		if ($oMessage) {
			$rMessageStream = \X2Mail\Mail\Base\ResourceRegistry::CreateMemoryResource();

			$iMessageStreamSize = \X2Mail\Mail\Base\Utils::WriteStream($oMessage->ToStream(false), $rMessageStream, 8192, true);
...
/*
		...
		if ($aAuth) {
			...
			$oAccount->setSmtpPass(new \X2Mail\Engine\SensitiveString($aAuth['password']));
...
*/
		...
		try
		{
			if ($oMessage) {
				...
				$iMessageStreamSize = \X2Mail\Mail\Base\Utils::WriteStream(
				...
				if (false !== $iMessageStreamSize) {
					...
					if (\strlen($sSaveFolder)) {
						...
						{
							if ($oMessage->GetBcc()) {
								$rAppendMessageStream = \X2Mail\Mail\Base\ResourceRegistry::CreateMemoryResource();
								$iAppendMessageStreamSize = \X2Mail\Mail\Base\Utils::WriteStream(
		...
		catch (\X2Mail\Mail\Smtp\Exceptions\LoginBadCredentialsException $oException)
		{
		...
		try
		{
			if ($oMessage && $this->AddressBookProvider($oAccount)->IsActive()) {
				...
				if ($oToCollection) {
					foreach ($oToCollection as /* @var $oEmail \X2Mail\Mail\Mime\Email */ $oEmail) {
			...
			if ($oMessage) {
				...
				if (false !== $iMessageStreamSize) {
					...
					if (\strlen($sFolderFullName) && 0 < $iUid) {
						...
						{
							$this->Cacher($oAccount)->Set(\X2Mail\Engine\KeyPathHelper::ReadReceiptCache($oAccount->Email(), $sFolderFullName, $iUid), '1');
						}
					}
				}
			}
...
//			$accountHash = $aValues[3];
		...
		try
		{
			...
			if ($oMessage->smimeSigned && ($bAutoVerify || !$oMessage->smimeSigned['detached'])) try {
				...
				$result = (new \X2Mail\Engine\SMime\OpenSSL(''))->verify($sBody, null, $bOpaque);
				if ($result) {
					if ($bOpaque) {
							...
							$this->MailClient()->MessageMimeStream(
								function($rResource, $sContentType, $sFileName, $sMimeIndex = '') use ($oAccount, $sTempName, $self, &$aAttachments, $mIndex) {
									if (\is_resource($rResource)) {
										...
										$sTempName .= \X2Mail\Engine\File\MimeType::toExtension($sContentType);
...
//												'size' => 0
							...
							$sContentType = \X2Mail\Engine\File\MimeType::fromStream($rResource, $sTempName)
								?: \X2Mail\Engine\File\MimeType::fromFilename($sTempName)
								?: 'application/octet-stream'; // 'text/plain'
							$aAttachments[$mIndex] = [
//								'name' => '',
								'tempName' => $sTempName,
...
//								'size' => $oFilesProvider->FileSize($oAccount, $sTempName)
	 ...
	 * @throws \X2Mail\Mail\Net\Exceptions\ConnectionException
	 */
	private function smtpSendMessage(Account $oAccount, \X2Mail\Mail\Mime\Message $oMessage,
		...
		bool $bDsn = false, bool $bRequireTLS = false)
	{
		...
		try
		{
			...
			$sFrom = $oFrom instanceof \X2Mail\Mail\Mime\Email ? $oFrom->GetEmail() : '';
			...
			$oSmtpClient = new \X2Mail\Mail\Smtp\SmtpClient();
			...
			if ($oSmtpClient->Settings->usePhpMail) {
				...
				foreach ($oRcpt as /* @var $oEmail \X2Mail\Mail\Mime\Email */ $oEmail) {
		...
		catch (\X2Mail\Mail\Smtp\Exceptions\LoginException $oException)
		{
			throw new ClientException(Notifications::AuthError->value, $oException);
		}
		catch (\Throwable $oException)
	...
	private function buildReadReceiptMessage(Account $oAccount) : \X2Mail\Mail\Mime\Message
	{
		...
		$oMessage = new \X2Mail\Mail\Mime\Message();

		if ($this->Config()->Get('security', 'hide_x_mailer_header', true)) {
			$oMessage->DoesNotAddDefaultXMailer();
		}

		$oMessage->SetFrom(new \X2Mail\Mail\Mime\Email($oIdentity->Email(), $oIdentity->Name()));
		...
		if (!empty($sReplyTo)) {
			$oReplyTo = new \X2Mail\Mail\Mime\EmailCollection($sReplyTo);
		...
		$oToEmails = new \X2Mail\Mail\Mime\EmailCollection($sReadReceipt);
	 ...
	 */
	private function buildMessage(Account $oAccount, bool $bWithDraftInfo = true) : \X2Mail\Mail\Mime\Message
	{
		$oMessage = new \X2Mail\Mail\Mime\Message();
...
//			$oMessage->SetReadReceipt(($oIdentity ?: $oAccount)->Email());
		...
		if (!empty($this->GetActionParam('markAsImportant', 0))) {
			$oMessage->SetPriority(\X2Mail\Mail\Mime\Enumerations\MessagePriority::HIGH->value);
		}

		$oMessage->SetSubject($this->GetActionParam('subject', ''));

		$oMessage->SetTo(new \X2Mail\Mail\Mime\EmailCollection($this->GetActionParam('to', '')));
		$oMessage->SetCc(new \X2Mail\Mail\Mime\EmailCollection($this->GetActionParam('cc', '')));
		$oMessage->SetBcc(new \X2Mail\Mail\Mime\EmailCollection($this->GetActionParam('bcc', '')));
...
//			$sProtocol = $match[1][0];
			...
			$sHtml = \X2Mail\Mail\Base\HtmlUtils::BuildHtml($sHtml, $aFoundCids, $aFoundDataURL, $aFoundContentLocationUrls);
			...
			// First add plain
			$sPlain = $this->GetActionParam('plain', '') ?: \X2Mail\Mail\Base\HtmlUtils::ConvertHtmlToPlain($sHtml);
			...
			$oAlternativePart->Body = \X2Mail\Mail\Base\StreamWrappers\Binary::CreateStream(
				\X2Mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString(\preg_replace('/\\r?\\n/su', "\r\n", \trim($sHtml))),
				'convert.quoted-printable-encode'
			);
...
/*
			if ($sSignature = $this->GetActionParam('pgpSignature', null)) {
				...
				$oAlternativePart->Body = \X2Mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString(\preg_replace('/\\r?\\n/su', "\r\n", \trim($sPlain)));
				$oPart->SubParts->append($oAlternativePart);
			...
			foreach ($aAttachments as $sTempName => $aData) {
				...
				if (\is_resource($rResource)) {
					...
					$oMessage->Attachments()->append(
						new \X2Mail\Mail\Mime\Attachment($rResource, $sFileName, $iFileSize, $bIsInline,
...
/*
		...
		if ($sPublicKey) {
			$oMessage->Attachments()->append(
				new \X2Mail\Mail\Mime\Attachment(
...
*/
		foreach ($aFoundDataURL as $sCidHash => $sDataUrlString) {
			...
			{
				$sRaw = \X2Mail\Mail\Base\Utils::Base64Decode($aMatch[2]);
				$iFileSize = \strlen($sRaw);
				if (0 < $iFileSize) {
					$sFileName = \preg_replace('/[^a-z0-9]+/i', '.', $aMatch[1]);
					$rResource = \X2Mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString($sRaw);
					...
					$oMessage->Attachments()->append(
						new \X2Mail\Mail\Mime\Attachment($rResource, $sFileName, $iFileSize, true, true, $sCID)
					);
				}
			}
		}

		$oPassphrase = new \X2Mail\Engine\SensitiveString($this->GetActionParam('signPassphrase', ''));
...
*/
			if ($sCertificate && $sPrivateKey) {
				...
				$resource = $oBody->ToStream();
				\X2Mail\Mail\Base\StreamFilters\LineEndings::appendTo($resource);
				$tmp = new \X2Mail\Engine\File\Temporary('mimepart');
