...
namespace X2Mail\Engine\GPG;

use X2Mail\Engine\SensitiveString;

class PGP extends Base implements \X2Mail\Engine\PGP\PGPInterface
{
	private
		$_message,
...
//		\rewind($fp);
	...
	{
//		\X2Mail\Engine\Log::debug('GnuPG', "export({$fingerprint}, {$passphrase})");
...
//			'NO_SECKEY' => [],
	...
	private function exec(array $arguments, bool $throw = true) /*: array|false*/
	{
		if (\version_compare($this->version, '2.2.5', '<')) {
			\X2Mail\Engine\Log::error('GPG', "{$this->version} too old");
			return false;
		}
