...
class PECL implements \X2Mail\Engine\PGP\PGPInterface
{
	private
		$homedir,
		// Instance of gnupg pecl extension https://www.php.net/gnupg
		$GnuPG,
		// Instance of \X2Mail\Engine\GPG\PGP
		$GPG;
...
*/
	...
	{
//		\X2Mail\Engine\Log::debug('GnuPG', "addDecryptKey({$fingerprint}, {$passphrase})");
	...
	{
//		\X2Mail\Engine\Log::debug('GnuPG', "addSignKey({$fingerprint}, {$passphrase})");
...
//		\rewind($fp);
	...
	public function verify(string $signed_text, string $signature, ?string &$plaintext = null) /*: array|false*/
	{
		...
		if (!$result) {
			\X2Mail\Engine\Log::notice('GnuPG', 'gnupg_verify() failed: ' . $this->GnuPG->geterror());
			\X2Mail\Engine\Log::info('GnuPG', \print_r($this->GnuPG->geterrorinfo(),1));
		}
		return $result;
	}
