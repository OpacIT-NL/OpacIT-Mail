...
//		if ($this->striked   ) $style[] = '';
		...
		if (!empty(self::$underlineStyles[$this->underline])) {
			$value = 'text-decoration:underline';
			if (!empty(static::$colortbl[$this->ulc])) {
				$value .= ' ' . static::$colortbl[$this->ulc];
			}
			$value .= ' ' . self::$underlineStyles[$this->underline];
			$style[] = $value;
		}
...
//			$style[] = 'font-size:'.\ceil($word->parameter * 16 / 24).'px';
		...
		} else if (!empty(self::$highlights[$this->highlight])) {
			$style[] = 'background-color:' . self::$highlights[$this->highlight];
		}
