...
trait AdminDomains
{
	...
	public function DoAdminDomainMatch() : array
	{
		$sCredentials = $this->resolveLoginCredentials(
			...
			new \X2Mail\Engine\SensitiveString('********')
		);
	...
	{
		$this->IsAdminLoggined();
//		$sDomain = \X2Mail\Engine\IDN::toAscii($this->GetActionParam('domain'));
		$sDomain = \strtolower(\idn_to_ascii($this->GetActionParam('domain')));
		$sEmail = "test@{$sDomain}";
		return $this->DefaultResponse(array(
			'email' => $sEmail,
			'config' => \X2Mail\Engine\Providers\Domain\Autoconfig::discover($sEmail)
		));
	}
	...
	public function DoAdminDomainTest() : array
	{
		...
		if ($oDomain) {
			...
			catch (\X2Mail\Mail\Net\Exceptions\SocketCanNotConnectToHostException $oException)
			{
			...
			if ($oDomain->SmtpSettings()->usePhpMail) {
				$mSmtpResult = \X2Mail\Mail\Base\Utils::FunctionCallable('mail');
				if (!$mSmtpResult) {
