...
//							PUT /ocs/v2.php/apps/files_sharing/api/v1/shares/2 {"password":"ABC09"}
	...
	onBuild(dom) {
		this.tree = dom.querySelector('#x2m-nc-files-tree');
...
class NextcloudCalendarsPopupView extends rl.pluginPopupView {
	...
	onBuild(dom) {
		this.tree = dom.querySelector('#x2m-nc-calendars');
		this.tree.addEventListener('click', event => {
			let el = event.target;
			if (el.matches('button')) {
				this.select = el.href;
...
rl.nextcloud = {
	...
	calendarExists: (path, uid) =>
		davFetch('calendars', path + '/' + uid + '.ics', { method: 'HEAD' })
		.then(r => r.status === 200)
		.catch(() => false),
	...
	calendarPut: (path, event) => {
		...
		.then(response => {
			if (response.status !== 201 && response.status !== 204) {
				response.text().then(t => console.warn('Calendar save HTTP ' + response.status, t));
			}
