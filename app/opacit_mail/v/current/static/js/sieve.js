/* opacit_mail Webmail (c) 2026 NK-IT Dev | Licensed under AGPL v3 */
(function () {
	'use strict';
	...
	*/
	...
	function filtersToSieveScript(filters)
	{
		let eol = '\r\n',
			...
			require = {},
			parts = [
				'# This is opacit_mail sieve script.',
	...
	function sieveScriptToFilters(script)
	{
		let regex = /BEGIN:HEADER([\s\S]+?)END:HEADER/gm,
			...
			json,
			filter;
		if (script.length && (script.includes('OPACIT_MAIL:SIEVE') || script.includes('RAINLOOP:SIEVE'))) {
	...
	//			body: this.allowFilters() ? this.body() : this.filtersToRaw()
		...
		/**
		 * Only 'opacit_mail.user' script supports filters
		 */
		allowFilters() {
