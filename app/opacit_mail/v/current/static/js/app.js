/* opacit_mail Webmail (c) opacit_mail | Licensed under AGPL v3 */
(function () {
	'use strict';
	...
	*/
	...
	//			+ b64EncodeJSONSafe(url.replace(/ /g, '%20')),
		...
		themePreviewLink = theme => {
			if (theme.endsWith('@nextcloud')) {
				...
				return parent.OC.webroot + '/themes/' + encodeURI(theme) + '/opacit_mail/preview.png';
			}
	...
	const
		...
		// see /opacit_mail/v/0.0.0/app/localization/relativetimeformat/
	...
	const
		errorTip = (element, value) => value
				? setTimeout(() => element.setAttribute('data-opacit_mailErrorTip', value), 100)
				: element.removeAttribute('data-opacit_mailErrorTip'),
	...
	*/
	...
	isMobile.onchange = e => {
		...
		$htmlCL.toggle('x2m-mobile', e.matches);
		/*$htmlCL.contains('x2m-msgView-side') || */leftPanelDisabled(e.matches);
	};
	isMobile.onchange(isMobile);

	//isSmall.onchange = e => $htmlCL.contains('x2m-msgView-side') && leftPanelDisabled(e.matches);
	...
	const SettingsUserStore = new class {
		constructor() {
			...
			const toggleLayout = () => {
				...
				$htmlCL.toggle('x2m-msgView-side', LayoutSideView === value);
				$htmlCL.toggle('x2m-msgView-bottom', LayoutBottomView === value);
				fireEvent('x2m-layout', value);
			};
	...
	*/
	...
	//		tmpl.content.querySelectorAll('blockquote').forEach(node => {
			[...tmpl.content.querySelectorAll('blockquote')].reverse().forEach(node => {
				const el = createElement('details', {class:'x2m-bq-switcher'});
	...
	//			.replace(/<\/?(html|head)[^>]*>/gi, '')
				// Fix Reddit: strip hidden preview-text spans
				.replace(/<span class="preview-text"[\s\S]+?<\/span>/, '')
				// Replace unicode line separator with space
				.replace(/\u2028/g,' ')
				// Remove extraneous <br> before </p>
				.replace(/<br>\s*<\/p>/gi,'</p>')
				.trim();
			html = '';
	...
	*/
	...
	const contentType = 'opacit_mail/emailaddress',
	...
	*/
						...
						const endShowHide = e => {
							if (e.target === vmDom && 'opacity' === e.propertyName) {
								if (vmDom.classList.contains('animate')) {
									vm.afterShow?.();
									fireEvent('x2m-vm-visible', vm);
								} else {
									vmDom.close();
									vm.afterHide?.();
	//								fireEvent('x2m-vm-hidden', vm);
								}
							}
						};
					...
					fireEvent('x2m-view-model.create', vm);
		...
		screenOnRoute = (screenName, subPart) => {
			...
			if (screenName && fireEvent('x2m-show-screen', screenName + (subPart ?  '/' + subPart : ''), 1)) {
				// Close all popups
	...
	const
		...
		startScreens = screensClasses => {
			...
			setTimeout(() => $htmlCL.remove('x2m-started-trigger'), 100);

			const c = elementById('x2m-content'), l = elementById('x2m-loading');
			c && (c.hidden = false);
			l?.remove();
		},
	...
	fetchJSON = (action, sUrl, params, timeout, jsonCallback) => {
		...
		// Don't abort previous request of same action type
	//	abort(action, 0, 1);
	...
	*/
	...
	class AbstractViewLogin extends AbstractView {
		...
		onShow() {
			elementById('x2m-left').hidden = true;
			elementById('x2m-right').hidden = true;
			rl.route.off();
		}

		onHide() {
			elementById('x2m-left').hidden = false;
			elementById('x2m-right').hidden = false;
		}
			...
			"uids": [
				{
					"name": "demo",
					"comment": "",
					"email": "demo@opacit_mail.dev",
					"uid": "demo <demo@opacit_mail.dev>",
					"revoked": false,
					"invalid": false
				}
			],
...
<body></body>
		...
		msgHtml = msg => cleanHtml(msg.html(), msg.attachments(), '#x2m-msg-' + msg.hash),
	...
	*/
		...
		popupMessage(print) {
			const
				...
				win = open('', 'x2m-win-'+this.requestHash
		...
		bodyAsHTML() {
			if (this.body) {
				...
				clone.querySelectorAll('.x2m-bq-switcher').forEach(
	...
	if (appEl) {
		...
		if (appEl.requestFullscreen) {
			doc.addEventListener(event, () => {
				...
				$htmlCL.toggle('x2m-fullscreen', appFullscreen());
			});
		}
	}
		...
		constructor() {
			...
			addSubscribablesTo(this, {
				message: message => {
					...
					elementById('x2m-right').classList.toggle('message-selected', !!message);
					if (message) {
	...
	MessagelistUserStore.moveMessages = (
		...
		if (messages.length) {
			...
			if (copy) {
				...
				// Select next email after delete
	...
	*/
	...
	const rlContentType = 'opacit_mail/action',
	...
	*/
	...
	download = (link, name = "") => {
		...
		// Firefox 98 issue: download fallback to open in new tab
	...
	class LoginUserView extends AbstractViewLogin {
		...
		submitCommand(self, event) {
			...
			let form = event.target.form,
				...
				valid = form.reportValidity() && fireEvent('x2m-user-login', data, 1);
			...
			if (valid) {
				...
				Remote.request('Login',
					(iError, oData) => {
						fireEvent('x2m-user-login-response', {
							error: iError,
							data: oData
						});
						if (iError) {
	...
	class ContactsPopupView extends AbstractViewPopup {
		constructor() {
			...
			addComputablesTo(this, {
				...
				isBusy: () => ContactUserStore.importing() | ContactUserStore.loading()
					| this.isSaving()
			});
	...
	//					message.flags.valueHasMutated();
		...
		onBuild(dom) {
			...
			setTimeout(() => {
				...
				if (top) {
					fToggle();
					addEventListener('x2m-layout', fToggle);
				}
			}, 1);
	...
	*/
		...
		onBuild() {
			doc.addEventListener('click', event =>
				event.target.closest('#x2m-right') && moveAction(0)
			);
		}
	...
	//			const vmPlace = elementById('V-SettingsPane') || elementById('V-AdminPane);
				...
				if (SettingsViewModelClass.__vm) {
					...
					fireEvent('x2m-view-model.create', settingsScreen);
					...
					fireEvent('x2m-view-model', settingsScreen);
				} else {
	...
	class UserSettingsGeneral extends AbstractViewSettings {
		...
		testSystemNotification() {
			NotificationUserStore.display('opacit_mail', 'Test notification');
		}
		...
		registerMailto() {
			try {
				...
				console.error('registerProtocolHandler:', e);
			}
			this.mailto(0);
		}
	}
	...
	*/
	...
	class AppUser extends AbstractApp {
		constructor() {
			...
			addEventsListener(doc, ['keydown','keyup'], (ev=>$htmlCL.toggle('x2m-ctrl-key-pressed', ev.ctrlKey)).debounce(500));
				...
				loadFolders((success, error) => {
					try {
						if (success) {
							...
							setTimeout(() => {
								const left = elementById('x2m-left'),
									fToggle = () =>
