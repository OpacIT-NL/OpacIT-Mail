...
class Domain extends AbstractProvider
{
	...
	private \opacit_mail\Engine\Plugins\Manager $oPlugins;

	public function __construct(Domain\DomainInterface $oDriver, \opacit_mail\Engine\Plugins\Manager $oPlugins)
	{
	...
	public function SaveAlias(string $sName, string $sAlias) : bool
	{
		if ($this->Load($sName, false, false)) {
			throw new ClientException(\opacit_mail\Engine\Notifications::DomainAlreadyExists->value);
		}
	...
	public function LoadOrCreateNewFromAction(\opacit_mail\Engine\Actions $oActions, ?string $sNameForTest = null) : ?\opacit_mail\Engine\Model\Domain
	{
		...
		if (\strlen($sName) || $sNameForTest) {
			...
			return \opacit_mail\Engine\Model\Domain::fromArray($sNameForTest ?: $sName, [
	...
	public function getByEmailAddress(string $sEmail) : \opacit_mail\Engine\Model\Domain
	{
		$oDomain = $this->Load(\opacit_mail\Mail\Base\Utils::getEmailAddressDomain($sEmail), true);
		if (!$oDomain) {
