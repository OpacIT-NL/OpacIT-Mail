...
class FileStorage implements \opacit_mail\Engine\Providers\Files\IFiles
{
	/**
	 * @var array
	 */
	...
	public function PutFile(\opacit_mail\Engine\Model\Account $oAccount, string $sKey, /*resource*/ $rSource) : bool
	{
		...
		if ($rSource) {
			...
			if ($rOpenOutput) {
				$bResult = (false !== \opacit_mail\Mail\Base\Utils::WriteStream($rSource, $rOpenOutput));
				\fclose($rOpenOutput);
			}
		}
		return $bResult;
	...
	public function GetFileName(\opacit_mail\Engine\Model\Account $oAccount, string $sKey) /*: string|false*/
	{
	...
	public function FileSize(\opacit_mail\Engine\Model\Account $oAccount, string $sKey) /*: int|false*/
	{
		$sFileName = $this->generateFullFileName($oAccount, $sKey);
		...
		if (0 < $iTimeToClearInHours) {
			...
			foreach (\glob("{$this->sDataPath}/*", GLOB_ONLYDIR) as $sDomain) {
				foreach (\glob("{$sDomain}/*", GLOB_ONLYDIR) as $sLocal) {
					\opacit_mail\Mail\Base\Utils::RecTimeDirRemove("{$sLocal}/.files", $iTimeToClear);
				}
			}
			// Old
			\opacit_mail\Mail\Base\Utils::RecTimeDirRemove("{$this->sDataPath}/files", $iTimeToClear);
	...
	private function generateFullFileName(\opacit_mail\Engine\Model\Account $oAccount, string $sKey, bool $bMkDir = false) : string
	{
		...
		$sFilePath = $this->sDataPath
			...
			.($sSubEmail ? '/'.\opacit_mail\Mail\Base\Utils::SecureFileName($sSubEmail) : '')
			.'/.files/'.\sha1($sKey);

		$bMkDir && \opacit_mail\Mail\Base\Utils::mkdir(\dirname($sFilePath));
