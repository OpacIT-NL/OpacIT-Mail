...
class Manager
{
	...
	private \opacit_mail\Engine\Actions $oActions;
	...
	public function __construct(\opacit_mail\Engine\Actions $oActions)
	{
	...
	public function CreatePluginByName(string $sName) : ?\opacit_mail\Engine\Plugins\AbstractPlugin
	{
		$oPlugin = null;
		...
		if ($sClassName) {
			...
			$oPlugin
				...
				->SetPluginConfig(new \opacit_mail\Engine\Config\Plugin($sName, $oPlugin->ConfigMap(true)))
			;
	...
	public function loadPluginByName(string $sName) : ?string
	{
		if (\preg_match('/^[a-z0-9\\-]+$/', $sName)) {
			...
			if (\class_exists($sClassName) && \is_subclass_of($sClassName, 'opacit_mail\\Engine\\Plugins\\AbstractPlugin')) {
				return $sClassName;
			}
	...
	public function Actions() : \opacit_mail\Engine\Actions
	{
	...
	public function InitAppData(bool $bAdmin, array &$aAppData, ?\opacit_mail\Engine\Model\Account $oAccount = null) : self
	{
		if ($this->bIsEnabled && isset($aAppData['Plugins']) && \is_array($aAppData['Plugins'])) {
			...
			foreach ($this->aPlugins as $oPlugin) {
				if ($oPlugin) {
					...
					if (\is_array($aMap)) {
						foreach ($aMap as /* @var $oPluginProperty \opacit_mail\Engine\Plugins\Property */ $oPluginProperty) {
