...
abstract class AbstractPlugin
{
	const
		NAME     = '',
		AUTHOR   = 'opacit_mail',
		URL      = 'https://github.com/NK-IT-CLOUD/opacit_mail',
		VERSION  = '0.0',
		RELEASE  = '2020-11-01',
		REQUIRED = '2.0.0',
//		DEPRECATED = '',
	...
	/**
	 * @var \opacit_mail\Engine\Config\Plugin
	 */
	...
	final public function Config() : \opacit_mail\Engine\Config\Plugin
	{
		return $this->oPluginConfig;
	}

	final public function Manager() : \opacit_mail\Engine\Plugins\Manager
	{
	...
	final public function ConfigMap(bool $flatten = false) : array
	{
		...
		if ($flatten) {
			...
			foreach ($this->aConfigMap as $oItem) {
				if ($oItem) {
					if ($oItem instanceof \opacit_mail\Engine\Plugins\Property) {
						$result[] = $oItem;
					} else if ($oItem instanceof \opacit_mail\Engine\Plugins\PropertyCollection) {
						foreach ($oItem as $oSubItem) {
							if ($oSubItem && $oSubItem instanceof \opacit_mail\Engine\Plugins\Property) {
								$result[] = $oSubItem;
							}
						}
					}
	...
	final public function SetPluginManager(\opacit_mail\Engine\Plugins\Manager $oPluginManager) : self
	{
	...
	final public function SetPluginConfig(\opacit_mail\Engine\Config\Plugin $oPluginConfig) : self
	{
