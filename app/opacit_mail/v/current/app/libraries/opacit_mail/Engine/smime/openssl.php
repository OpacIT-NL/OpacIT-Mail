...
use opacit_mail\Engine\File\Temporary;
...
class OpenSSL
{
	...
	public function storeCertificate(string $certificate) : bool
	{
		...
		if (!$data) {
			\opacit_mail\Engine\Log::error('OpenSSL', "parse: " . \openssl_error_string());
			return false;
		}
		...
		if (\file_exists($filename)) {
			\opacit_mail\Engine\Log::debug('OpenSSL', "certificate {$key} already imported");
		} else {
			\file_put_contents("{$this->homedir}/{$key}.crt", $certificate);
//			\unlink("{$this->homedir}/certificates.json");
			$this->certificates(true);
			\opacit_mail\Engine\Log::debug('OpenSSL', "certificate {$key} imported");
		}
		return true;
	}
...
//						'validTo' => \gmdate('Y-m-d\\TH:i:s\\Z', $data['validTo_time_t']),
					...
					\opacit_mail\Engine\Log::error('OpenSSL', "parse({$file}): " . \openssl_error_string());
				}
			}
...
//		\glob("{$this->homedir}/*.key");
	...
	public function setPrivateKey(/*OpenSSLAsymmetricKey|string*/$privateKey,
		?\opacit_mail\Engine\SensitiveString $passphrase = null
	) : void
	{
	...
	public function exportPrivateKey(?\opacit_mail\Engine\SensitiveString $passphrase = null): string
	{
		if (!$this->privateKey) {
