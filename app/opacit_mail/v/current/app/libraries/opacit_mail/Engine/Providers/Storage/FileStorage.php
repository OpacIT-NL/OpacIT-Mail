...
class FileStorage implements \opacit_mail\Engine\Providers\Storage\IStorage
{
	...
	public function Put($mAccount, int $iStorageType, string $sKey, string $sValue) : bool
	{
		...
		try {
			$sFileName && \opacit_mail\Engine\Utils::saveFile($sFileName, $sValue);
			return true;
		} catch (\Throwable $e) {
			\opacit_mail\Engine\Log::warning('FileStorage', $e->getMessage());
		}
		return false;
	}
	...
	public function DeleteStorage($mAccount) : bool
	{
		...
		if ($sPath && \is_dir($sPath)) {
			\opacit_mail\Mail\Base\Utils::RecRmDir($sPath);
		}
		return true;
	}
	...
	public function GenerateFilePath($mAccount, int $iStorageType, bool $bMkDir = false) : string
	{
		...
		if (null === $mAccount || StorageType::NOBODY->value === $iStorageType) {
			...
			if ($mAccount instanceof \opacit_mail\Engine\Model\MainAccount) {
				$sEmail = $mAccount->Email();
			} else if ($mAccount instanceof \opacit_mail\Engine\Model\AdditionalAccount) {
			...
			switch ($iStorageType)
			{
				...
				case StorageType::ROOT->value:
					...
					if (\is_dir("{$this->sDataPath}/cfg")) {
						\opacit_mail\Engine\Upgrade::FileStorage($this->sDataPath);
					}
					...
					$sFilePath = $this->sDataPath
						...
						.'/'.($sSubFolder ? \opacit_mail\Mail\Base\Utils::SecureFileName($sSubFolder).'/' : '');
					break;
				default:
		...
		$bMkDir && $sFilePath && \opacit_mail\Mail\Base\Utils::mkdir($sFilePath);
	...
	protected function generateFileName($mAccount, int $iStorageType, string $sKey, bool $bMkDir = false) : string
	{
		...
		if ($sFilePath) {
			if (StorageType::NOBODY->value === $iStorageType) {
				...
				$sFilePath .= ($sKey ? \opacit_mail\Mail\Base\Utils::SecureFileName($sKey) : '');
			}
		}
		return $sFilePath;
	}
	...
	public function GC() : void
	{
		...
		foreach (\glob("{$this->sDataPath}/*", GLOB_ONLYDIR) as $sDomain) {
			foreach (\glob("{$sDomain}/*", GLOB_ONLYDIR) as $sLocal) {
				\opacit_mail\Mail\Base\Utils::RecTimeDirRemove("{$sLocal}/.sign_me", 3600 * 24 * 30); // 30 days
				\opacit_mail\Mail\Base\Utils::RecTimeDirRemove("{$sLocal}/.sessions", 3600 * 3); // 3 hours
			}
		}
	}
}
