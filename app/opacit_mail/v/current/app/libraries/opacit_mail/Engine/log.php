...
abstract class Log
{
	...
	{
		if (\opacit_mail\Engine\Api::Logger()->IsEnabled()) {
			\opacit_mail\Engine\Api::Logger()->Write($msg, $level, $prefix);
		} else {
/*
			static $log_level;
			// Default to level 4, 0 = LOG_EMERG, 7 = LOG_DEBUG
			if (!$log_level) {
				$log_level = \max(3, \opacit_mail\Engine\Api::Config()->Get('logs', 'level', \LOG_WARNING));
			}
			if ($level <= $log_level) {
				if (\opacit_mail\Engine\Api::Config()->Get('logs', 'syslog') && \openlog('opacit_mail', \LOG_ODELAY, \LOG_USER)) {
...
//					\error_log($prefix . ' ' . static::$levels[$level] . ': ' . $msg, 3, 'filename');
			...
			if (\class_exists('OC')) {
//				\OCP\Log\logger('opacit_mail')->log(\intval($level), $msg);
				\OCP\Log\logger('opacit_mail')->{static::$levels[$level]}($msg);
			}
*/
		}
	}
