...
//				\ini_set('display_errors', '1');
			...
			\opacit_mail\Mail\Config::$BoundaryPrefix = \trim($oConfig->Get('labs', 'boundary_prefix', ''));
		}
		return $oConfig;
	}

	public static function getCSP(?string $sScriptNonce = null) : \opacit_mail\Engine\HTTP\CSP
	{
		$oConfig = static::Config();
		$CSP = new \opacit_mail\Engine\HTTP\CSP(\trim($oConfig->Get('security', 'content_security_policy', '')));
		$CSP->report = $oConfig->Get('security', 'csp_report', false);
		$CSP->report_only = $oConfig->Get('debug', 'enable', false); // || OPACIT_MAIL_DEV
	...
	public static function Logger() : \opacit_mail\Mail\Log\Logger
	{
		static $oLogger = null;
		if (!$oLogger) {
			$oConfig = static::Config();
			$oLogger = new \opacit_mail\Mail\Log\Logger(true);
			...
			if ($oConfig->Get('debug', 'enable', false)) {
				...
				$oLogger->SetLevel(\max(3, \opacit_mail\Engine\Api::Config()->Get('logs', 'level', \LOG_WARNING)));
			}
		}
		return $oLogger;
	}
	...
	{
		$sSsoHash = \opacit_mail\Mail\Base\Utils::Sha1Rand(\sha1($sPassword.$sEmail));

		return static::Actions()->Cacher()->Set(
			KeyPathHelper::SsoCacherKey($sSsoHash),
			\opacit_mail\Engine\Crypt::EncryptToJSON(array(
				'Email' => $sEmail,
				'Password' => $sPassword,
