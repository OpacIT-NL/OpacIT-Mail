...
namespace opacit_mail\Engine\Config;

class Application extends \opacit_mail\Engine\Config\AbstractConfig
{
	...
	public function __construct()
	{
		parent::__construct('application.ini',
			'; opacit_mail configuration file
; Please don\'t add custom parameters here, those will be overwritten',
	...
	public function Load() : bool
	{
		...
		if ((\is_array($_ENV) && \count($_ENV)) ||
			(\is_array($_SERVER) && \count($_SERVER)))
		{
		...
		if (!$sCipher || !\opacit_mail\Engine\Crypt::cipherSupported($sCipher)) {
			$sCipher && \opacit_mail\Engine\Log::warning('Crypt', "OpenSSL no support for cipher '{$sCipher}'");
			$aCiphers = \opacit_mail\Engine\Crypt::listCiphers();
	...
	public function Set(string $sSectionKey, string $sParamKey, $mParamValue) : void
	{
		...
		if ('language' === $sParamKey) {
			$mParamValue = \opacit_mail\Engine\L10n::validLanguage($mParamValue, false) ?: 'en';
		}
	...
	protected function defaultValues() : array
	{
		...
		return array(
			...
			'webmail' => array(
				...
				'theme'                       => array('opacit_mail', 'Theme used by default'),
...
Warning: only enable when server does not do this, else double compression errors occur')
			...
			'contacts' => array(
				'enable'            => array(true, 'Enable contacts'),
