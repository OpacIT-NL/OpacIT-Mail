...
use opacit_mail\Engine\Providers\Suggestions;
...
trait User
{
	...
	public function DoLogin() : array
	{
		try {
			$oAccount = $this->LoginProcess(
				\opacit_mail\Mail\Base\Utils::Trim($this->GetActionParam('Email', '')),
				new \opacit_mail\Engine\SensitiveString($this->GetActionParam('Password', ''))
			);
	...
	public function DoAppDelayStart() : array
	{
		...
		$sTimers = $this->StorageProvider()->Get(null,
			\opacit_mail\Engine\Providers\Storage\Enumerations\StorageType::NOBODY->value, 'Cache/Timers', '');
...
//			$oSettingsLocal->SetConf('Theme', $this->ValidateTheme($oConfig->Get('webmail', 'theme', 'Default')));
		...
		$this->setSettingsFromParams($oSettings, 'Layout', 'int', function ($iValue) {
			return (int) (\in_array((int) $iValue, array(\opacit_mail\Engine\Enumerations\Layout::NO_PREVIEW->value,
				\opacit_mail\Engine\Enumerations\Layout::SIDE_PREVIEW->value, \opacit_mail\Engine\Enumerations\Layout::BOTTOM_PREVIEW->value)) ?
					$iValue : \opacit_mail\Engine\Enumerations\Layout::SIDE_PREVIEW->value);
		});
	...
	private function setSettingsFromParams(\opacit_mail\Engine\Settings $oSettings, string $sConfigName, string $sType = 'string', ?callable $cCallback = null) : void
	{
