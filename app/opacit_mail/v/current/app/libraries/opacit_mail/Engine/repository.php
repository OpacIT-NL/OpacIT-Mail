...
abstract class Repository
{
	...
	public static function getLatestReleaseVersion() : ?string
	{
		try {
			...
			$oHTTP->timeout = 5;
			$response = $oHTTP->doRequest('GET', 'https://api.github.com/repos/NK-IT-CLOUD/opacit_mail/releases/latest');
	...
	public static function getEnabledPackagesNames() : array
	{
		return \array_map('trim',
			\explode(',', \strtolower(\opacit_mail\Engine\Api::Config()->Get('plugins', 'enabled_list', '')))
		);
	}
	...
	public static function getPackagesList() : array
	{
		...
		foreach (\opacit_mail\Engine\Api::Actions()->Plugins()->InstalledPlugins() as $aItem) {
			if ($aItem) {
				$aList[] = [
					'type' => 'plugin',
					'id' => $aItem[0],
	...
	public static function deletePackage(string $sId) : bool
	{
		...
		return (!\is_dir($sPath) || \opacit_mail\Mail\Base\Utils::RecRmDir($sPath))
