...
class Cookies
{
	...
	public static function getSecure(string $sName)
	{
		return isset($_COOKIE[$sName])
			? Crypt::DecryptFromJSON(\opacit_mail\Mail\Base\Utils::UrlSafeBase64Decode(static::get($sName)))
			: null;
	}
	...
	public static function setSecure(string $sName, $data): void
	{
		if (\is_null($data)) {
			...
			static::set(
				$sName,
				\opacit_mail\Mail\Base\Utils::UrlSafeBase64Encode(Crypt::EncryptToJSON($data))
			);
		}
	}
...
*/
		...
		foreach (\str_split($sValue, $iMaxSize) as $i => $sPart) {
			...
			self::_set($sCookieName, $sPart, $iExpire, $httponly);
		}
		...
		foreach (\array_keys($_COOKIE) as $sCookieName) {
			...
			if (isset($aSplit[1]) && $aSplit[0] == $sName && $aSplit[1] > $i) {
				...
				self::_set($sCookieName, '', 0, $httponly);
			}
		}
	}
