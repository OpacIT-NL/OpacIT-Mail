 ...
 *
 * GET https://keys.openpgp.org/pks/lookup?op=get&options=mr&search=security@opacit_mail.dev
...
namespace opacit_mail\Engine\PGP;
...
*/
	...
	private static function fetch(string $host, string $op, string $search, bool $fingerprint = false, bool $exact = false) : ?\opacit_mail\Engine\HTTP\Response
	{
		...
		\opacit_mail\Engine\Log::debug('PGP', $url);
		return self::HTTP()->doRequest('GET', $url);
	}

	private static $HTTP;
	private static function HTTP() : \opacit_mail\Engine\HTTP\Request
	{
		if (!self::$HTTP) {
			self::$HTTP = \opacit_mail\Engine\HTTP\Request::factory(/*'socket' or 'curl'*/);
			self::$HTTP->max_response_kb = 0;
			self::$HTTP->timeout = 15; // timeout in seconds.
		}
		return self::$HTTP;
	}
...
*/
		foreach (static::$hosts as $host) {
			$oResponse = self::fetch($host, 'get', $keyId);
			if (!$oResponse) {
				\opacit_mail\Engine\Log::info('PGP', "No response for key {$keyId} on {$host}");
				continue;
			}
			if (200 !== $oResponse->status) {
				\opacit_mail\Engine\Log::debug('PGP', "{$oResponse->status} for key {$keyId} on {$host}");
				continue;
			}
	...
	public static function index(string $search, bool $fingerprint = true, bool $exact = false) : array
	{
		$keys = [];
		foreach (static::$hosts as $host) {
			$oResponse = self::fetch($host, 'index', $search, $fingerprint, $exact);
			if (!$oResponse) {
				\opacit_mail\Engine\Log::info('PGP', "No response for key {$search} on {$host}");
				continue;
			}
			if (200 !== $oResponse->status) {
				\opacit_mail\Engine\Log::debug('PGP', "{$oResponse->status} for search `{$search}` on {$host}");
				continue;
			}
			...
			foreach ($result as $line) {
				...
				if ('pub' === $line[0]) {
					...
					// Ignore invalid line
					if (7 !== \count($line)) {
						\opacit_mail\Engine\Log::info('PGP', "Invalid pub line for search `{$search}` on {$host}");
						continue;
					}
...
//						'expired'  => \str_contains($line[6], 'e'),
				...
				// uid:<escaped uid string>:<creationdate>:<expirationdate>:<flags>
					...
					// Ignore invalid line
					if (5 !== \count($line)) {
						\opacit_mail\Engine\Log::info('PGP', "Invalid uid line for search `{$search}` on {$host}");
						continue;
					}
					// Ignore flagged or expired key
...
//						'expired'  => \str_contains($line[4], 'e'),
	 ...
	 *
	 * @param string $host
	 * @param string $key  The PGP public key.
	 *
	 * @throws \Exception
	 */
...
/*
		...
		if (!$this->get($key->id)) {
			...
			self::HTTP()->doRequest('POST', "{$host}/pks/add", 'keytext=' . $keytext, [
