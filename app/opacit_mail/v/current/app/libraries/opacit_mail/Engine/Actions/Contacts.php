...
{
	/**
	 * @var \opacit_mail\Engine\Providers\AddressBook
	 */
	protected $oAddressBookProvider = null;

	public function AddressBookProvider(?\opacit_mail\Engine\Model\Account $oAccount = null): \opacit_mail\Engine\Providers\AddressBook
	{
...
//				if ($this->oConfig->Get('contacts', 'enable', false)) {
				...
				\opacit_mail\Engine\Log::error('AddressBook', $e->getMessage()."\n".$e->getTraceAsString());
				$oDriver = null;
//				$oDriver = new \opacit_mail\Engine\Providers\AddressBook\PdoAddressBook();
			}
			$this->oAddressBookProvider = new \opacit_mail\Engine\Providers\AddressBook($oDriver);
		...
		if ($this->HasActionParam('uid') && $this->HasActionParam('jCard')) {
			...
			if ($oAddressBookProvider && $oAddressBookProvider->IsActive()) {
				...
				if ($vCard && $vCard instanceof \Sabre\VObject\Component\VCard) {
					...
					$vCard->PRODID = 'opacit_mail-'.APP_VERSION;
					$sUid = \trim($this->GetActionParam('uid'));
					$oContact = $sUid ? $oAddressBookProvider->GetContactByID($sUid) : null;
					if (!$oContact) {
						$oContact = new \opacit_mail\Engine\Providers\AddressBook\Classes\Contact();
					}
	...
	public function UploadContacts(?array $aFile, int $iError) : array
	{
		...
		if ($oAccount && UPLOAD_ERR_OK === $iError && \is_array($aFile)) {
			...
			if (!$this->FilesProvider()->MoveUploadedFile($oAccount, $sSavedName, $aFile['tmp_name'])) {
				$iError = \opacit_mail\Engine\Enumerations\UploadError::ON_SAVING->value;
			} else {
		...
		if (UPLOAD_ERR_OK !== $iError) {
			$iClientError = 0;
			$sError = \opacit_mail\Engine\Enumerations\UploadError::getUserMessage($iError, $iClientError);
			if (!empty($sError)) {
	...
	private function importContactsFromVcfFile(\opacit_mail\Engine\Model\Account $oAccount, /*resource*/ $rFile): int
	{
		$iCount = 0;
		$oAddressBookProvider = $this->AddressBookProvider($oAccount);
		if (\is_resource($rFile) && $oAddressBookProvider && $oAddressBookProvider->IsActive()) {
			try
			{
				...
				foreach (\opacit_mail\Engine\Providers\AddressBook\Utils::VcfStreamToContacts($rFile) as $oContact) {
	...
	private function importContactsFromCsvFile(\opacit_mail\Engine\Model\Account $oAccount, /*resource*/ $rFile, string $sFileStart): int
	{
		$iCount = 0;
		$oAddressBookProvider = $this->AddressBookProvider($oAccount);
		if (\is_resource($rFile) && $oAddressBookProvider && $oAddressBookProvider->IsActive()) {
			try
			{
				...
				foreach (\opacit_mail\Engine\Providers\AddressBook\Utils::CsvStreamToContacts($rFile, $sDelimiter) as $oContact) {
					if ($oAddressBookProvider->ContactSave($oContact)) {
						++$iCount;
					}
				}
