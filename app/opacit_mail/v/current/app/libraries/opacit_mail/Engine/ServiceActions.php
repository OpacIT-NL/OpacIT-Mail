...
{
	protected \opacit_mail\Mail\Base\Http $oHttp;
	...
	public function __construct(\opacit_mail\Mail\Base\Http $oHttp, Actions $oActions)
	{
		$this->oHttp = $oHttp;
		$this->oActions = $oActions;
	}

	private function Logger() : \opacit_mail\Mail\Log\Logger
	{
	...
	private function Cacher() : \opacit_mail\Mail\Cache\CacheClient
	{
...
//						$this->oActions->logMask($value);
			...
			if (!\is_array($aResponse)) {
				throw new Exceptions\ClientException(Notifications::UnknownError->value);
			}
		}
		catch (\Throwable $oException)
		{
			\opacit_mail\Engine\Log::warning('SERVICE', "{$oException}");
			if ($e = $oException->getPrevious()) {
				\opacit_mail\Engine\Log::warning('SERVICE', "- {$e}");
			}
...
//				\is_uploaded_file($_FILES[$sInputName]['tmp_name'])
				...
				if (UPLOAD_ERR_OK === $iError && 0 < $iSizeLimit && $iSizeLimit < (int) $_FILES[$sInputName]['size']) {
					$iError = Enumerations\UploadError::CONFIG_SIZE->value;
				}
				...
				$iError = Enumerations\UploadError::EMPTY_FILE->value;
			}
	...
	public function ServiceProxyExternal() : string
	{
		...
		if ($sData && $this->Config()->Get('labs', 'use_local_proxy_for_external_images', false)) {
			...
			$sUrl = \opacit_mail\Mail\Base\Utils::UrlSafeBase64Decode($sData);
			if (!empty($sUrl)) {
				\header('X-Content-Location: '.$sUrl);
				$tmp = \tmpfile();
				$HTTP = \opacit_mail\Engine\HTTP\Request::factory();
				...
				if ($oResponse) {
					$sContentType = \opacit_mail\Engine\File\MimeType::fromStream($tmp) ?: $oResponse->getHeader('content-type');
					if (200 === $oResponse->status && \str_starts_with($sContentType, 'image/')) {
						try {
							...
							\opacit_mail\Engine\Log::error('Proxy', \get_class($HTTP) . ': ' . $e->getMessage());
						}
					} else {
		...
		\opacit_mail\Mail\Base\Http::StatusHeader(404);
		return '';
	}

	public function ServiceCspReport() : void
	{
		\opacit_mail\Engine\HTTP\CSP::logReport();
	}
...
//					error_log(print_r($e,1));
		...
		{
			$sRawError = Notifications::AuthError->value == $oException->getCode()
	...
	public function ServiceMailto() : string
	{
		...
		if (\preg_match('/^mailto:/i', $sTo)) {
			\opacit_mail\Engine\Cookies::set(
		...
		\opacit_mail\Mail\Base\Http::Location('./');
		return '';
	}
	...
	{
		$this->oHttp->ServerNoCache();
		\opacit_mail\Engine\Integrity::test();
		return '';
	}

	/**
	 * Login with the \opacit_mail\Engine\API::CreateUserSsoHash() generated hash
	 */
	public function ServiceSso() : string
	{
		...
		if (!empty($sSsoHash)) {
			...
			if (!empty($sSsoSubData)) {
				$aData = \opacit_mail\Engine\Crypt::DecryptFromJSON($sSsoSubData, $sSsoHash);
					...
					(\time() - 10 < $aData['Time']))
				{
					...
					try
					{
						$oAccount = $this->oActions->LoginProcess(
							\trim($aData['Email']),
							new \opacit_mail\Engine\SensitiveString($aData['Password'])
						);
