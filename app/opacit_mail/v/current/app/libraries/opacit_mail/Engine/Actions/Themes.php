...
trait Themes
{
	public function GetTheme(bool $bAdmin): string
	{
		...
		if (!$sTheme) {
			...
			if (!$bAdmin
			 ...
			 && $this->GetCapa(Capa::THEMES->value)
	...
	public function GetThemes(): array
	{
		...
		if (\class_exists('OC', false)) {
			...
			if (\is_dir($sDir) && ($rDirH = \opendir($sDir))) {
				while (($sFile = \readdir($rDirH)) !== false) {
					if ('.' !== $sFile[0] && \is_dir("{$sDir}/{$sFile}") && \file_exists("{$sDir}/{$sFile}/opacit_mail/style.css")) {
	...
	public function compileCss(string $sTheme, bool $bAdmin) : string
	{
		...
		if ('@nextcloud' === \substr($sTheme, -10)) {
			...
			$sBase = \OC::$WEBROOT . '/';
			/** @phpstan-ignore class.notFound */
			$sThemeCSSFile = \OC::$SERVERROOT . '/themes/' . \str_replace('@nextcloud', '/opacit_mail/style.css', $sTheme);
		} else {
			$bCustomTheme = '@custom' === \substr($sTheme, -7);
			if ($bCustomTheme) {
				$sTheme = \substr($sTheme, 0, -7);
				$sBase = \opacit_mail\Engine\Utils::WebPath();
			} else {
				$sBase = \opacit_mail\Engine\Utils::WebVersionPath();
			}
			$sBase .= "themes/{$sTheme}/";
...
//			: \str_replace(';}', '}', \preg_replace('/\\s*([:;{},])\\s*/', '\1', \preg_replace('/\\s+/', ' ', \preg_replace('#/\\*.*?\\*/#s', '', $mResult))));
	...
	public function UploadBackground(?array $aFile, int $iError): array
	{
		...
		if ($oAccount && UPLOAD_ERR_OK === $iError && \is_array($aFile)) {
			$sMimeType = \opacit_mail\Engine\File\MimeType::fromFile($aFile['tmp_name'], $aFile['name'])
			...
			if (\in_array($sMimeType, array('image/png', 'image/jpg', 'image/jpeg', 'image/webp'))) {
				$sSavedName = 'upload-post-' . \md5($aFile['name'] . $aFile['tmp_name'])
					. \opacit_mail\Engine\File\MimeType::toExtension($sMimeType);
					...
					if (\is_resource($rData)) {
						...
						if (!empty($sData) && \strlen($sData)) {
							...
							if ($this->StorageProvider()->Put($oAccount,
								...
								\opacit_mail\Engine\Utils::jsonEncode(array(
								...
								if ($oSettings) {
									$sHash = \opacit_mail\Mail\Base\Utils::Sha1Rand($sName . APP_VERSION . APP_SALT);
				...
				$iError = \opacit_mail\Engine\Enumerations\UploadError::FILE_TYPE->value;
			}
		}
