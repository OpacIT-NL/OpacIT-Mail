...
use opacit_mail\Engine\PGP\Backup;
use opacit_mail\Engine\PGP\Keyservers;
use opacit_mail\Engine\PGP\GnuPG;
...
trait Pgp
{
	 ...
	 */
	public function GnuPG() : ?\opacit_mail\Engine\PGP\PGPInterface
	{
		...
		$homedir = \rtrim($this->StorageProvider()->GenerateFilePath(
			$oAccount,
			\opacit_mail\Engine\Providers\Storage\Enumerations\StorageType::ROOT->value
		), '/') . '/.gnupg';

		\opacit_mail\Mail\Base\Utils::mkdir($homedir);
		...
		if (80 < \strlen($homedir)) {
			...
			$tmpdir = \sys_get_temp_dir() . '/opacit_mail';
//			if (\opacit_mail\Engine\Utils::inOpenBasedir($tmpdir) &&
...
//				if (\opacit_mail\Engine\Utils::inOpenBasedir($tmpdir) &&
	...
	public function DoGnupgDecrypt() : array
	{
		...
		$oPassphrase = new \opacit_mail\Engine\SensitiveString($this->GetActionParam('passphrase', ''));
...
//			$result['signatures'] = $oPart->SubParts[0];
	...
	public function DoGnupgExportKey() : array
	{
		$oPassphrase = $this->GetActionParam('isPrivate', '')
			? new \opacit_mail\Engine\SensitiveString($this->GetActionParam('passphrase', ''))
			: null;
		$GPG = $this->GnuPG();
...
//			$sMicAlg = $this->GetActionParam('micAlg', '');
			...
			if ($sSigPartId) {
				$aParts[] = FetchType::BODY_PEEK->value.'['.$sSigPartId.']';
			}
			...
			if ($sSigPartId) {
				...
				// clearsigned text
				$result = [
					'text' => $oFetchResponse->GetFetchValue(FetchType::BODY->value.'['.$sPartId.']'),
					'signature' => ''
				];
				$decode = (new \opacit_mail\Mail\Mime\HeaderCollection($sBodyMime))->ValueByName(MimeEnumHeader::CONTENT_TRANSFER_ENCODING->value);
				if ('base64' === $decode) {
