...
namespace opacit_mail\Engine\HTTP\Request;

use opacit_mail\Engine\HTTP\Response;

class Socket extends \opacit_mail\Engine\HTTP\Request
{
			...
			if (401 === $code && $this->auth['user'] && !isset($extra_headers['Authorization'])) {
				...
				if ($this->auth['type'] & self::AUTH_DIGEST && \preg_match("/WWW-Authenticate:\\s+Digest\\s+([^\\r\\n]*)/i", $data, $match)) {
					...
					if (empty($challenge['qop'])) {
						...
						$challenge['cnonce'] = 'Req2.' . \random_int(0, PHP_INT_MAX);
