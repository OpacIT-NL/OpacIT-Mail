...
use opacit_mail\Engine\Enumerations\PluginPropertyType;
...
class Property implements \JsonSerializable
{
	...
	private PluginPropertyType $iType = PluginPropertyType::STRING;
	...
	public function SetType(int|PluginPropertyType $iType) : self
	{
		$this->iType = $iType instanceof PluginPropertyType ? $iType : PluginPropertyType::from($iType);
		return $this;
	}
...
//				throw new \Exception('Not allowed to set group value');
	...
	public function jsonSerialize()
	{
		...
		if ($this->encrypted && $mValue) try {
			$mValue = \opacit_mail\Engine\Crypt::DecryptFromJSON($mValue, \APP_SALT);
		} catch (\Throwable $e) {
		}
		return array(
