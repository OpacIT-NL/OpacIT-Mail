...
use opacit_mail\Mail\Imap\Enumerations\FolderType;
...
trait Folders
{
	...
	public function DoFolderAppend(): array
	{
		...
		if (\UPLOAD_ERR_OK != $_FILES['appendFile']['error']) {
			...
			return $this->FalseResponse($iErrorCode, \opacit_mail\Engine\Enumerations\UploadError::getMessage($iErrorCode));
		}
...
//			FolderInformation(string $sFolderName, int $iPrevUidNext = 0, array $aUids = array())
	...
	public function DoFolderDelete() : array
	{
		...
		catch (\opacit_mail\Mail\Client\Exceptions\NonEmptyFolder $oException)
		{
	...
	public function DoFolderInformation() : array
	{
		...
		try
		{
			return $this->DefaultResponse($this->MailClient()->FolderInformation(
				...
				new \opacit_mail\Mail\Imap\SequenceSet($this->GetActionParam('flagsUids', []))
			));
		}
		catch (\Throwable $oException)
		{
	...
	{
//		$oImapClient->FolderSetACL('INBOX', 'demo@opacit_mail.dev', 'lrwstipekxacd');
//		$oImapClient->FolderSetACL($sFolderFullName, 'demo@opacit_mail.dev', 'lrwstipekxacd');
//		$oImapClient->FolderSetACL($sFolderFullName, 'foobar@opacit_mail.dev', 'lr');
		$this->initMailClientConnection();
