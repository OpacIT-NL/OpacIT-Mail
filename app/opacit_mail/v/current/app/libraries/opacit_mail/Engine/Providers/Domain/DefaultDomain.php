...
use opacit_mail\Mail\Cache\CacheClient;
...
class DefaultDomain implements DomainInterface
{
	...
	{
//		return ('*' === $sName) ? 'default' : \str_replace('*', '_wildcard_', \opacit_mail\Engine\IDN::toAscii($sName));
...
//		$aList = \glob($this->sDomainPath.'/*.{ini,json,alias}', GLOB_BRACE);
		...
		foreach ($aList as $sFile) {
			...
			if ('default' === $sName || false !== \strpos($sName, '_wildcard_')) {
				$aNames[] = self::decodeFileName($sName);
			}
		}
...
//		$sName = \opacit_mail\Engine\IDN::toAscii($sName);
		...
		if ($bFindWithWildCard) {
			...
			$sFoundValue = '';
			if (\strlen($sNames)
			 && \opacit_mail\Engine\Plugins\Helper::ValidateWildcardValues($sName, $sNames, $sFoundValue)
			 && \strlen($sFoundValue)
			) {
	...
	{
		$sRealFileName = self::encodeFileName($oDomain->Name());
		if (!$sRealFileName) {
			return false;
		}
		$this->Delete($oDomain->Name());
		\opacit_mail\Engine\Utils::saveFile("{$this->sDomainPath}/{$sRealFileName}.json", \json_encode($oDomain, \JSON_PRETTY_PRINT));
	...
	{
//		$sTarget = \opacit_mail\Engine\IDN::toAscii($sTarget);
//		$sTarget = self::encodeFileName($sTarget);
		$sTarget = \strtolower(\idn_to_ascii($sTarget));
		$sRealFileName = self::encodeFileName($sName);
		if (!$sRealFileName || !$sTarget/* || !\is_readable("{$this->sDomainPath}/{$sTarget}.json")*/) {
			return false;
		}
//		$this->Delete($sName);
		\opacit_mail\Engine\Utils::saveFile("{$this->sDomainPath}/{$sRealFileName}.alias", $sTarget);
		$this->oCacher && $this->oCacher->Delete(static::CACHE_KEY);
		return true;
	}
	...
	protected function getDisabled() : array
	{
		...
		$aDisabled = array();
		// Legacy format used comma, we use newline
		$sItem = \strtok($sFile, ",\n");
		while (false !== $sItem) {
//			$aDisabled[] = \opacit_mail\Engine\IDN::toAscii($sItem);
...
//		$sName = \opacit_mail\Engine\IDN::toAscii($sName);
		...
		if ($sName) {
			...
			\opacit_mail\Engine\Utils::saveFile($this->sDomainPath.'/disabled', \implode("\n", $aResult));
	...
	public function Delete(string $sName) : bool
	{
		$bResult = true;
		if (\strlen($sName)) {
			$sRealFileName = $this->sDomainPath . '/' . self::encodeFileName($sName);
			$bResult =
...
//		$aList = \glob($this->sDomainPath.'/*.{ini,json,alias}', GLOB_BRACE);
		...
		foreach ($aList as $sFile) {
			...
			if ($bAlias || '.json' === \substr($sFile, -5) || '.ini' === \substr($sFile, -4)) {
				$sName = self::decodeFileName(\preg_replace('/\.(ini|json|alias)$/', '', $sFile));
				if ($bAlias) {
					if ($bIncludeAliases) {
