...
trait Raw
{
			...
			return $this->MailClient()->MessageMimeStream(
				function ($rResource) use ($sRawKey) {
					if (\is_resource($rResource)) {
						...
						\opacit_mail\Mail\Base\Utils::FpassthruWithTimeLimitReset($rResource);
					}
				},
	...
	public function RawUserBackground() : bool
	{
		...
		$mData = $this->StorageProvider()->Get($oAccount,
			\opacit_mail\Engine\Providers\Storage\Enumerations\StorageType::CONFIG->value,
			'background'
		);
	...
	private function clearFileName(string $sFileName, string $sContentType, string $sMimeIndex, int $iMaxLength = 250): string
	{
		...
		$sClearedFileName = \opacit_mail\Mail\Base\Utils::StripSpaces(\preg_replace('/[\.]+/', '.', $sFileName));
		$sExt = \opacit_mail\Mail\Base\Utils::GetFileExtension($sClearedFileName);
		...
		return \opacit_mail\Mail\Base\Utils::SecureFileName(\opacit_mail\Mail\Base\Utils::Utf8Clear($sClearedFileName));
	}
	...
	private function rawSmart(bool $bDownload, bool $bThumbnail = false) : bool
	{
		...
		$sRange = \opacit_mail\Mail\Base\Http::GetHeader('Range');
		$aMatch = array();
		...
		if (!empty($aValues['fileHash'])) {
			...
			if ('.pdf' === \substr($sFileNameIn,-4)) {
				...
				$sContentTypeOut = $sContentTypeIn
					?: \opacit_mail\Engine\File\MimeType::fromFilename($sFileNameIn)
					?: 'application/octet-stream';
			}
			$sFileNameOut = $this->clearFileName($sFileNameIn, $sContentTypeIn, $sMimeIndex);
			\header('Content-Type: '.$sContentTypeOut);
			\opacit_mail\Mail\Base\Http::setContentDisposition('attachment', ['filename' => $sFileNameOut]);
					...
					if (!$bDownload) {
						...
						try
						{
							if ($bThumbnail) {
								...
								\opacit_mail\Mail\Base\Http::setContentDisposition('inline', ['filename' => $sFileName.'_thumb60x60.png']);
								$oImage->show('png');
//								$oImage->show('webp'); // Little Britain: "Safari says NO"
								...
								\opacit_mail\Mail\Base\Http::setContentDisposition('inline', ['filename' => $sFileName]);
								$oImage->show();
//								$oImage->show('webp'); // Little Britain: "Safari says NO"
								exit;
							}
						...
						catch (\Throwable $oException)
						{
							...
							\opacit_mail\Mail\Base\Http::StatusHeader(500);
							exit;
						}
					}
					...
					if (!\headers_sent()) {
						...
						\opacit_mail\Mail\Base\Http::setContentDisposition($bDownload ? 'attachment' : 'inline', ['filename' => $sFileName]);
					...
					if ($sLoadedData) {
						if ($bIsRangeRequest && (\strlen($sRangeStart) || \strlen($sRangeEnd))) {
							...
							\opacit_mail\Mail\Base\Http::StatusHeader(206);
	...
	private static function loadImage($resource, bool $bDetectImageOrientation = false, int $iThumbnailBoxSize = 0) : ?\opacit_mail\Engine\Image
	{
		...
		$handler = 'opacit_mail\\Engine\\Image\\'.$handler.'::createFromStream';
