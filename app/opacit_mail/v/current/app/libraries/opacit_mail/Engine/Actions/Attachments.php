...
trait Attachments
{
	...
	public function DoAttachmentsActions() : array
	{
		...
		$sFilename = \opacit_mail\Mail\Base\Utils::SecureFileName($this->GetActionParam('filename', ''));
		...
		switch (\strtolower($sAction))
		{
			case 'zip':
				...
				$sZipHash = \opacit_mail\Mail\Base\Utils::Sha1Rand();
				...
				if (!empty($sZipFileName)) {
					if (\class_exists('ZipArchive')) {
						...
						$oZip->setArchiveComment('opacit_mail/'.APP_VERSION);
...
/*
						...
						@\unlink($sZipFileName);
						$oZip = new \opacit_mail\Engine\Stream\ZIP($sZipFileName);
//						$oZip->setArchiveComment('opacit_mail/'.APP_VERSION);
						foreach ($aData as $aItem) {
							if ($aItem['fileHash']) {
								$sFullFileNameHash = $oFilesProvider->GetFileName($oAccount, $aItem['fileHash']);
...
*/
						...
						foreach ($aData as $aItem) {
							...
							if ($aItem['data']) {
								try {
									$oZip->addFromString($sFileName, \opacit_mail\Mail\Base\Utils::UrlSafeBase64Decode($aItem['data']));
								} catch (\Throwable $e) {
									$bError = true;
								}
							} else {
			...
			default:
				$data = new \opacit_mail\Engine\AttachmentsAction;
				$data->action = $sAction;
				$data->items = $aData;
	...
	private function getMimeFileByHash(\opacit_mail\Engine\Model\Account $oAccount, string $sHash) : array
	{
		...
		if (isset($aValues['data'])) {
			$aValues['data'] = \opacit_mail\Mail\Base\Utils::UrlSafeBase64Decode($aValues['data']);
			...
			$this->MailClient()->MessageMimeStream(
				...
				use ($oAccount, $oFileProvider, &$aValues) {
					...
					if (\is_resource($rResource)) {
						...
						$sHash = \opacit_mail\Mail\Base\Utils::Sha1Rand($aValues['fileName'].'~'.$sContentTypeIn);
						$rTempResource = $oFileProvider->GetFile($oAccount, $sHash, 'wb+');
						if (\is_resource($rTempResource)) {
							if (-1 < \opacit_mail\Mail\Base\Utils::WriteStream($rResource, $rTempResource)) {
								$sResultHash = $sHash;
