...
class Utils
{
	...
	{
		try {
			/* Issue with \opacit_mail\Engine\HTTP\Stream
	...
	{
		return \opacit_mail\Engine\Crypt::EncryptUrlSafe(
			$aValues,
	...
	{
		return \opacit_mail\Engine\Crypt::DecryptUrlSafe(
			$sEncodedValues,
	...
	{
		$sToken = \opacit_mail\Engine\Cookies::get(self::SESSION_TOKEN);
		if (!$sToken) {
			if (!$generate) {
				return null;
			}
			\opacit_mail\Engine\Log::debug('TOKENS', 'New SESSION_TOKEN');
			$sToken = \opacit_mail\Mail\Base\Utils::Sha1Rand(APP_SALT);
			\opacit_mail\Engine\Cookies::set(self::SESSION_TOKEN, $sToken);
		}
...
//		$oAccount = $oActions->getMainAccountFromToken(false);
		...
		if (!$sSeed) {
			...
			$sSeed = $sFallback ??= \opacit_mail\Mail\Base\Utils::Sha1Rand(APP_SALT);
		}
...
//		return $sHtml;
	...
	{
		// opacit_mail: use static 'current' path — version is in info.xml, not the directory name
		return self::WebPath() . 'opacit_mail/v/current/';
	}
...
//			\opacit_mail\Engine\Log::warning('OpenBasedir', "open_basedir restriction in effect. {$name} is not within the allowed path(s): " . \ini_get('open_basedir'));
			return false;
		}
		return true;
	}
