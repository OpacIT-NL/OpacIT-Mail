	...
	public static function FileStorage(string $sDataPath)
	{
		...
		foreach (\glob("{$sDataPath}/cfg/*", GLOB_ONLYDIR) as $sOldDir) {
			foreach (\glob("{$sOldDir}/*", GLOB_ONLYDIR) as $sDomainDir) {
				...
				$sNewDir = $sDataPath
					.'/'.\opacit_mail\Mail\Base\Utils::SecureFileName($sDomain ?: 'unknown.tld')
					.'/'.\opacit_mail\Mail\Base\Utils::SecureFileName(\implode('@', $aEmail) ?: '.unknown');
//				\opacit_mail\Mail\Base\Utils::mkdir($sNewDir)
				if (\is_dir($sNewDir) || \mkdir($sNewDir, 0700, true)) {
					...
					\opacit_mail\Mail\Base\Utils::RecRmDir($sDomainDir);
				}
			}
		}
		\opacit_mail\Mail\Base\Utils::RecRmDir("{$sDataPath}/cfg");
		\opacit_mail\Mail\Base\Utils::RecRmDir("{$sDataPath}/data");
		\opacit_mail\Mail\Base\Utils::RecRmDir("{$sDataPath}/files");
	}

	/**
	 * Attempt to convert the old less secure data into better secured data
	 */
	public static function ConvertInsecureAccounts(\opacit_mail\Engine\Actions $oActions, \opacit_mail\Engine\Model\MainAccount $oMainAccount) : array
	{
		...
		if (1 < \count($aAccounts)) {
			...
			foreach ($aAccounts as $sEmail => $sToken) {
				...
				try {
					...
					if (!$sToken) {
						\opacit_mail\Engine\Log::warning('UPGRADE', "ConvertInsecureAccount {$sEmail} no token");
						continue;
					}
					...
					if (empty($aAccountHash[0]) || 'token' !== $aAccountHash[0] // simple token validation
						...
						\opacit_mail\Engine\Log::warning('UPGRADE', "ConvertInsecureAccount {$sEmail} invalid aAccountHash: " . \print_r($aAccountHash,1));
						continue;
					}
					...
					\opacit_mail\Engine\Log::warning('UPGRADE', "ConvertInsecureAccount {$sEmail} failed");
				}
			}
	...
	{
		$sData = \opacit_mail\Mail\Base\Utils::UrlSafeBase64Decode($sData);
		if (!\strlen($sData)) {
			return [];
		}
		$sKey = \md5(APP_SALT);
		$sData = \is_callable('xxtea_decrypt')
			...
			: \opacit_mail\Mail\Base\Xxtea::decrypt($sData, $sKey);
		try {
...
//		$tar_destination = APP_DATA_FOLDER_PATH . APP_VERSION . '.tar';
		...
		if (\class_exists('PharData')) {
			...
			$tar_destination .= '.tgz';
			$tar = new \opacit_mail\Engine\Stream\TAR($tar_destination);
		}
		...
		if ($tar instanceof \opacit_mail\Engine\Stream\TAR) {
	...
	// opacit_mail: in-app core update removed — managed via NC app store / CI
	...
	public static function fixPermissions($mode = 0755) : void
	{
		...
		foreach (\glob("{$target}/opacit_mail/v/*", \GLOB_ONLYDIR) as $dir) {
			\chmod($dir, 0755);
