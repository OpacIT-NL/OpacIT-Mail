...
abstract class Crypt
{
	...
	) /* : mixed */
	{
		...
		return static::Decrypt(\array_map('opacit_mail\Mail\\Base\\Utils::UrlSafeBase64Decode', $data), $key);
	}
...
*/
	...
	{
		return \implode('.', \array_map('opacit_mail\Mail\\Base\\Utils::UrlSafeBase64Encode', static::Encrypt($data, $key)));
	}
	...
	) /* : string|false */
	{
		...
		return \sodium_crypto_aead_xchacha20poly1305_ietf_decrypt(
			...
			APP_SALT,
			$nonce,
			\str_pad('', \SODIUM_CRYPTO_AEAD_XCHACHA20POLY1305_IETF_KEYBYTES, self::Passphrase($key))
		);
	}
	...
	) /* : mixed */
	{
		...
		$key = $salt . self::Passphrase($key);
		return \is_callable('xxtea_decrypt')
			? \xxtea_decrypt($data, $key)
			: \opacit_mail\Mail\Base\Xxtea::decrypt($data, $key);
	}
	...
	) : string
	{
		...
		$result = \is_callable('xxtea_encrypt')
			...
			: \opacit_mail\Mail\Base\Xxtea::encrypt($data, $key);
		if (!$result) {
...
\opacit_mail\Engine\Crypt::setCipher(\opacit_mail\Engine\Api::Config()->Get('security', 'encrypt_cipher', 'aes-256-cbc-hmac-sha1'));
