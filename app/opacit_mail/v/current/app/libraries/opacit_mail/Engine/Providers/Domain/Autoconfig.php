...
abstract class Autoconfig
{
	...
	{
//		$emailaddress = \opacit_mail\Engine\IDN::emailToAscii($emailaddress);
		$domain = \opacit_mail\Mail\Base\Utils::getEmailAddressDomain($emailaddress);
//		$domain = \opacit_mail\Engine\IDN::toAscii($domain);
		$domain = \strtolower(\idn_to_ascii($domain));
		return
			// First try autoconfig
			self::resolve($domain, $emailaddress)
			// Else try MX, but it is mostly useless
//			?: self::mx($domain, $emailaddress)
			// Else try Microsoft autodiscover
			?: self::autodiscover($domain)
			// Else try DNS SRV
			?: self::srv($domain);
	}
	...
	{
		$oCache = \opacit_mail\Engine\Api::Actions()->Cacher();
	...
	{
		$hostnames = [];
		$suffixes = self::publicsuffixes();
		foreach (\opacit_mail\Engine\DNS::MX($domain) as $hostname) {
		...
		foreach ($hostnames as $mxfulldomain => $mxbasedomain) {
			if ($domain != $mxfulldomain) {
				$autoconfig = self::resolve($mxfulldomain, $emailaddress);
				if (!$autoconfig && $mxfulldomain != $mxbasedomain && $domain != $mxbasedomain) {
					$autoconfig = self::resolve($mxbasedomain, $emailaddress);
				}
				if ($autoconfig) {
					return $autoconfig;
				}
	...
	{
		$srv = \opacit_mail\Engine\DNS::SRV('_submissions._tcp.'.$domain);
		if (empty($srv[0]['target'])) {
			$srv = \opacit_mail\Engine\DNS::SRV('_submission._tcp.'.$domain);
		}
		if (!empty($srv[0]['target'])) {
			$result = [
				'incomingServer' => [],
			...
			$srv = \opacit_mail\Engine\DNS::SRV('_imaps._tcp.'.$domain);
			if (empty($srv[0]['target'])) {
				$srv = \opacit_mail\Engine\DNS::SRV('_imap._tcp.'.$domain);
			}
			if (!empty($srv[0]['target'])) {
	...
	{
		foreach (\opacit_mail\Engine\DNS::SRV("_autodiscover._tcp.{$domain}") as $record) {
			if (443 == $record['port']) {
				$result = self::autodiscover_resolve("https://{$record['target']}", $domain);
			} else if (80 == $record['port']) {
				$result = self::autodiscover_resolve("http://{$record['target']}", $domain);
			} else {
				$result = self::autodiscover_resolve("https://{$record['target']}:{$record['port']}", $domain);
			}
			if ($result) {
				return $result;
			}
		...
		foreach ([
			...
			$result = self::autodiscover_resolve($host, $domain);
			if ($result) {
				return $result;
			}
		}
