...
use opacit_mail\Engine\Cookies;
...
trait UserAuth
{
	...
	{
		$sEmail = \opacit_mail\Engine\IDN::emailToAscii(\opacit_mail\Mail\Base\Utils::Trim($sEmail));
		...
		$sSmtpUser = $sEmail;
		if (\str_contains($sEmail, '@')
		 && ($oDomain || ($oDomain = $oDomainProvider->Load(\opacit_mail\Mail\Base\Utils::getEmailAddressDomain($sEmail), true)))
		) {
...
//				,new SensitiveString($oPassword)
	...
	{
		$helper = \OCP\Server::get(\OCA\opacit_mail\Util\EngineHelper::class);
	...
	public function switchAccount(string $sEmail) : bool
	{
		...
		if ($sEmail && $oMainAccount && $this->GetCapa(Capa::ADDITIONAL_ACCOUNTS->value)) {
			...
			if ($oMainAccount->Email() !== $sEmail) {
				$sEmail = \opacit_mail\Engine\IDN::emailToAscii($sEmail);
	...
	/**
	 * Returns opacit_mail\Engine\Model\AdditionalAccount when it exists,
	 * else returns opacit_mail\Engine\Model\MainAccount when it exists,
	 * else null
	 *
	 * @throws \opacit_mail\Engine\Exceptions\ClientException
	 */
...
//		Cookies::clear(Utils::SESSION_TOKEN);
	 ...
	 * @throws \opacit_mail\Engine\Exceptions\ClientException
	 */
	protected function imapConnect(Account $oAccount, bool $bAuthLog = false, ?\opacit_mail\Mail\Imap\ImapClient $oImapClient = null): void
	{
		try {
			if (!$oImapClient) {
		...
		} catch (\opacit_mail\Mail\Imap\Exceptions\LoginBadCredentialsException $oException) {
			if ($bAuthLog) {
