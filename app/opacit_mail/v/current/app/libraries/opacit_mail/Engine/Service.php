...
//		\header('Permissions-Policy: interest-cohort=()');
		...
		if ($oConfig->Get('security', 'force_https', false) && !$oHttp->IsSecure()) {
			\opacit_mail\Mail\Base\Http::Location('https://'.$oHttp->GetHost(false).$oHttp->GetUrl());
			return true;
		}
		...
		if (\count($aPaths) && !empty($aPaths[0]) && 'index' !== \strtolower($aPaths[0])) {
			if ('mailto' !== \strtolower($aPaths[0]) && !\opacit_mail\Engine\HTTP\SecFetch::matchAnyRule($oConfig->Get('security', 'secfetch_allow', ''))) {
				\opacit_mail\Mail\Base\Http::StatusHeader(403);
				echo $oServiceActions->ErrorTemplates('Access Denied.',
					"Disallowed Sec-Fetch
					Dest: " . ($_SERVER['HTTP_SEC_FETCH_DEST'] ?? '') . "
					Mode: " . ($_SERVER['HTTP_SEC_FETCH_MODE'] ?? '') . "
					Site: " . ($_SERVER['HTTP_SEC_FETCH_SITE'] ?? '') . "
					User: " . (\opacit_mail\Engine\HTTP\SecFetch::user() ? 'true' : 'false'));
				return false;
			}
...
//			if (!\opacit_mail\Engine\HTTP\SecFetch::isEntering()) {
			\header('Content-Type: text/html; charset=utf-8');

			if (!\is_dir(APP_DATA_FOLDER_PATH) || !\is_writable(APP_DATA_FOLDER_PATH)) {
				echo $oServiceActions->ErrorTemplates(
					'Permission denied!',
					'opacit_mail can not access the data folder "'.APP_DATA_FOLDER_PATH.'"'
				);
				return false;
			}
			...
			$aTemplateParameters = array(
				...
				'{{LoadingDescriptionEsc}}' => \htmlspecialchars($oConfig->Get('webmail', 'loading_description', 'opacit_mail'), ENT_QUOTES|ENT_IGNORE, 'UTF-8'),
				'{{BaseAppAdmin}}' => 0
			);
...
//			$oActions->verifyCacheByKey($sCacheFileName);
			...
			$sScriptNonce = \opacit_mail\Engine\UUID::generate();
			self::setCSP($sScriptNonce);
