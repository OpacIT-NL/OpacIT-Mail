...
namespace opacit_mail\Engine\Imap;

use opacit_mail\Mail\Imap\Enumerations\FetchType;
use opacit_mail\Mail\Imap\Enumerations\MessageFlag;
use opacit_mail\Mail\Mime\Enumerations\Header;

class Sync
{
	public \opacit_mail\Mail\Imap\ImapClient $oImapSource;
	public \opacit_mail\Mail\Imap\ImapClient $oImapTarget;
...
//		$this->oImapTarget->logWrite('Get oImapTarget->FolderList');
		\opacit_mail\Engine\Log::notice('SYNC', 'Get oImapTarget->FolderList');
		...
		\opacit_mail\Engine\Log::notice('SYNC', 'Get oImapSource->FolderList');
		...
		if ($isCli) {
			...
			\opacit_mail\Engine\HTTP\Stream::start();
			\opacit_mail\Engine\HTTP\Stream::JSON([
...
/*
							...
							if ($iAppendUid && $aFlags) {
								$this->MessageStoreFlag(
									...
									$aFlags,
									\opacit_mail\Mail\Imap\Enumerations\StoreAction::ADD_FLAGS_SILENT
								);
							}
*/
						}
