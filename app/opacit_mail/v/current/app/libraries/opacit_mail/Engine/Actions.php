...
class Actions
{
	...
	/**
	 * This session cookie contains a \opacit_mail\Engine\Model\Account
	 * Value is Base64 EncryptToJSON
	 */
	const AUTH_SPEC_TOKEN_KEY = 'x2maccount';

	/**
	 * This session cookie optionally contains a \opacit_mail\Engine\Model\AdditionalAccount
	 * Value is Base64 EncryptToJSON
	 */
	...
	/**
	 * @var \opacit_mail\Mail\Base\Http
	 */
	...
	/**
	 * @var \opacit_mail\Mail\Client\MailClient
	 */
	...
	/**
	 * @var \opacit_mail\Mail\Log\Logger
	 */
	protected $oLoggerAuth;

	/**
	 * @var array of \opacit_mail\Mail\Cache\CacheClient
	 */
	protected $aCachers = array();

	/**
	 * @var \opacit_mail\Engine\Providers\Storage
	 */
	...
	/**
	 * @var \opacit_mail\Engine\Providers\Files
	 */
	protected $oFilesProvider = null;

	/**
	 * @var \opacit_mail\Engine\Providers\Domain
	 */
	protected $oDomainProvider = null;

	/**
	 * @var \opacit_mail\Engine\Providers\Settings
	 */
	...
	function __construct()
	{
		...
		if ($this->oConfig->Get('logs', 'enable', false) || $this->oConfig->Get('debug', 'enable', false)) {
			...
			if ('syslog' === $sLogFileName) {
				$oDriver = new \opacit_mail\Mail\Log\Drivers\Syslog();
			} else if ('stderr' === $sLogFileName) {
				$oDriver = new \opacit_mail\Mail\Log\Drivers\StderrStream();
			} else {
				...
				$oDriver = new \opacit_mail\Mail\Log\Drivers\File($sLogFileFullPath . '/' . $this->compileLogFileName($sLogFileName));
			}
			...
			$this->logWrite(
				...
				. '][PID:' . (\opacit_mail\Mail\Base\Utils::FunctionCallable('getmypid') ? \getmypid() : 'unknown')
				. '][' . \opacit_mail\Mail\Base\Http::GetServer('SERVER_SOFTWARE', '~')
				. '][' . \PHP_SAPI
				. '][Streams:' . \implode(',', \stream_get_transports())
				. '][' . $oHttp->GetMethod() . ' ' . $oHttp->GetScheme() . '://' . $oHttp->GetHost(false) . \opacit_mail\Mail\Base\Http::GetServer('REQUEST_URI', '') . ']'
			);
		}
		...
		if (null === $mResult) {
			switch ($sName) {
				...
				case 'address-book':
					...
					// No standalone fallback — opacit_mail requires Nextcloud
					break;
				case 'identities':
				case 'suggestions':
					$mResult = [];
...
/*				// See function Cacher
				case 'cache':
					$mResult = new \opacit_mail\Mail\Cache\Drivers\File(
...
/*
		try {
			if (!\opacit_mail\Engine\Shutdown::count() && $this->ImapClient()->IsLoggined()) {
...
*/
		...
		if (false !== \strpos($sLine, '{request:')) {
			...
			if (false !== \strpos($sLine, '{request:domain-clear}')) {
				$sLine = \str_replace('{request:domain-clear}',
					\opacit_mail\Mail\Base\Utils::GetClearDomainName($this->Http()->GetHost(true, true)),
					$sLine);
			}
		...
		if (false !== \strpos($sLine, '{user:')) {
			...
			if (\preg_match('/\{user:(email|login|domain)\}/i', $sLine)) {
				...
				if ($oAccount) {
					...
					$sLine = \str_replace('{user:login}', \opacit_mail\Mail\Base\Utils::getEmailAddressLocalPart($sEmail), $sLine);
					$sLine = \str_replace('{user:domain}', \opacit_mail\Mail\Base\Utils::getEmailAddressDomain($sEmail), $sLine);
					$sLine = \str_replace('{user:domain-clear}',
						\opacit_mail\Mail\Base\Utils::GetClearDomainName(\opacit_mail\Mail\Base\Utils::getEmailAddressDomain($sEmail)),
						$sLine);
				}
			}
	...
	public function Http(): \opacit_mail\Mail\Base\Http
	{
		if (null === $this->oHttp) {
			$this->oHttp = \opacit_mail\Mail\Base\Http::SingletonInstance();
		}

		return $this->oHttp;
	}

	public function MailClient(): \opacit_mail\Mail\Client\MailClient
	{
		if (null === $this->oMailClient) {
			$this->oMailClient = new \opacit_mail\Mail\Client\MailClient();
	...
	public function ImapClient(): \opacit_mail\Mail\Imap\ImapClient
	{
//		$this->initMailClientConnection();
	 ...
	 */
	public function Cacher(?Model\Account $oAccount = null, bool $bForceFile = false): \opacit_mail\Mail\Cache\CacheClient
	{
		$sKey = '';
		if ($oAccount) {
		...
		if (!isset($this->aCachers[$sIndexKey])) {
			$this->aCachers[$sIndexKey] = new \opacit_mail\Mail\Cache\CacheClient();

			$oDriver = $bForceFile ? null : $this->fabrica('cache');
			if (!($oDriver instanceof \opacit_mail\Mail\Cache\DriverInterface)) {
				$oDriver = new \opacit_mail\Mail\Cache\Drivers\File(
					\trim($this->oConfig->Get('cache', 'path', '')) ?: APP_PRIVATE_DATA . 'cache'
				);
			}
	...
	{
		if ($sLogin) {
			$sHost = $admin ? $this->Http()->GetHost(true, true) : \opacit_mail\Mail\Base\Utils::getEmailAddressDomain($sLogin);
			$aAdditionalParams = array(
				...
				'{smtp:host}' => $sHost,
				'{user:email}' => $sLogin,
				'{user:login}' => $admin ? $sLogin : \opacit_mail\Mail\Base\Utils::getEmailAddressLocalPart($sLogin),
				'{user:domain}' => $sHost,
			);
		} else {
			$aAdditionalParams = array();
		...
		if (!empty($sLine)) {
			if (!$this->oLoggerAuth) {
				$this->oLoggerAuth = new \opacit_mail\Mail\Log\Logger(false);
...
//					$this->oLoggerAuth->SetLevel(\LOG_WARNING);
					...
					$this->oLoggerAuth->append(
						(new \opacit_mail\Mail\Log\Drivers\File($sAuthLogFileFullPath))
							->DisableTimePrefix()
							->DisableGuidPrefix()
							->DisableTypedPrefix()
					);
		...
		if (($this->oConfig->Get('logs', 'auth_logging', false) || $this->oConfig->Get('logs', 'auth_syslog', false))
		 && \openlog('opacit_mail', 0, \LOG_AUTHPRIV)) {
	...
	public function AppData(bool $bAdmin): array
	{
		...
		$aResult = array(
			'Auth' => false,
			'title' => $oConfig->Get('webmail', 'title', 'opacit_mail'),
			'loadingDescription' => $oConfig->Get('webmail', 'loading_description', 'opacit_mail'),
			'Plugins' => array(),
			'System' => array(
				'version' => APP_VERSION,
				'token' => Utils::GetCsrfToken(),
				'languages' => \opacit_mail\Engine\L10n::getLanguages(false),
				'webPath' => \opacit_mail\Engine\Utils::WebPath(),
				'webVersionPath' => \opacit_mail\Engine\Utils::WebVersionPath()
			),
		);
		...
		if ($oAccount) {
				...
				$sToken = \opacit_mail\Engine\Cookies::get(self::AUTH_MAILTO_TOKEN_KEY);
				if (null !== $sToken) {
					\opacit_mail\Engine\Cookies::clear(self::AUTH_MAILTO_TOKEN_KEY);

					$mMailToData = Utils::DecodeKeyValuesQ($sToken);
					if (!empty($mMailToData['MailTo']) && 'MailTo' === $mMailToData['MailTo'] && !empty($mMailToData['To'])) {
						$aResult['mailToEmail'] = \opacit_mail\Engine\IDN::emailToUtf8($mMailToData['To']);
					}
				}
...
//				}
	...
	public function Upload(?array $aFile, int $iError): array
	{
		...
		if ($oAccount && UPLOAD_ERR_OK === $iError && \is_array($aFile)) {
			...
			// Detect content-type
			$type = \opacit_mail\Engine\File\MimeType::fromFile($aFile['tmp_name'], $aFile['name'])
				?: \opacit_mail\Engine\File\MimeType::fromFilename($aFile['name']);
			if ($type) {
				$aFile['type'] = $type;
				$sSavedName .= \opacit_mail\Engine\File\MimeType::toExtension($type);
			}
	...
	public function Capa(bool $bAdmin, ?Model\Account $oAccount = null): array
	{
		...
		if (!$aResult) {
			...
			$aResult = array(
				...
				Capa::GNUPG->value               => (bool) $oConfig->Get('security', 'gnupg', true) && \opacit_mail\Engine\PGP\GnuPG::isSupported(),
...
//		if ($sKey && $this->oConfig->Get('cache', 'enable', true) && $this->oConfig->Get('cache', 'http', true)) {
	...
	{
		if ($sKey && $this->oConfig->Get('cache', 'enable', true) && $this->oConfig->Get('cache', 'http', true)) {
			\opacit_mail\Mail\Base\Http::ServerUseCache(
				$this->etag($sKey),
	...
	{
		if ($sKey && $this->oConfig->Get('cache', 'enable', true) && $this->oConfig->Get('cache', 'http', true)) {
			\opacit_mail\Mail\Base\Http::checkETag($this->etag($sKey));
//			\opacit_mail\Mail\Base\Http::checkLastModified(0);
		}
	}
	...
	public function encodeRawKey(array $aValues): string
	{
		...
		return \opacit_mail\Mail\Base\Utils::UrlSafeBase64Encode(\json_encode($aValues));
	}

	public function decodeRawKey(string $sRawKey): array
	{
		return empty($sRawKey) ? []
			: (\json_decode(\opacit_mail\Mail\Base\Utils::UrlSafeBase64Decode($sRawKey), true) ?: []);
/*
...
*/
	...
	public function Location(string $sUrl, int $iStatus = 302): void
	{
		...
		\opacit_mail\Mail\Base\Http::Location($sUrl, $iStatus);
	}
