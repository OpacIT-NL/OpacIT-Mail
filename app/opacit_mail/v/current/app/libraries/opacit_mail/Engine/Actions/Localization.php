...
trait Localization
{
	...
	{
		$sResult = \opacit_mail\Engine\L10n::validLanguage($sLanguage, $bAdmin)
		?: \opacit_mail\Engine\L10n::validLanguage($sDefault, $bAdmin);
		...
		return \opacit_mail\Engine\L10n::validLanguage($sResult, $bAdmin) ? $sResult : 'en';
	}
	...
	public function detectClientLanguage(bool $bAdmin): string
	{
		...
		$sAcceptLang = \strtolower(\opacit_mail\Mail\Base\Http::GetServer('HTTP_ACCEPT_LANGUAGE', 'en'));
	...
	public function StaticI18N(string $sKey): string
	{
		...
		if (null === $aLang) {
			...
			$aLang = \opacit_mail\Engine\L10n::load($sLang, 'static');
	...
	public function compileLanguage(string $sLanguage, bool $bAdmin = false) : string
	{
		...
		$langs = \array_flip(\opacit_mail\Engine\L10n::getLanguages($bAdmin));
		...
		$aResultLang = \array_replace_recursive(
			$aResultLang,
			\opacit_mail\Engine\L10n::load($sLanguage, ($bAdmin ? 'admin' : 'user'))
		);
