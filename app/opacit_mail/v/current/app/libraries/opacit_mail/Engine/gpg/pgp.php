...
namespace opacit_mail\Engine\GPG;

use opacit_mail\Engine\SensitiveString;

class PGP extends Base implements \opacit_mail\Engine\PGP\PGPInterface
{
	private
		$_message,
...
//		\rewind($fp);
	...
	{
		// TODO: implement decrypt+verify for files
		return false;
	}
	...
	{
//		\opacit_mail\Engine\Log::debug('GnuPG', "export({$fingerprint}, {$passphrase})");
...
//			'NO_SECKEY' => [],
	...
	private function exec(array $arguments, bool $throw = true) /*: array|false*/
	{
		if (\version_compare($this->version, '2.2.5', '<')) {
			\opacit_mail\Engine\Log::error('GPG', "{$this->version} too old");
			return false;
		}
