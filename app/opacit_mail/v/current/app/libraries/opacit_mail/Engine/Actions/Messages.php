...
use opacit_mail\Mail\Imap\SequenceSet;
use opacit_mail\Mail\Imap\Enumerations\FetchType;
use opacit_mail\Mail\Imap\Enumerations\MessageFlag;
use opacit_mail\Mail\Mime\Part as MimePart;
use opacit_mail\Mail\Mime\Enumerations\Header as MimeEnumHeader;
...
trait Messages
{
	...
	{
//		\sleep(1);
//		throw new ClientException(Notifications::CantGetMessageList->value);
		...
		if ($oSettingsLocal instanceof \opacit_mail\Engine\Settings) {
...
//			\ignore_user_abort(true);
		...
		{
			throw new ClientException(Notifications::CantGetMessageList->value, $oException);
		}
	}
	...
	public function DoSaveMessage() : array
	{
		...
		$mResult = false;
		if ($oMessage) {
			$rMessageStream = \opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResource();

			$iMessageStreamSize = \opacit_mail\Mail\Base\Utils::WriteStream($oMessage->ToStream(false), $rMessageStream, 8192, true);
			...
			if (false !== $iMessageStreamSize) {
				...
				$iNewUid = $this->ImapClient()->MessageAppendStream(
					$sDraftFolder, $rMessageStream, $iMessageStreamSize, array(MessageFlag::SEEN->value)
				);
...
/*
		...
		if ($aAuth) {
			...
			$oAccount->setSmtpPass(new \opacit_mail\Engine\SensitiveString($aAuth['password']));
...
*/
		...
		try
		{
			if ($oMessage) {
				...
				$iMessageStreamSize = \opacit_mail\Mail\Base\Utils::WriteStream(
				...
				if (false !== $iMessageStreamSize) {
					...
					if (\is_array($aDraftInfo) && 3 === \count($aDraftInfo)) {
						...
						try
						{
							switch (\strtolower($sDraftInfoType))
							{
								case 'reply':
								case 'reply-all':
									$this->MailClient()->MessageSetFlag($sDraftInfoFolder, new SequenceSet($iDraftInfoUid), MessageFlag::ANSWERED->value);
									break;
								case 'forward':
									$this->MailClient()->MessageSetFlag($sDraftInfoFolder, new SequenceSet($iDraftInfoUid), MessageFlag::FORWARDED->value);
									break;
							}
						}
						catch (\Throwable $oException)
					...
					if (\strlen($sSaveFolder)) {
						...
						{
							if ($oMessage->GetBcc()) {
								$rAppendMessageStream = \opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResource();
								$iAppendMessageStreamSize = \opacit_mail\Mail\Base\Utils::WriteStream(
		...
		catch (\opacit_mail\Mail\Smtp\Exceptions\LoginBadCredentialsException $oException)
		{
		...
		{
			throw new ClientException(Notifications::CantSendMessage->value, $oException);
		}
		...
		try
		{
			if ($oMessage && $this->AddressBookProvider($oAccount)->IsActive()) {
				...
				if ($oToCollection) {
					foreach ($oToCollection as /* @var $oEmail \opacit_mail\Mail\Mime\Email */ $oEmail) {
			...
			if ($oMessage) {
				...
				if (false !== $iMessageStreamSize) {
					...
					if (\strlen($sFolderFullName) && 0 < $iUid) {
						try
						{
							$this->MailClient()->MessageSetFlag($sFolderFullName, new SequenceSet($iUid), MessageFlag::MDNSENT->value, true, true);
						}
						catch (\Throwable $oException)
						{
							$this->Cacher($oAccount)->Set(\opacit_mail\Engine\KeyPathHelper::ReadReceiptCache($oAccount->Email(), $sFolderFullName, $iUid), '1');
						}
					}
				}
			}
	...
	{
		return $this->messageSetFlag(MessageFlag::SEEN->value);
	}
	...
	{
		return $this->messageSetFlag(MessageFlag::FLAGGED->value, true);
	}

	public function DoMessageSetDeleted() : array
	{
		return $this->messageSetFlag(MessageFlag::DELETED->value, true);
	}
...
//			$accountHash = $aValues[3];
		...
		try
		{
			...
			if ($oMessage->smimeSigned && ($bAutoVerify || !$oMessage->smimeSigned['detached'])) try {
				...
				$result = (new \opacit_mail\Engine\SMime\OpenSSL(''))->verify($sBody, null, $bOpaque);
				if ($result) {
					if ($bOpaque) {
				...
				if ($GPG) {
					if ($oMessage->pgpSigned['sigPartId']) {
						...
						$aParts = [
							...
							FetchType::BODY_PEEK->value.'['.$sSigPartId.']'
						];
						$oFetchResponse = $this->ImapClient()->Fetch($aParts, $oMessage->Uid, true)[0];
						$sBodyMime = $oFetchResponse->GetFetchValue(FetchType::BODY->value.'['.$sPartId.'.MIME]');
						$info = $this->GnuPG()->verify(
							\preg_replace('/\\r?\\n/su', "\r\n",
								$sBodyMime . $oFetchResponse->GetFetchValue(FetchType::BODY->value.'['.$sPartId.']')
							),
							\preg_replace('/[^\x00-\x7F]/', '',
								$oFetchResponse->GetFetchValue(FetchType::BODY->value.'['.$sSigPartId.']')
							)
						);
					} else {
						// clearsigned text
		...
		{
			throw new ClientException(Notifications::CantGetMessage->value, $oException);
		}
	...
	public function DoMessageDelete() : array
	{
		...
		{
			throw new ClientException(Notifications::CantDeleteMessage->value, $oException);
		}
	...
	public function DoMessageMove() : array
	{
		...
		if (!empty($this->GetActionParam('markAsRead', '0'))) {
			try
			{
				$this->MailClient()->MessageSetFlag($sFromFolder, $oUids, MessageFlag::SEEN->value);
			}
			catch (\Throwable $oException)
			{
				unset($oException);
...
//					$this->MailClient()->MessageSetFlag($sFromFolder, $oUids, '\\junk');
					$this->MailClient()->MessageSetFlag($sFromFolder, $oUids, MessageFlag::JUNK->value);
					$this->MailClient()->MessageSetFlag($sFromFolder, $oUids, MessageFlag::NOTJUNK->value, false);
				} else if ('HAM' === $sLearning) {
					$this->MailClient()->MessageSetFlag($sFromFolder, $oUids, MessageFlag::NOTJUNK->value);
					$this->MailClient()->MessageSetFlag($sFromFolder, $oUids, MessageFlag::JUNK->value, false);
//					$this->MailClient()->MessageSetFlag($sFromFolder, $oUids, '\\junk', false);
		...
		{
			throw new ClientException(Notifications::CantMoveMessage->value, $oException);
		}
	...
	public function DoMessageCopy() : array
	{
		...
		{
			throw new ClientException(Notifications::CantCopyMessage->value, $oException);
		}
							...
							$this->MailClient()->MessageMimeStream(
								function($rResource, $sContentType, $sFileName, $sMimeIndex = '') use ($oAccount, $sTempName, $self, &$aAttachments, $mIndex) {
									if (\is_resource($rResource)) {
										$sContentType =
											$sContentType
											?: \opacit_mail\Engine\File\MimeType::fromStream($rResource, $sFileName)
											?: \opacit_mail\Engine\File\MimeType::fromFilename($sFileName)
											?: 'application/octet-stream'; // 'text/plain'

										$sTempName .= \opacit_mail\Engine\File\MimeType::toExtension($sContentType);
...
//												'size' => 0
							...
							$sContentType = \opacit_mail\Engine\File\MimeType::fromStream($rResource, $sTempName)
								?: \opacit_mail\Engine\File\MimeType::fromFilename($sTempName)
								?: 'application/octet-stream'; // 'text/plain'
							$aAttachments[$mIndex] = [
//								'name' => '',
								'tempName' => $sTempName,
...
//								'size' => $oFilesProvider->FileSize($oAccount, $sTempName)
	 ...
	 * @throws \opacit_mail\Mail\Net\Exceptions\ConnectionException
	 */
	private function smtpSendMessage(Account $oAccount, \opacit_mail\Mail\Mime\Message $oMessage,
		...
		bool $bDsn = false, bool $bRequireTLS = false)
	{
		...
		if (!$oRcpt || !$oRcpt->count()) {
			throw new ClientException(Notifications::InvalidRecipients->value);
		}
		...
		try
		{
			...
			$sFrom = $oFrom instanceof \opacit_mail\Mail\Mime\Email ? $oFrom->GetEmail() : '';
			...
			$oSmtpClient = new \opacit_mail\Mail\Smtp\SmtpClient();
			...
			if ($oSmtpClient->IsConnected()) {
				...
				foreach ($oRcpt as /* @var $oEmail \opacit_mail\Mail\Mime\Email */ $oEmail) {
		...
		catch (\opacit_mail\Mail\Net\Exceptions\ConnectionException $oException)
		{
			if ($oSmtpClient && $oSmtpClient->Settings->viewErrors) {
				throw new ClientException(Notifications::ClientViewError->value, $oException);
			}
			throw new ClientException(Notifications::ConnectionError->value, $oException);
		}
		catch (\opacit_mail\Mail\Smtp\Exceptions\LoginException $oException)
		{
			throw new ClientException(Notifications::AuthError->value, $oException);
		}
		catch (\Throwable $oException)
	...
	private function buildReadReceiptMessage(Account $oAccount) : \opacit_mail\Mail\Mime\Message
	{
		...
		$oMessage = new \opacit_mail\Mail\Mime\Message();

		if ($this->Config()->Get('security', 'hide_x_mailer_header', true)) {
			$oMessage->DoesNotAddDefaultXMailer();
		}

		$oMessage->SetFrom(new \opacit_mail\Mail\Mime\Email($oIdentity->Email(), $oIdentity->Name()));
		...
		if (!empty($sReplyTo)) {
			$oReplyTo = new \opacit_mail\Mail\Mime\EmailCollection($sReplyTo);
		...
		$oToEmails = new \opacit_mail\Mail\Mime\EmailCollection($sReadReceipt);
		...
		$oPart = new MimePart;
		$oPart->Headers->AddByName(MimeEnumHeader::CONTENT_TYPE->value, 'text/plain; charset="utf-8"');
		$oPart->Headers->AddByName(MimeEnumHeader::CONTENT_TRANSFER_ENCODING->value, 'quoted-printable');
		$oPart->Body = \opacit_mail\Mail\Base\StreamWrappers\Binary::CreateStream(
			\opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString(\preg_replace('/\\r?\\n/su', "\r\n", \trim($sText))),
	 ...
	 */
	private function buildMessage(Account $oAccount, bool $bWithDraftInfo = true) : \opacit_mail\Mail\Mime\Message
	{
		$oMessage = new \opacit_mail\Mail\Mime\Message();
		...
		$oMessage->SetFrom(\opacit_mail\Mail\Mime\Email::Parse($this->GetActionParam('from', '')));
		$oFrom = $oMessage->GetFrom();

		$oMessage->RegenerateMessageId($oFrom ? $oFrom->GetDomain() : '');

		$oMessage->SetReplyTo(new \opacit_mail\Mail\Mime\EmailCollection($this->GetActionParam('replyTo', '')));
...
//			$oMessage->SetReadReceipt(($oIdentity ?: $oAccount)->Email());
		...
		if (!empty($this->GetActionParam('markAsImportant', 0))) {
			$oMessage->SetPriority(\opacit_mail\Mail\Mime\Enumerations\MessagePriority::HIGH->value);
		}

		$oMessage->SetSubject($this->GetActionParam('subject', ''));

		$oMessage->SetTo(new \opacit_mail\Mail\Mime\EmailCollection($this->GetActionParam('to', '')));
		$oMessage->SetCc(new \opacit_mail\Mail\Mime\EmailCollection($this->GetActionParam('cc', '')));
		$oMessage->SetBcc(new \opacit_mail\Mail\Mime\EmailCollection($this->GetActionParam('bcc', '')));
...
//			$sProtocol = $match[1][0];
			...
			$oPart->Headers->AddByName(
				MimeEnumHeader::CONTENT_TYPE->value,
			...
			$oPart = new MimePart;
			$oPart->Headers->AddByName(MimeEnumHeader::CONTENT_TYPE->value, 'multipart/alternative');
			$oMessage->SubParts->append($oPart);

			$sHtml = \opacit_mail\Mail\Base\HtmlUtils::BuildHtml($sHtml, $aFoundCids, $aFoundDataURL, $aFoundContentLocationUrls);
			...
			// First add plain
			$sPlain = $this->GetActionParam('plain', '') ?: \opacit_mail\Mail\Base\HtmlUtils::ConvertHtmlToPlain($sHtml);
			...
			$oAlternativePart->Headers->AddByName(MimeEnumHeader::CONTENT_TYPE->value, 'text/html; charset=utf-8');
			$oAlternativePart->Headers->AddByName(MimeEnumHeader::CONTENT_TRANSFER_ENCODING->value, 'quoted-printable');
			$oAlternativePart->Body = \opacit_mail\Mail\Base\StreamWrappers\Binary::CreateStream(
				\opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString(\preg_replace('/\\r?\\n/su', "\r\n", \trim($sHtml))),
				'convert.quoted-printable-encode'
			);
...
/*
			if ($sSignature = $this->GetActionParam('pgpSignature', null)) {
				...
				$oAlternativePart = new MimePart;
				$oAlternativePart->Headers->AddByName(MimeEnumHeader::CONTENT_TYPE->value, 'text/plain; charset="utf-8"; protected-headers="v1"');
				$oAlternativePart->Headers->AddByName(MimeEnumHeader::CONTENT_TRANSFER_ENCODING->value, 'base64');
				$oAlternativePart->Body = \opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString(\preg_replace('/\\r?\\n/su', "\r\n", \trim($sPlain)));
				$oPart->SubParts->append($oAlternativePart);

				$oAlternativePart = new MimePart;
				$oAlternativePart->Headers->AddByName(MimeEnumHeader::CONTENT_TYPE->value, 'application/pgp-signature; name="signature.asc"');
				$oAlternativePart->Headers->AddByName(MimeEnumHeader::CONTENT_TRANSFER_ENCODING->value, '7Bit');
				$oAlternativePart->Body = \opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString(\preg_replace('/\\r?\\n/su', "\r\n", \trim($sSignature)));
				$oPart->SubParts->append($oAlternativePart);
			...
			foreach ($aAttachments as $sTempName => $aData) {
				...
				if (\is_resource($rResource)) {
					...
					$oMessage->Attachments()->append(
						new \opacit_mail\Mail\Mime\Attachment($rResource, $sFileName, $iFileSize, $bIsInline,
...
/*
		...
		if ($sPublicKey) {
			$oMessage->Attachments()->append(
				new \opacit_mail\Mail\Mime\Attachment(
...
*/
		foreach ($aFoundDataURL as $sCidHash => $sDataUrlString) {
			...
			{
				$sRaw = \opacit_mail\Mail\Base\Utils::Base64Decode($aMatch[2]);
				$iFileSize = \strlen($sRaw);
				if (0 < $iFileSize) {
					$sFileName = \preg_replace('/[^a-z0-9]+/i', '.', $aMatch[1]);
					$rResource = \opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString($sRaw);
					...
					$oMessage->Attachments()->append(
						new \opacit_mail\Mail\Mime\Attachment($rResource, $sFileName, $iFileSize, true, true, $sCID)
					);
				}
			}
		}

		$oPassphrase = new \opacit_mail\Engine\SensitiveString($this->GetActionParam('signPassphrase', ''));
...
*/
			...
			\opacit_mail\Mail\Base\StreamFilters\LineEndings::appendTo($resource);
...
//			\stream_copy_to_stream($resource, $fp); // Fails
			...
			$oSignaturePart->Headers->AddByName(MimeEnumHeader::CONTENT_TYPE->value, 'application/pgp-signature; name="signature.asc"');
			$oSignaturePart->Headers->AddByName(MimeEnumHeader::CONTENT_TRANSFER_ENCODING->value, '7Bit');
...
*/
			if ($sCertificate && $sPrivateKey) {
				...
				$resource = $oBody->ToStream();
				\opacit_mail\Mail\Base\StreamFilters\LineEndings::appendTo($resource);
				$tmp = new \opacit_mail\Engine\File\Temporary('mimepart');
				...
				if ($detached) {
					...
					$oSignaturePart = new MimePart;
					$oSignaturePart->Headers->AddByName(MimeEnumHeader::CONTENT_TYPE->value, 'application/pkcs7-signature; name="signature.p7s"');
					$oSignaturePart->Headers->AddByName(MimeEnumHeader::CONTENT_TRANSFER_ENCODING->value, 'base64');
					$oSignaturePart->Body = $sSignature;
					$oPart->SubParts->append($oSignaturePart);
				} else {
					$oPart->Headers->AddByName(
					...
					$oPart->Headers->AddByName(MimeEnumHeader::CONTENT_TRANSFER_ENCODING->value, 'base64');
