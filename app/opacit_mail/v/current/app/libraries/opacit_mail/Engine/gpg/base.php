...
//		$this->pinentries[\substr($keyId, -16)] = $passphrase;
	...
	{
		\opacit_mail\Engine\Log::debug('GPG', $msg);
	}
...
//		throw new \Exception("Passphrase not found for {$key}");
	...
	protected static function findBinary($name) : ?string
	{
		...
		if ($binary && \opacit_mail\Engine\Utils::inOpenBasedir($binary) && \is_executable($binary)) {
			return $binary;
		}
		...
		], '\opacit_mail\Engine\Utils::inOpenBasedir');
