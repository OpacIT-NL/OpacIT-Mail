...
class Domain implements \JsonSerializable
{
	...
	private \opacit_mail\Mail\Imap\Settings $IMAP;

	private \opacit_mail\Mail\Smtp\Settings $SMTP;

	private \opacit_mail\Mail\Sieve\Settings $Sieve;
	...
	{
//		$this->Name = \opacit_mail\Engine\IDN::toAscii($sName);
		$this->Name = \strtolower(\idn_to_ascii($sName));
		$this->IMAP = new \opacit_mail\Mail\Imap\Settings;
		$this->SMTP = new \opacit_mail\Mail\Smtp\Settings;
		$this->Sieve = new \opacit_mail\Mail\Sieve\Settings;
	}
	...
	{
//		$this->aliasName = \opacit_mail\Engine\IDN::toAscii($sAliasName);
	...
	public function ValidateWhiteList(string $sEmail) : bool
	{
		...
		$sEmail = \opacit_mail\Engine\IDN::emailToAscii(\mb_strtolower($sEmail));
		$iPos = \strrpos($sEmail, '@');
		$sUserPart = \substr($sEmail, 0, $iPos);
		$sUserDomain = \substr($sEmail, $iPos);
		$sItem = \strtok($sW, " ;,\n");
		while (false !== $sItem) {
			$sItem = \opacit_mail\Engine\IDN::emailToAscii(\mb_strtolower(\trim($sItem)));
	...
	public function ImapSettings() : \opacit_mail\Mail\Imap\Settings
	{
		return $this->IMAP;
	}

	public function SieveSettings() : \opacit_mail\Mail\Sieve\Settings
	{
		return $this->Sieve;
	}

	public function SmtpSettings() : \opacit_mail\Mail\Smtp\Settings
	{
		return $this->SMTP;
	}
	...
	public static function fromArray(string $sName, array $aDomain) : ?self
	{
		...
		if (!empty($aDomain['IMAP'])) {
			$oDomain->IMAP = \opacit_mail\Mail\Imap\Settings::fromArray($aDomain['IMAP']);
			$oDomain->SMTP = \opacit_mail\Mail\Smtp\Settings::fromArray($aDomain['SMTP']);
			$oDomain->Sieve = \opacit_mail\Mail\Sieve\Settings::fromArray($aDomain['Sieve']);
