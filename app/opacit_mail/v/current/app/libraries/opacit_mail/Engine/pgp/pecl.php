...
class PECL implements \opacit_mail\Engine\PGP\PGPInterface
{
	private
		$homedir,
		// Instance of gnupg pecl extension https://www.php.net/gnupg
		$GnuPG,
		// Instance of \opacit_mail\Engine\GPG\PGP
		$GPG;
...
*/
	...
	{
//		\opacit_mail\Engine\Log::debug('GnuPG', "addDecryptKey({$fingerprint}, {$passphrase})");
	...
	{
//		\opacit_mail\Engine\Log::debug('GnuPG', "addSignKey({$fingerprint}, {$passphrase})");
...
//		\rewind($fp);
	...
	public function verify(string $signed_text, string $signature, ?string &$plaintext = null) /*: array|false*/
	{
		...
		if (!$result) {
			\opacit_mail\Engine\Log::notice('GnuPG', 'gnupg_verify() failed: ' . $this->GnuPG->geterror());
			\opacit_mail\Engine\Log::info('GnuPG', \print_r($this->GnuPG->geterrorinfo(),1));
		}
		return $result;
	}
