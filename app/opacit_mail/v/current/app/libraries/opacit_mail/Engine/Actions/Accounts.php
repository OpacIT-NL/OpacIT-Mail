...
use opacit_mail\Engine\Model\MainAccount;
use opacit_mail\Engine\Model\AdditionalAccount;
use opacit_mail\Engine\Model\Identity;
use opacit_mail\Engine\Notifications;
use opacit_mail\Engine\Providers\Identities;
...
trait Accounts
{
	private ?\opacit_mail\Engine\Providers\Identities $oIdentitiesProvider = null;
	...
	public function GetAccounts(MainAccount $oAccount): array
	{
		if ($this->GetCapa(Capa::ADDITIONAL_ACCOUNTS->value)) {
			...
			$aAccounts = $sAccounts ? \json_decode($sAccounts, true)
				: \opacit_mail\Engine\Upgrade::ConvertInsecureAccounts($this, $oAccount);
	...
	public function DoAccountSetup(): array
	{
		...
		$oPassword = new \opacit_mail\Engine\SensitiveString($this->GetActionParam('password', ''));
		$bNew = !empty($this->GetActionParam('new', 1));

		if ($bNew || \strlen($oPassword)) {
			/** @var \opacit_mail\Engine\Model\AdditionalAccount $oNewAccount */
			...
			$aAccount = \opacit_mail\Engine\Model\AdditionalAccount::convertArray($aAccounts[$sEmail]);
		}
	...
	protected function loadAdditionalAccountImapClient(string $sEmail): \opacit_mail\Mail\Imap\ImapClient
	{
		...
		$oImapClient = new \opacit_mail\Mail\Imap\ImapClient;
	...
	public function DoAccountImport(): array
	{
		...
		$oMainAccount = $this->getMainAccountFromToken();
		$oImapTarget = new \opacit_mail\Mail\Imap\ImapClient;
		$oImapTarget->SetLogger($this->Logger());
		$this->imapConnect($oMainAccount, false, $oImapTarget);

		$oSync = new \opacit_mail\Engine\Imap\Sync;
...
//				$this->SetAdditionalAuthToken(null);
				\opacit_mail\Engine\Cookies::clear(self::AUTH_ADDITIONAL_TOKEN_KEY);
				$bReload = true;
			}
...
//			'Email' => IDN::emailToUtf8($oAccount->Email()),
			...
			'mainEmail' => \opacit_mail\Engine\Api::Actions()->getMainAccountFromToken()->Email(),
...
/*		// TODO: verify private key for certificate?
		if ($oIdentity->smimeCertificate && $oIdentity->smimeKey) {
			new \opacit_mail\Engine\SMime\Certificate($oIdentity->smimeCertificate, $oIdentity->smimeKey);
		}
*/
