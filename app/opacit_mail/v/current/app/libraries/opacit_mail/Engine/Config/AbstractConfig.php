...
abstract class AbstractConfig implements \ArrayAccess, \JsonSerializable
{
	...
	public function __construct(string $sFileName, string $sFileHeader = '', string $sAdditionalFileName = '')
	{
		...
		$this->bUseApcCache = defined('APP_USE_APCU_CACHE') && APP_USE_APCU_CACHE &&
			\opacit_mail\Mail\Base\Utils::FunctionsCallable(array('apcu_fetch', 'apcu_store'));
	}
	...
	{
		// $salt = \basename($this->sFile) not possible due to opacit_mail\Engine\Plugins\Property
		if (!empty($this->aData[$sSection][$sName][0])) try {
			return \opacit_mail\Engine\Crypt::DecryptFromJSON($this->aData[$sSection][$sName][0], \APP_SALT);
		} catch (\Throwable $e) {
		}
		return $mDefault;
	}
	...
	{
		// $salt = \basename($this->sFile) not possible due to opacit_mail\Engine\Plugins\Property
		$mParamValue = \opacit_mail\Engine\Crypt::EncryptToJSON($mParamValue, \APP_SALT);
	...
	public function Save() : bool
	{
		...
		if (\str_ends_with($this->sFile, '.json')) {
			$this->clearCache();
			\opacit_mail\Engine\Utils::saveFile($this->sFile, \json_encode($this, \JSON_PRETTY_PRINT | \JSON_UNESCAPED_SLASHES));
			// Remove old .ini file
		...
		\opacit_mail\Engine\Utils::saveFile($this->sFile,
