...
//			'version' => APP_VERSION,
	...
	public function ExceptionResponse(\Throwable $oException) : array
	{
		...
		if ($oException instanceof \opacit_mail\Engine\Exceptions\ClientException) {
	...
	private function responseObject($mResponse, string $sParent = '')
	{
		...
		if ($mResponse instanceof \opacit_mail\Mail\Client\Message) {
			...
			if (!$sParent && \strlen($aResult['readReceipt']) && !\in_array('$mdnsent', $aResult['flags']) && !\in_array('\\answered', $aResult['flags'])) {
				...
				if ('1' === $this->Cacher($oAccount)->Get(\opacit_mail\Engine\KeyPathHelper::ReadReceiptCache($oAccount->Email(), $aResult['folder'], $aResult['uid']), '0')) {
					$aResult['readReceipt'] = '';
				}
			}
			return $aResult;
		...
		if ($mResponse instanceof \opacit_mail\Mail\Imap\Folder) {
			$aResult = $mResponse->jsonSerialize();
