...
namespace opacit_mail\Mail\Sieve;
 ...
 * @package Sieve
 */
class SieveClient extends \opacit_mail\Mail\Net\NetClient
{
	/** @var Settings */
	...
	public function Login(Settings $oSettings) : self
	{
		...
		if ($this->hasCapability('SASL')) {
			foreach ($oSettings->SASLMechanisms as $sasl_type) {
				if (\in_array(\strtoupper($sasl_type), $this->aAuth) && \opacit_mail\Engine\SASL::isSupported($sasl_type)) {
					$type = $sasl_type;
					break;
				}
			}
...
//			$this->writeLogException(new \UnexpectedValueException('No supported SASL mechanism found'), \LOG_ERR);
			$this->writeLogException(new \opacit_mail\Mail\Sieve\Exceptions\LoginException, \LOG_ERR);
		}
		...
		catch (\opacit_mail\Mail\Sieve\Exceptions\NegativeResponseException $oException)
		{
			$this->writeLogException(
				new \opacit_mail\Mail\Sieve\Exceptions\LoginBadCredentialsException($oException->GetResponses(), '', 0, $oException),
				\LOG_ERR);
		}

		$bAuth || $this->writeLogException(new \opacit_mail\Mail\Sieve\Exceptions\LoginBadMethodException, \LOG_ERR);
	...
	private function parseResponse() : array
	{
		...
		while (true) {
			...
			// \opacit_mail\Mail\Imap\Enumerations\ResponseStatus
		...
		if (!$aResult || 'OK' !== \substr($aResult[\array_key_last($aResult)], 0, 2)) {
			$this->writeLogException(new \opacit_mail\Mail\Sieve\Exceptions\NegativeResponseException($aResult), \LOG_WARNING);
		}
