...
use opacit_mail\Mail\Mime\Enumerations\ContentType;
...
class BodyStructure implements \JsonSerializable
{
	...
	/**
	 * \opacit_mail\Mail\Imap\BodyStructure[]
	 */
...
//		return 'application/pgp-encrypted' !== $this->sContentType
	...
	public function GetHtmlAndPlainParts() : array
	{
		 ...
		 * Only when \opacit_mail\Engine\Api::Config()->Get('security', 'openpgp', true)
		 */
		if (!$aParts) {
	...
	public function SearchByCallback(callable $fCallback, /*BodyStructure*/ $parent = null) : \Generator
	{
		if ($fCallback($this, $parent)) {
			yield $this;
		}
		foreach ($this->aSubParts as /* @var $oSubPart \opacit_mail\Mail\Imap\BodyStructure */ $oSubPart) {
			yield from $oSubPart->SearchByCallback($fCallback, $this);
		}
	}

	public function SearchAttachmentsParts() : \Generator
	{
...
//			return $oItem->IsAttachment();
	...
	public function SearchByContentType(string $sContentType) : \Generator
	{
	...
	private static function decodeAttrParameter(array $aParams, string $sParamName, string $sCharset) : string
	{
		...
		if (isset($aParams[$sParamName])) {
			$sResult = \opacit_mail\Mail\Base\Utils::DecodeHeaderValue($aParams[$sParamName], $sCharset);
		} else if (isset($aParams[$sParamName.'*'])) {
			$aValueParts = \explode("''", $aParams[$sParamName.'*'], 2);
			if (2 === \count($aValueParts)) {
				$sCharset = isset($aValueParts[0]) ? $aValueParts[0] : \opacit_mail\Mail\Base\Enumerations\Charset::UTF_8->value;
				$sResult = \opacit_mail\Mail\Base\Utils::ConvertEncoding(
					\urldecode($aValueParts[1]), $sCharset, \opacit_mail\Mail\Base\Enumerations\Charset::UTF_8->value);
			} else {
			...
			if (\count($aFileNames)) {
				...
				if (\strlen($sCharset)) {
					$sResult = \opacit_mail\Mail\Base\Utils::ConvertEncoding($sResult,
						$sCharset, \opacit_mail\Mail\Base\Enumerations\Charset::UTF_8->value);
				}
			}
		}
	...
	public static function NewInstance(array $aBodyStructure, string $sPartID = '') : ?self
	{
		...
		$oStructure->sFileName = \opacit_mail\Mail\Base\Utils::Utf8Clear($sFileName);
...
//			'mimeTypeParams' => $this->aContentTypeParams,
			'fileName' => \opacit_mail\Mail\Base\Utils::SecureFileName($this->FileName(true)),
