...
namespace opacit_mail\Mail\Mime;
...
class Header implements \JsonSerializable
{
	...
	public static function NewInstanceFromEncodedString(string $sEncodedLines, string $sIncomingCharset = \opacit_mail\Mail\Base\Enumerations\Charset::ISO_8859_1->value) : Header|false
	{
		if (empty($sIncomingCharset)) {
			$sIncomingCharset = \opacit_mail\Mail\Base\Enumerations\Charset::ISO_8859_1->value;
		}
		...
		if (isset($aParts[0]) && isset($aParts[1]) && \strlen($aParts[0]) && \strlen($aParts[1])) {
			return new self(
				\trim($aParts[0]),
				\trim(\opacit_mail\Mail\Base\Utils::DecodeHeaderValue(\trim($aParts[1]), $sIncomingCharset)),
				\trim($aParts[1]),
				$sIncomingCharset
			);
		}
	...
	public function SetParentCharset(string $sParentCharset) : Header
	{
		if ($this->sParentCharset !== $sParentCharset && \strlen($this->sEncodedValue)) {
			$this->initInputData(
				$this->sName,
				\trim(\opacit_mail\Mail\Base\Utils::DecodeHeaderValue($this->sEncodedValue, $sParentCharset)),
				$this->sEncodedValue
			);
		}
	...
	public function __toString() : string
	{
		...
		if ($this->IsSubject()) {
			if (!\opacit_mail\Mail\Base\Utils::IsAscii($sResult) && \function_exists('iconv_mime_encode')) {
				return \iconv_mime_encode($this->Name(), $sResult, array(
//					'scheme' => \opacit_mail\Mail\Base\Enumerations\Encoding::QUOTED_PRINTABLE_SHORT,
					'scheme' => \opacit_mail\Mail\Base\Enumerations\Encoding::BASE64_SHORT->value,
					'input-charset' => \opacit_mail\Mail\Base\Enumerations\Charset::UTF_8->value,
					'output-charset' => \opacit_mail\Mail\Base\Enumerations\Charset::UTF_8->value,
					'line-length' => 74,
					'line-break-chars' => "\r\n"
				));
			}
...
//			\strtolower(Enumerations\Header::RETURN_PATH->value),
	...
	{
		if (!\opacit_mail\Mail\Base\Utils::IsAscii($this->Value())
		 && \strlen($this->sEncodedValue)
		 && !\opacit_mail\Mail\Base\Utils::IsAscii($this->sEncodedValue)
