...
*/
	...
	{
		$sReferences = \opacit_mail\Mail\Base\Utils::StripSpaces($sReferences);
	...
	private function generateNewMessageId(string $sHostName = '') : string
	{
		...
		if (empty($sHostName) && \opacit_mail\Mail\Base\Utils::FunctionCallable('php_uname')) {
		...
		return '<'.
			\opacit_mail\Mail\Base\Utils::Sha1Rand($sHostName.
				(\opacit_mail\Mail\Base\Utils::FunctionCallable('getmypid') ? \getmypid() : '')).'@'.$sHostName.'>';
	}
				...
				if (1 === \count($aAttachments) && isset($aAttachments[0])) {
					...
					if ($resource = $aAttachments[0]->Resource()) {
						if (\is_resource($resource)) {
							...
							$oPart->Body = \opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString($resource);
						}
					}
	...
	public function ToStream(bool $bWithoutBcc = false)
	{
		...
		if ($this->bAddDefaultXMailer && !isset($this->aHeadersValue[Enumerations\Header::X_MAILER->value])) {
			$oRootPart->Headers->SetByName(Enumerations\Header::X_MAILER->value, 'opacit_mail/'.APP_VERSION, true);
		}
