...
class FetchResponse
{
	...
	public function GetEnvelope(bool $bForce = false) : ?array
	{
		if (null === $this->aEnvelopeCache || $bForce) {
			$this->aEnvelopeCache = $this->GetFetchValue(Enumerations\FetchType::ENVELOPE->value);
		}
	...
	public function GetFetchEnvelopeEmailCollection(int $iIndex, string $sParentCharset = \opacit_mail\Mail\Base\Enumerations\Charset::ISO_8859_1->value) : ?\opacit_mail\Mail\Mime\EmailCollection
	{
		$oResult = null;
		$aEmails = $this->GetFetchEnvelopeValue($iIndex);
		if (\is_array($aEmails) && \count($aEmails)) {
			$oResult = new \opacit_mail\Mail\Mime\EmailCollection;
			foreach ($aEmails as $aEmailItem) {
				if (\is_array($aEmailItem) && 4 === \count($aEmailItem)) {
					$sDisplayName = \opacit_mail\Mail\Base\Utils::DecodeHeaderValue(
						self::findEnvelopeIndex($aEmailItem, 0, ''), $sParentCharset);

//					$sRemark = \opacit_mail\Mail\Base\Utils::DecodeHeaderValue(
//						self::findEnvelopeIndex($aEmailItem, 1, ''), $sParentCharset);
					...
					if (\strlen($sLocalPart) && \strlen($sDomainPart)) {
						$oResult->append(
							new \opacit_mail\Mail\Mime\Email($sLocalPart.'@'.$sDomainPart, $sDisplayName)
						);
					}
				}
			}
	...
	{
		$aBodyStructureArray = $this->GetFetchValue(Enumerations\FetchType::BODYSTRUCTURE->value);
	...
	public static function isValidImapResponse(Response $oImapResponse) : bool
	{
		return
			...
			&& Enumerations\ResponseType::UNTAGGED->value === $oImapResponse->ResponseType
	...
	{
		return \in_array(Enumerations\FetchType::UID->value, $oImapResponse->ResponseList[3])
			&& \in_array(Enumerations\FetchType::RFC822_SIZE->value, $oImapResponse->ResponseList[3]);
	}
