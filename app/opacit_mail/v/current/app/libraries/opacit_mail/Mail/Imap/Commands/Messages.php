...
namespace opacit_mail\Mail\Imap\Commands;

use opacit_mail\Mail\Imap\FetchResponse;
use opacit_mail\Mail\Imap\Enumerations\FetchType;
use opacit_mail\Mail\Imap\ResponseCollection;
use opacit_mail\Mail\Imap\SequenceSet;
use opacit_mail\Mail\Imap\Enumerations\MessageFlag;
use opacit_mail\Mail\Imap\Enumerations\ResponseType;
	...
	public function FetchIterate(array $aInputFetchItems, string $sIndexRange, bool $bIndexIsUid) : iterable
	{
		...
		try {
			...
			foreach ($aInputFetchItems as $mFetchKey) {
				switch ($mFetchKey)
				{
					case FetchType::UID->value:
					case FetchType::RFC822_SIZE->value:
						// Already defined by default
						break;

					// Macro's
					case FetchType::FULL->value:
						$aFetchItems[] = FetchType::BODY->value;
						// Falls through
					case FetchType::ALL->value:
						$aFetchItems[] = FetchType::ENVELOPE->value;
						// Falls through
					case FetchType::FAST->value:
						$aFetchItems[] = FetchType::FLAGS->value;
						$aFetchItems[] = FetchType::INTERNALDATE->value;
						break;
			...
			if ($this->hasCapability('OBJECTID')) {
				$aFetchItems[] = FetchType::EMAILID->value;
				$aFetchItems[] = FetchType::THREADID->value;
...
/*
			if ($this->hasCapability('X-GM-EXT-1') && \in_array(FetchType::FLAGS->value, $aFetchItems)) {
...
*/
	...
	public function FetchMessagePart(int $iUid, string $sPartId) : string
	{
		if ('TEXT' === $sPartId) {
			...
			$sHeader = $oFetchResponse->GetFetchValue(FetchType::BODY_HEADER->value);
		} else {
			$oFetchResponse = $this->Fetch([
				FetchType::BODY_PEEK->value.'['.$sPartId.']',
			...
			$sHeader = $oFetchResponse->GetFetchValue(FetchType::BODY->value.'['.$sPartId.'.MIME]');
		}
		return $sHeader . $oFetchResponse->GetFetchValue(FetchType::BODY->value.'['.$sPartId.']');
	}
...
*/
	...
	private function writeMessageStream($rMessageStream) : ?int
	{
		...
		\opacit_mail\Mail\Base\Utils::WriteStream($rMessageStream, $this->ConnectionResource());
	...
	*/
	...
	public function MessageDelete(string $sFolder, SequenceSet $oRange, bool $bExpungeAll = false) : void
	{
		...
		$this->MessageStoreFlag($oRange,
			array(MessageFlag::DELETED->value),
			StoreAction::ADD_FLAGS_SILENT->value
		);
...
*/
	 ...
	 * @throws \InvalidArgumentException
	 * @throws \opacit_mail\Mail\RuntimeException
	 * $sStoreAction = \opacit_mail\Mail\Imap\Enumerations\StoreAction::ADD_FLAGS_SILENT->value
	 */
	public function MessageStoreFlag(SequenceSet $oRange, array $aInputStoreItems, string $sStoreAction) : ?ResponseCollection
	{
		...
		$aInputStoreItems = \array_map('\\opacit_mail\Mail\\Base\\Utils::Utf8ToUtf7Modified', $aInputStoreItems);
	...
	{
		$oSort = new \opacit_mail\Mail\Imap\Requests\SORT($this);
	...
	{
		$oESearch = new \opacit_mail\Mail\Imap\Requests\ESEARCH($this);
...
//		if (!$this->UTF8 && !\mb_check_encoding($sSearchCriterias, 'UTF-8')) {
		if (!$this->UTF8 && !\opacit_mail\Mail\Base\Utils::IsAscii($sSearchCriterias)) {
...
//		if (!$this->UTF8 && !\mb_check_encoding($sSearchCriterias, 'UTF-8')) {
		...
		if ('' !== $sCont) {
			...
			if ($oItem && ResponseType::CONTINUATION->value === $oItem->ResponseType) {
				$aParts = \explode("\r\n", $sCont);
		...
		foreach ($oResult as $oResponse) {
			...
			if (ResponseType::UNTAGGED->value === $oResponse->ResponseType
	...
	{
		$oThread = new \opacit_mail\Mail\Imap\Requests\THREAD($this);
