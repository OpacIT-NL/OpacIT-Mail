		...
		if ('message/rfc822' !== \strtolower($this->ContentType())) {
			$oAttachmentPart->Headers->append(
				new Header(
					...
					\opacit_mail\Mail\Base\Enumerations\Encoding::BASE64_LOWER->value
				)
			);

			if (\is_resource($oAttachmentPart->Body) && !\opacit_mail\Mail\Base\StreamWrappers\Binary::IsStreamRemembed($oAttachmentPart->Body)) {
				$oAttachmentPart->Body =
					\opacit_mail\Mail\Base\StreamWrappers\Binary::CreateStream($oAttachmentPart->Body,
						\opacit_mail\Mail\Base\StreamWrappers\Binary::GetInlineDecodeOrEncodeFunctionName(
							\opacit_mail\Mail\Base\Enumerations\Encoding::BASE64_LOWER->value, false));

				\opacit_mail\Mail\Base\StreamWrappers\Binary::RememberStream($oAttachmentPart->Body);
			}
		}
