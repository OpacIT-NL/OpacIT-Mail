...
use opacit_mail\Mail\Imap\Responses\ACL as ACLResponse;
...
*/
	...
	private function FolderACLRequest(string $sFolderName, string $sCommand, array $aParams) : \opacit_mail\Mail\Imap\ResponseCollection
	{
...
//			\array_unshift($aParams, $this->EscapeFolderName($sFolderName));
		...
		return new \opacit_mail\Mail\Imap\ResponseCollection();
	}
	...
	public function FolderGetACL(string $sFolderName) : array
	{
		...
		if ($response->hasRight('a')) {
			...
			foreach ($oResponses as $oResponse) {
				// * ACL INBOX.shared demo@opacit_mail.dev akxeilprwtscd demo2@opacit_mail.dev lrwstipekxacd
				if (\opacit_mail\Mail\Imap\Enumerations\ResponseType::UNTAGGED->value === $oResponse->ResponseType
