...
//		$aFlags = [],
	...
	protected ?\opacit_mail\Mail\Mime\EmailCollection
		$oFrom = null,
		$oSender = null,
		$oReplyTo = null,
		$oDeliveredTo = null,
	...
	private ?\opacit_mail\Mail\Mime\HeaderCollection
		$Headers = null;
	...
	public function From() : ?\opacit_mail\Mail\Mime\EmailCollection
	{
		return $this->oFrom;
	}
	...
	public static function fromFetchResponse(string $sFolder, \opacit_mail\Mail\Imap\FetchResponse $oFetchResponse, ?\opacit_mail\Mail\Imap\BodyStructure $oBodyStructure = null) : self
	{
		$oMessage = new self;
...
//		$oMessage->aFlags = $aFlags;
		$oMessage->aFlagsLowerCase = \array_map('mb_strtolower', \array_map('\\opacit_mail\Mail\\Base\\Utils::Utf7ModifiedToUtf8', $aFlags));
		$oMessage->iInternalTimeStampInUTC = \opacit_mail\Mail\Base\DateTimeHelper::ParseInternalDateString(
...
//		$oMessage->sEmailId = $oMessage->sEmailId ?: $oFetchResponse->GetFetchValue('X-GUID');
		...
		$oHeaders = \strlen($sHeaders) ? new \opacit_mail\Mail\Mime\HeaderCollection($sHeaders, $sCharset) : null;
		if ($oHeaders) {
			$oMessage->Headers = $oHeaders;

			$sContentTypeCharset = $oHeaders->ParameterValue(
				MimeHeader::CONTENT_TYPE->value,
				\opacit_mail\Mail\Mime\Enumerations\Parameter::CHARSET->value
			);
			...
			$oMessage->iHeaderTimeStampInUTC = \opacit_mail\Mail\Base\DateTimeHelper::ParseRFC2822DateString(
...
//			$oMessage->sDeliveryReceipt = \trim($oHeaders->ValueByName(MimeHeader::RETURN_RECEIPT_TO->value));
			...
			if ($sReadReceipt) {
				try
				{
					if (!\opacit_mail\Mail\Mime\Email::Parse($sReadReceipt)) {
						$sReadReceipt = '';
					}
				}
			...
			if (\strlen($sDraftInfo)) {
				...
				$oParameters = new \opacit_mail\Mail\Mime\ParameterCollection($sDraftInfo);
		...
		{
			$sCharset = $sCharset ?: \opacit_mail\Mail\Base\Enumerations\Charset::ISO_8859_1->value;
...
*/
			...
			if ($aTextParts) {
				$sCharset = $sCharset ?: \opacit_mail\Mail\Base\Enumerations\Charset::UTF_8->value;
				...
				foreach ($aTextParts as $oPart) {
					...
					if (\is_string($sText) && \strlen($sText)) {
						...
						$sText = Utils::ConvertEncoding($sText,
							...
							\opacit_mail\Mail\Base\Enumerations\Charset::UTF_8->value
						);
			...
			if ($gAttachmentsParts->valid()) {
				...
				foreach ($gAttachmentsParts as /* @var $oAttachmentItem \opacit_mail\Mail\Imap\BodyStructure */ $oAttachmentItem) {
...
*/
		$result = array(
			...
			// \opacit_mail\Mail\Mime\EmailCollection
			'from' => $this->oFrom,
