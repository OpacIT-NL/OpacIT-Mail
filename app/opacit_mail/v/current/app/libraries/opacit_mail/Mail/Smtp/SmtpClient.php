...
namespace opacit_mail\Mail\Smtp;

use opacit_mail\Mail\Net\Enumerations\ConnectionSecurityType;
use opacit_mail\Engine\IDN;
 ...
 * @package Smtp
 */
class SmtpClient extends \opacit_mail\Mail\Net\NetClient
{
	/** @var Settings */
	public \opacit_mail\Mail\Net\ConnectSettings $Settings;
	 ...
	 */
	public function Connect(\opacit_mail\Mail\Net\ConnectSettings $oSettings) : void
	{
	...
	private function StartTLS() : void
	{
		if ($this->hasCapability('STARTTLS')) {
			...
			$this->writeLogException(
				new \opacit_mail\Mail\Net\Exceptions\SocketUnsuppoterdSecureConnectionException('STARTTLS is not supported'),
				\LOG_ERR);
		}
	}
	...
	public function Login(Settings $oSettings) : self
	{
		...
		foreach ($oSettings->SASLMechanisms as $sasl_type) {
			if (\in_array(\strtoupper($sasl_type), $this->aAuthTypes) && \opacit_mail\Engine\SASL::isSupported($sasl_type)) {
				$type = $sasl_type;
				break;
			}
		}
		if (!$type) {
			...
			$this->writeLogException(new \opacit_mail\Mail\Smtp\Exceptions\LoginBadMethodException);
		}

		$SASL = \opacit_mail\Engine\SASL::factory($type);
		$sResult = '';

		if ($this->Settings->authPlainLine && $SASL instanceof \opacit_mail\Engine\SASL\Plain) {
			try
			{
			...
			catch (\opacit_mail\Mail\Smtp\Exceptions\NegativeResponseException $oException)
			{
				$this->writeLogException(
					new \opacit_mail\Mail\Smtp\Exceptions\LoginBadCredentialsException($oException->GetResponses(), $oException->getMessage(), 0, $oException)
				);
			}
		} else {
			...
			{
				$this->writeLogException(
					new \opacit_mail\Mail\Smtp\Exceptions\LoginBadMethodException($oException->GetResponses(), $oException->getMessage(), 0, $oException)
				);
			}
...
//			$sCmd .= ' BODY=BINARYMIME';
	...
	{
		if (!$this->bMail) {
			$this->writeLogException(new \opacit_mail\Mail\RuntimeException('No sender reverse path has been supplied'), \LOG_ERR);
		}
...
//		$sTo = IDN::emailToAscii($sTo);
	...
	public function Data(string $sData) : self
	{
		...
		$rDataStream = \opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString($sData);
		unset($sData);
		$this->DataWithStream($rDataStream);
		\opacit_mail\Mail\Base\ResourceRegistry::CloseMemoryResource($rDataStream);
	...
	public function DataWithStream($rDataStream) : self
	{
		...
		if (!$this->bRcpt) {
			$this->writeLogException(new \opacit_mail\Mail\RuntimeException('No recipient forward path has been supplied'), \LOG_ERR);
		}
...
/*
		...
		if ($this->hasCapability('CHUNKING')) {
			...
			while (!\feof($rDataStream)) {
				...
				if (false === $sBuffer) {
					if (!\feof($rDataStream)) {
						$this->writeLogException(new \opacit_mail\Mail\RuntimeException('Cannot read input resource'), \LOG_ERR);
					}
					break;
				}
				$this->sendRequestWithCheck("BDAT " . \strlen($sBuffer) . "\r\n{$sBuffer}", 250);
				\opacit_mail\Mail\Base\Utils::ResetTimeLimit();
			}
...
*/
	...
	{
		$sUser = \opacit_mail\Mail\Base\Utils::Trim($sUser);
/*
		// RFC 6531
		if ($this->hasCapability('SMTPUTF8')) {
...
*/
	...
	/**
	 * @throws \opacit_mail\Mail\Smtp\Exceptions\ResponseException
	 */
