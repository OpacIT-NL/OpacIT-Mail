...
namespace opacit_mail\Mail\Client;

use opacit_mail\Mail\Imap\FolderCollection;
use opacit_mail\Mail\Imap\FolderInformation;
use opacit_mail\Mail\Imap\Enumerations\FetchType;
use opacit_mail\Mail\Imap\Enumerations\MessageFlag;
use opacit_mail\Mail\Imap\Enumerations\StoreAction;
use opacit_mail\Mail\Imap\SequenceSet;
use opacit_mail\Mail\Mime\Enumerations\Header as MimeHeader;
use opacit_mail\Mail\Mime\Enumerations\Parameter as MimeParameter;
...
class MailClient
{
	...
	private \opacit_mail\Mail\Imap\ImapClient $oImapClient;

	private bool $bThreadSort = false;

	function __construct()
	{
		$this->oImapClient = new \opacit_mail\Mail\Imap\ImapClient;
	}
...
//			MimeHeader::RETURN_RECEIPT_TO->value,
		...
		\opacit_mail\Engine\Api::Actions()->Plugins()->RunHook('imap.message-headers', array(&$aHeaders));
	...
	public function MessageSetFlag(string $sFolderName, SequenceSet $oRange, string $sMessageFlag, bool $bSetAction = true, bool $bSkipUnsupportedFlag = false) : void
	{
		if (\count($oRange)) {
			if ($this->oImapClient->FolderSelect($sFolderName)->IsFlagSupported($sMessageFlag)) {
				...
				throw new \opacit_mail\Mail\RuntimeException('Message flag "'.$sMessageFlag.'" is not supported.');
			}
		}
	}
	 ...
	 * @throws \opacit_mail\Mail\RuntimeException
	 */
	public function Message(string $sFolderName, int $iIndex, bool $bIndexIsUid = true, ?\opacit_mail\Mail\Cache\CacheClient $oCacher = null) : ?Message
	{
		if (1 > $iIndex) {
...
//			FetchType::FAST->value,
	...
	public function MessageMimeStream($mCallback, string $sFolderName, int $iIndex, string $sMimeIndex) : bool
	{
		...
		if (\count($aFetchResponse)) {
			...
			if (\strlen($sMime)) {
				$oHeaders = new \opacit_mail\Mail\Mime\HeaderCollection($sMime);
				...
				if (\strlen($sMimeIndex)) {
					...
					$sMailEncoding = \opacit_mail\Mail\Base\StreamWrappers\Binary::GetInlineDecodeOrEncodeFunctionName(
			...
			use ($mCallback, $sMimeIndex, $sMailEncoding, $sContentType, $sFileName)
			{
				if (\strlen($sLiteralAtomUpperCase) && \is_resource($rImapLiteralStream) && 'FETCH' === $sParent) {
					$mCallback($sMailEncoding
						? \opacit_mail\Mail\Base\StreamWrappers\Binary::CreateStream($rImapLiteralStream, $sMailEncoding)
						: $rImapLiteralStream,
...
//				FetchType::BINARY_SIZE->value.'['.$sMimeIndex.']',
				// Push in the aFetchCallbacks array and then called by \opacit_mail\Mail\Imap\Traits\ResponseParser::partialResponseLiteralCallbackCallable
				array(
					$sPeek.'['.$sMimeIndex.']',
					$callback
				)), $iIndex, true);
		} catch (\opacit_mail\Mail\Imap\Exceptions\NegativeResponseException $oException) {
		...
		if ($this->oImapClient->Settings->fetch_new_messages && $iPrevUidNext && $iPrevUidNext != $iCurrentUidNext && 'INBOX' === $sFolderName) {
			...
			foreach ($aFetchResponse as $oFetchResponse) {
				...
				if (!\in_array(\strtolower(MessageFlag::SEEN->value), $aFlags)) {
					...
					$oHeaders = new \opacit_mail\Mail\Mime\HeaderCollection($oFetchResponse->GetHeaderFieldsValue());
...
//			$aInfo = $this->oImapClient->FolderExamine($sFolderName)->jsonSerialize();
			...
			if (\count($oRange)) {
				...
				foreach ($aFetchResponse as $oFetchResponse) {
					$iUid = (int) $oFetchResponse->GetFetchValue(FetchType::UID->value);
					$aLowerFlags = \array_map('mb_strtolower', \array_map('\\opacit_mail\Mail\\Base\\Utils::Utf7ModifiedToUtf8', $oFetchResponse->GetFetchValue(FetchType::FLAGS->value)));
					$aInfo['messagesFlags'][] = array(
						'uid' => $iUid,
						'flags' => $aLowerFlags
					);
...
//			return $this->oImapClient->FolderStatusAndSelect($sFolderName)->etag;
		...
		{
			\opacit_mail\Engine\Log::warning('IMAP', "FolderHash({$sFolderName}) Exception: {$oException->getMessage()}");
		}
		return '';
	}
	...
	public function MessageThread(string $sFolderName, string $sMessageID) : array
	{
		...
		$sMessageID = \opacit_mail\Mail\Imap\SearchCriterias::escapeSearchString($this->oImapClient, $sMessageID);
		...
		catch (\opacit_mail\Mail\RuntimeException $oException)
		{
			\opacit_mail\Engine\Log::warning('MailClient', 'MessageThread ' . $oException->getMessage());
			unset($oException);
		}
//		$this->logWrite('MessageThreadList: '.\print_r($threads, 1));
	 ...
	 * @throws \opacit_mail\Mail\RuntimeException
	 */
	protected function ThreadsMap(string $sAlgorithm, MessageCollection $oMessageCollection, ?\opacit_mail\Mail\Cache\CacheClient $oCacher, bool $bBackground = false) : array
	{
...
/*
			// Idea to fetch all UID's in background
				...
				\opacit_mail\Engine\Shutdown::add(function($oMailClient, $oFolderInfo, $oCacher) {
...
*/
		...
		{
			\opacit_mail\Engine\Log::warning('MailClient', 'ThreadsMap ' . $oException->getMessage());
			unset($oException);
		}
	...
	protected function ThreadsOldUids(array $aAllThreads, MessageCollection $oMessageCollection, ?\opacit_mail\Mail\Cache\CacheClient $oCacher, bool $bBackground = false) : array
	{
		$oFolderInfo = $oMessageCollection->FolderInfo;
		...
		if ($bThreadSort) {
			...
			foreach ($aAllThreads as $aThreadUIDs) {
				$oParams->oSequenceSet = new \opacit_mail\Mail\Imap\SequenceSet($aThreadUIDs);
...
/*
			...
			$oParams->oSequenceSet = new \opacit_mail\Mail\Imap\SequenceSet($aSortUids);
...
*/
	...
	private function GetUids(MessageListParams $oParams, FolderInformation $oInfo, bool $onlyCache = false) : ?array
	{
		...
		$oSearchCriterias = \opacit_mail\Mail\Imap\SearchCriterias::fromString(
			$this->oImapClient,
			$sFolderName,
			$oParams->sSearch,
...
//			&& ($this->oImapClient->hasCapability('THREAD=REFS') || $this->oImapClient->hasCapability('THREAD=REFERENCES') || $this->oImapClient->hasCapability('THREAD=ORDEREDSUBJECT'));
		...
		if ($message_list_limit && !$oParams->iThreadUid && $oParams->oCacher && $oParams->oCacher->IsInited()) {
			...
			if (null !== $aUids) {
				...
				\opacit_mail\Engine\Shutdown::add(function($oMailClient, $oAllParams, $oInfo, $oMessageCollection) {
...
//			\opacit_mail\Engine\Log::warning('IMAP', 'RFC5258 not supported, using LSUB');
			try
			{
				$oSubscribedFolders = $this->oImapClient->FolderSubscribeList($sParent, $sListPattern);
				foreach ($oSubscribedFolders as /* @var $oImapFolder \opacit_mail\Mail\Imap\Folder */ $oImapFolder) {
			...
			{
				\opacit_mail\Engine\Log::error('IMAP', 'FolderSubscribeList: ' . $oException->getMessage());
				foreach ($oFolderCollection as /* @var $oImapFolder \opacit_mail\Mail\Imap\Folder */ $oImapFolder) {
	 ...
	 * @throws \ValueError
	 */
	public function FolderCreate(string $sFolderNameInUtf8, string $sFolderParentFullName = '', bool $bSubscribeOnCreation = true, string $sDelimiter = '') : ?\opacit_mail\Mail\Imap\Folder
	{
		...
		if (!\strlen($sDelimiter) || \strlen($sFolderParentFullName)) {
			...
			if (null === $sDelimiter) {
				// TODO: Translate
				throw new \opacit_mail\Mail\RuntimeException(
...
/*		// Allow non existent parent folders
		if (\strlen($sDelimiter) && false !== \strpos($sFolderNameInUtf8, $sDelimiter)) {
			// TODO: Translate
			throw new \opacit_mail\Mail\RuntimeException('New folder name contains delimiter.');
		}
*/
	...
	public function FolderRename(string $sPrevFolderFullName, string $sNewFolderFullName) : self
	{
		...
		if (!$this->oImapClient->FolderHierarchyDelimiter($sPrevFolderFullName)) {
			// TODO: Translate
			throw new \opacit_mail\Mail\RuntimeException('Cannot rename non-existent folder.');
		}
/*
...
*/
		...
		foreach ($oSubscribedFolders as /* @var $oFolder \opacit_mail\Mail\Imap\Folder */ $oFolder) {
