...
namespace opacit_mail\Mail\Log;
...
//		'SIGSTOP',
	...
	public static function Guid() : string
	{
		static $sCache = null;
		if (null === $sCache) {
			$sCache = \substr(\opacit_mail\Mail\Base\Utils::Sha1Rand(), -8);
		}
...
//		$this->bShowSecrets && $this->Write("AddSecret '{$sWord}'", \LOG_INFO);
	...
	public function __phpErrorHandler(int $iErrNo, string $sErrStr, string $sErrFile, int $iErrLine) : bool
	{
		if (\error_reporting() & $iErrNo) {
			$this->Write(
				...
				isset(static::PHP_TYPES[$iErrNo]) ? static::PHP_TYPES[$iErrNo] : \LOG_ERR,
				'PHP' . (isset(static::PHP_TYPE_POSTFIX[$iErrNo]) ? static::PHP_TYPE_POSTFIX[$iErrNo] : '')
			);
		}
	...
	public function __loggerShutDown() : void
	{
		if ($this->bUsed) {
			...
			$this->Write('Memory peak usage: '.\opacit_mail\Mail\Base\Utils::FormatFileSize(\memory_get_peak_usage(true), 2));
	...
	public function Write(string $sDesc, int $iType = \LOG_INFO, string $sName = '', bool $bDiplayCrLf = false) : bool
	{
		...
		foreach ($this as /* @var $oLogger \opacit_mail\Mail\Log\Driver */ $oLogger)
		{
