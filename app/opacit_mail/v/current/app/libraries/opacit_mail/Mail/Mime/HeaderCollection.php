 ...
 * @package Mime
 */
class HeaderCollection extends \opacit_mail\Mail\Base\Collection
{
	 ...
	 * spf=pass (ORIGINATING: domain of "opacit_mail@domain.tld" designates 0.0.0.0 as permitted sender) smtp.mailfrom="opacit_mail@domain.tld";
	...
	public function AuthStatuses() : array
	{
		...
		$aHeaders = $this->ValuesByName(Enumerations\Header::AUTHENTICATION_RESULTS->value);
		if (\count($aHeaders)) {
		...
		if (!\count($aResult['dkim'])) {
			...
			$aHeaders = $this->ValuesByName(Enumerations\Header::X_DKIM_AUTHENTICATION_RESULTS->value);
