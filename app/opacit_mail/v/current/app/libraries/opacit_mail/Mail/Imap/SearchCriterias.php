...
class SearchCriterias
{
	...
	*/
	...
	public static function fromString(\opacit_mail\Mail\Imap\ImapClient $oImapClient, string $sFolderName, string $sSearch, bool $bHideDeleted, bool &$bUseCache = true) : self
	{
		$iTimeFilter = 0;
		...
		if (\strlen($sSearch)) {
			$aLines = \preg_match('/^('.static::RegEx.'):/i', $sSearch)
				? self::parseSearchString($sSearch)
				: self::parseQueryString($sSearch);
			...
			if (!$aLines) {
				...
				foreach ($aLines as $sName => $sRawValue) {
					...
					switch ($sName) {
						case 'FROM':
						case 'SUBJECT':
						case 'BODY': // $sValue = \trim(\opacit_mail\Mail\Base\Utils::StripSpaces($sValue), '"');
						...
						case 'KEYWORD':
							...
							$aCriteriasResult[] = static::escapeSearchString(
								$oImapClient,
								\opacit_mail\Mail\Base\Utils::Utf8ToUtf7Modified($sRawValue)
							);
							break;
						...
						case 'SMALLER':
							$aCriteriasResult[] = $sName;
							$aCriteriasResult[] = self::parseFriendlySize($sRawValue);
							break;

						case 'SINCE':
							$sValue = self::parseSearchDate($sRawValue);
							if ($sValue) {
						...
						case 'DATE':
							...
							if (2 === \count($aDate)) {
								if (\strlen($aDate[0])) {
									$iDateStampFrom = self::parseSearchDate($aDate[0]);
								}
								if (\strlen($aDate[1])) {
									$iDateStampTo = self::parseSearchDate($aDate[1]);
									$iDateStampTo += 60 * 60 * 24;
								}
							} else if (\strlen($sRawValue)) {
								$iDateStampFrom = self::parseSearchDate($sRawValue);
...
//			$aCriteriasResult[] = \gmdate('j-M-Y', $iTimeFilter);
	...
	public static function escapeSearchString(\opacit_mail\Mail\Imap\ImapClient $oImapClient, string $sSearch) : string
	{
//		return $oImapClient->EscapeString($sSearch);
//		return \opacit_mail\Mail\Base\Utils::IsAscii($sSearch) || $oImapClient->hasCapability('QQMail'))
		return (\opacit_mail\Mail\Base\Utils::IsAscii($sSearch) || !$oImapClient->hasCapability('LITERAL+'))
	...
	{
		if (\strlen($sDate)) {
			$oDateTime = \DateTime::createFromFormat('Y-m-d', $sDate, \opacit_mail\Mail\Base\DateTimeHelper::GetUtcTimeZoneObject());
	...
	/**
	 * opacit_mail search like: 'from=foo&to=test&is[]=unseen&is[]=flagged'
	 */
	...
	/**
	 * Legacy search like: 'from:"foo" to:"test" is:unseen,flagged'
	 */
	private static function parseSearchString(string $sSearch) : array
	{
		...
		$sSearch = \opacit_mail\Mail\Base\Utils::StripSpaces($sSearch);
		...
		if (\preg_match_all('/".*?(?<!\\\)"/', $sSearch, $mMatch)) {
			foreach ($mMatch[0] as $sItem) {
				do {
					$sKey = \opacit_mail\Mail\Base\Utils::Sha1Rand();
				}
