...
class SORT extends Request
{
	...
	function __construct(\opacit_mail\Mail\Imap\ImapClient $oImapClient)
	{
		if (!$oImapClient->hasCapability('SORT')) {
			$oImapClient->writeLogException(new \opacit_mail\Mail\RuntimeException('SORT is not supported'), \LOG_ERR);
		}
	...
	{
		if (!$this->aSortTypes) {
			$this->oImapClient->writeLogException(new \opacit_mail\Mail\RuntimeException('SortTypes are missing'), \LOG_ERR);
		}
		...
		if ($this->aReturn) {
			...
			if (!$this->oImapClient->hasCapability('ESORT')) {
				$this->oImapClient->writeLogException(new \opacit_mail\Mail\RuntimeException('ESORT is not supported'), \LOG_ERR);
			}
			if (!$this->oImapClient->hasCapability('CONTEXT=SORT')) {
				foreach ($this->aReturn as $sReturn) {
					if (\preg_match('/PARTIAL|UPDATE|CONTEXT/i', $sReturn)) {
						$this->oImapClient->writeLogException(new \opacit_mail\Mail\RuntimeException('CONTEXT=SORT is not supported'), \LOG_ERR);
					}
				}
			}
			$aRequest[] = 'RETURN';
		...
		$aRequest[] = 'UTF-8'; // \strtoupper(\opacit_mail\Mail\Base\Enumerations\Charset::UTF_8)
