...
class THREAD extends Request
{
	...
	function __construct(\opacit_mail\Mail\Imap\ImapClient $oImapClient)
	{
		if ($oImapClient->hasCapability('THREAD=REFS')) {
			...
			$oImapClient->writeLogException(new \opacit_mail\Mail\RuntimeException('THREAD is not supported'), \LOG_ERR);
		}
	...
	public function setAlgorithm(string $sAlgorithm) : void
	{
		...
		if (!$this->oImapClient->hasCapability("THREAD={$sAlgorithm}")) {
			$this->oImapClient->writeLogException(new \opacit_mail\Mail\RuntimeException("THREAD={$sAlgorithm} is not supported"), \LOG_ERR);
		}
	...
	public function SendRequestIterateResponse() : iterable
	{
		$this->oImapClient->SendRequest(
			...
			array(
				$this->sAlgorithm,
				'UTF-8', // \strtoupper(\opacit_mail\Mail\Base\Enumerations\Charset::UTF_8)
