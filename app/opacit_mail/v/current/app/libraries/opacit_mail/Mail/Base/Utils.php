...
namespace opacit_mail\Mail\Base;
...
abstract class Utils
{
	...
	public static function NormalizeCharset(string $sEncoding, bool $bAsciAsUtf8 = false) : string
	{
		...
		switch ($sEncoding)
		{
			...
			case 'us-asci':
			case 'us-ascii':
				return $bAsciAsUtf8 ? Enumerations\Charset::UTF_8->value :
					Enumerations\Charset::ISO_8859_1->value;
			...
			case 'utf8':
			case 'utf-8':
				return Enumerations\Charset::UTF_8->value;
	...
	public static function NormalizeCharsetByValue(string $sCharset, string $sValue) : string
	{
		...
		if (Enumerations\Charset::UTF_8->value !== $sCharset &&
			static::IsUtf8($sValue) &&
			!\str_contains($sCharset, Enumerations\Charset::ISO_2022_JP->value)
		)
		{
			$sCharset = Enumerations\Charset::UTF_8->value;
		}
	...
	public static function MbConvertEncoding(string $sInputString, ?string $sFromEncoding, string $sToEncoding) : string
	{
		...
		if ($sFromEncoding) {
			...
			if (isset(self::$RenameEncoding[$sFromEncoding])) {
				$sFromEncoding = self::$RenameEncoding[$sFromEncoding];
			}
/*
...
//				return $sInputString;
	...
	public static function ConvertEncoding(string $sInputString, string $sFromEncoding, string $sToEncoding) : string
	{
		...
		if ('' === \trim($sInputString) || ($sFromEncoding === $sToEncoding && Enumerations\Charset::UTF_8->value !== $sFromEncoding))
		{
			return $sInputString;
		}
	...
	public static function DecodeHeaderValue(string $sEncodedValue, string $sIncomingCharset = '') : string
	{
		...
		if (\strlen($sIncomingCharset)) {
			...
			$sValue = static::ConvertEncoding($sValue, $sIncomingCharset, Enumerations\Charset::UTF_8->value);
		}
...
//		\preg_match_all('/=\?[^\?]+\?[q|b|Q|B]\?[^\?]*?\?=/', $sValue, $aMatch);
		...
		for ($iIndex = 0, $iLen = \count($aParts); $iIndex < $iLen; ++$iIndex) {
			if ($bOneCharset) {
				...
				$sValue = \str_replace($aParts[$iIndex][0],
					static::ConvertEncoding($aParts[$iIndex][1], $aParts[$iIndex][2], Enumerations\Charset::UTF_8->value),
					$sValue);
			}
		}
		...
		if ($bOneCharset && \strlen($sMainCharset)) {
			...
			$sValue = static::ConvertEncoding($sValue, $sMainCharset, Enumerations\Charset::UTF_8->value);
		}
	...
	public static function EncodeHeaderValue(string $sValue, string $sEncodeType = 'B') : string
	{
		...
		if (\strlen($sValue) && !static::IsAscii($sValue)) {
			...
			{
				case 'B':
					return '=?'.\strtolower(Enumerations\Charset::UTF_8->value).
...
//		return (false !== $sNewUtfString) ? \preg_replace('/\\p{Cc}/u', '', $sNewUtfString) : $sUtfString;
	...
	public static function FunctionCallable(string $sFunctionName) : bool
	{
		if (null === self::$disabled_functions) {
			self::$disabled_functions = \array_map('trim', \explode(',', \ini_get('disable_functions')));
		}
/*
...
*/
		return \function_exists($sFunctionName)
			&& !\in_array($sFunctionName, self::$disabled_functions);
//			&& \is_callable($mFunctionNameOrNames);
	...
	public static function IdnToUtf8(string $sStr) : string
	{
		...
		\opacit_mail\Engine\Log::warning('MailSo', "Deprecated function IdnToUtf8 called at {$trace['file']}#{$trace['line']}");
		if (\preg_match('/(^|\.|@)xn--/i', $sStr)) {
			$sStr = \str_contains($sStr, '@')
			? \opacit_mail\Engine\IDN::emailToUtf8($sStr)
			: \idn_to_utf8($sStr);
		}
		return $sStr;
	}
	...
	{
		$trace = \debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 2)[1];
		\opacit_mail\Engine\Log::warning('MailSo', "Deprecated function IdnToAscii called at {$trace['file']}#{$trace['line']}");
		$aParts = \explode('@', $sStr);
