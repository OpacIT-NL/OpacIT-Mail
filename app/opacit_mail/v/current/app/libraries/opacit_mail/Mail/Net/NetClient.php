...
namespace opacit_mail\Mail\Net;

/**
 * @category MailSo
 * @package Net
 * @method static Login(ConnectSettings $oSettings)
 */
abstract class NetClient
{
	...
	{
		throw new \opacit_mail\Mail\RuntimeException($sErrStr, $iErrNo);
	}

	/**
	 * @throws \ValueError
	 * @throws \opacit_mail\Mail\RuntimeException
	 * @throws \opacit_mail\Mail\Net\Exceptions\SocketAlreadyConnectedException
	 * @throws \opacit_mail\Mail\Net\Exceptions\SocketCanNotConnectToHostException
	 */
...
//			$this->Disconnect();
		...
		$this->ssl = \opacit_mail\Mail\Net\Enumerations\ConnectionSecurityType::UseSSL($this->Settings->port, $this->Settings->type);
...
//			$this->sConnectedHost = ($this->ssl ? 'ssl://' : '') . $this->sConnectedHost;
		...
		if (!$this->ssl && \opacit_mail\Mail\Net\Enumerations\ConnectionSecurityType::SSL->value === $this->Settings->type) {
			$this->writeLogException(
				new \opacit_mail\Mail\Net\Exceptions\SocketUnsuppoterdSecureConnectionException('SSL isn\'t supported: ('.\implode(', ', \stream_get_transports()).')'),
				\LOG_ERR);
		}
		...
		if (!$this->rConnect) {
			$this->writeLogException(
				new Exceptions\SocketCanNotConnectToHostException(
					\opacit_mail\Mail\Base\Locale::ConvertSystemString($sErrorStr), (int) $iErrorNo,
		...
		if ($this->rConnect && \opacit_mail\Mail\Base\Utils::FunctionCallable('stream_set_timeout')) {
	...
	{
		$bSuccess = false;
		if ($this->rConnect && \opacit_mail\Mail\Base\Utils::FunctionCallable('stream_socket_enable_crypto')) {
...
//			}
		...
		$bSuccess || $this->writeLogException(new \opacit_mail\Mail\RuntimeException('Cannot enable STARTTLS.'), \LOG_ERR);
	}
...
//	abstract public function Login(ConnectSettings $oSettings) : self;
	...
	{
		return $this->rConnect && \opacit_mail\Mail\Base\Utils::FunctionCallable('stream_context_get_options')
			? \stream_context_get_params($this->rConnect) : false;
	}

	/**
	 * @throws \opacit_mail\Mail\Net\Exceptions\SocketConnectionDoesNotAvailableException
	 * @throws \opacit_mail\Mail\Net\Exceptions\SocketWriteException
	 */
	...
	/**
	 * @throws \opacit_mail\Mail\Net\Exceptions\SocketConnectionDoesNotAvailableException
	 * @throws \opacit_mail\Mail\Net\Exceptions\SocketReadException
	 */
...
//		$iReadLen && $this->writeLog('Received '.$iReadBytes.'/'.$iReadLen.' bytes.');
	...
	{
		$this->logWrite($sDesc, \LOG_INFO, $this->getLogName(), true);
	}

	public function writeLogException(\Throwable $oException, int $iDescType = \LOG_NOTICE, bool $bThrowException = true) : void
	{
