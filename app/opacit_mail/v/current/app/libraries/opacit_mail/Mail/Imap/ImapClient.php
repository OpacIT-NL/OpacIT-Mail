...
namespace opacit_mail\Mail\Imap;
 ...
 * @package Imap
 */
class ImapClient extends \opacit_mail\Mail\Net\NetClient
{
	...
	/**
	 * Used by \opacit_mail\Mail\Client\MailClient::MessageMimeStream
	 */
...
//	public function Connect(Settings $oSettings) : void
	...
	public function Login(Settings $oSettings) : self
	{
		...
		foreach ($oSettings->SASLMechanisms as $sasl_type) {
			if ($this->hasCapability("AUTH={$sasl_type}") && \opacit_mail\Engine\SASL::isSupported($sasl_type)) {
				$type = $sasl_type;
				break;
			}
		}
		...
		if (!$type) {
			...
			throw new \opacit_mail\Mail\RuntimeException('No supported SASL mechanism found, remote server wants: '
	...
	public function GetNamespaces() : ?NamespaceResult
	{
		...
		try {
			...
			foreach ($oResponseCollection as $oResponse) {
				if (Enumerations\ResponseType::UNTAGGED->value === $oResponse->ResponseType
			...
			if (\is_resource($this->ConnectionResource())) {
				...
				while (true) {
					...
					if ($oResponse->IsStatusResponse
					 && Enumerations\ResponseType::UNTAGGED->value === $oResponse->ResponseType
					 && Enumerations\ResponseStatus::PREAUTH->value === $oResponse->StatusOrIndex
//					 && (Enumerations\ResponseStatus::PREAUTH->value === $oResponse->StatusOrIndex || Enumerations\ResponseStatus::BYE === $oResponse->StatusOrIndex)
					) {
						break;
					}
...
//						\opacit_mail\Engine\Log::warning('IMAP', "{$oResponse->OptionalResponse[0]}: {$this->lastCommand}");
					}

					if ($sEndTag === $oResponse->Tag || Enumerations\ResponseType::CONTINUATION->value === $oResponse->ResponseType) {
...
*/
	...
	{
		return $this->EscapeString($this->UTF8 ? $sFolderName : \opacit_mail\Mail\Base\Utils::Utf8ToUtf7Modified($sFolderName));
	}

	public function toUTF8(string $sText) : string
	{
		return $this->UTF8 ? $sText : \opacit_mail\Mail\Base\Utils::Utf7ModifiedToUtf8($sText);
	}
}
