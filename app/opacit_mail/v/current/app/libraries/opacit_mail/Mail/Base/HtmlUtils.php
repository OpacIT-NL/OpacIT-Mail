...
*/
		...
		@$oDoc->loadHTML('<?xml version="1.0" encoding="utf-8"?>'.
			...
			'<body '.\trim($sBodyAttrs).'>'.\opacit_mail\Mail\Base\Utils::Utf8Clear($sHtml).'</body></html>');

		$oDoc->normalizeDocument();

		if (\opacit_mail\Mail\Base\Utils::FunctionCallable('libxml_clear_errors')) {
			\libxml_clear_errors();
		}
	...
	{
		$sText = \opacit_mail\Mail\Base\Utils::StripSpaces($sText);
