...
use opacit_mail\Mail\Imap\Folder;
...
use opacit_mail\Mail\Imap\Enumerations\FolderStatus;
...
//			, ['(USE (\Drafts \Sent))'] RFC 6154
	...
	public function FolderDelete(string $sFolderName) : void
	{
		...
		if ($oInfo->MESSAGES) {
			throw new \opacit_mail\Mail\Client\Exceptions\NonEmptyFolder;
		}
...
//		$this->FolderCheck();
	...
	{
		$aStatusItems = array(
			FolderStatus::MESSAGES->value,
			FolderStatus::UNSEEN->value,
			FolderStatus::UIDNEXT->value,
			FolderStatus::UIDVALIDITY->value
		);
		// RFC 4551 or RFC 5162
		if ($this->hasCapability('CONDSTORE') || $this->hasCapability('QRESYNC')) {
			$aStatusItems[] = FolderStatus::HIGHESTMODSEQ->value;
		}
		// RFC 7889
		if ($this->hasCapability('APPENDLIMIT')) {
			$aStatusItems[] = FolderStatus::APPENDLIMIT->value;
		}
		// RFC 8438
		if ($this->hasCapability('STATUS=SIZE')) {
			$aStatusItems[] = FolderStatus::SIZE->value;
		}
		// RFC 8474
		if ($this->hasCapability('OBJECTID')) {
			$aStatusItems[] = FolderStatus::MAILBOXID->value;
/*
				...
				if (\is_array($oResponse->OptionalResponse)) {
					...
					if (\count($oResponse->OptionalResponse) > 1) {
						if ('PERMANENTFLAGS' === $key && \is_array($oResponse->OptionalResponse[1])) {
							$oResult->PermanentFlags = \array_map('\\opacit_mail\Mail\\Base\\Utils::Utf7ModifiedToUtf8', $oResponse->OptionalResponse[1]);
						}
...
//					$oResult->Flags = \array_map('\\opacit_mail\Mail\\Base\\Utils::Utf7ModifiedToUtf8', $oResponse->ResponseList[2]);
				}
			}
		}
...
//				$aParameters[0] = '()';
			...
			if ($this->isImap4Rev2()) {
				\opacit_mail\Engine\Log::warning('IMAP', 'IMAP4rev2 server without LIST-EXTENDED; LSUB fallback may fail');
			}
			$sCmd = 'LSUB';
		}
