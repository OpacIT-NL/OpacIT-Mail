...
class Part
{
	...
	public static function FromString(string $sRawMessage) : ?self
	{
		$rStreamHandle = \strlen($sRawMessage) ?
			\opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResource() : false;
		if ($rStreamHandle) {
			...
			try {
				...
				\opacit_mail\Mail\Base\ResourceRegistry::CloseMemoryResource($rStreamHandle);
			}
		}
		return null;
	}
	...
	public function ToStream()
	{
		...
		return \opacit_mail\Mail\Base\StreamWrappers\SubStreams::CreateStream($aSubStreams);
	}
	...
	public function addPgpEncrypted(string $sEncrypted)
	{
		...
		$oSubPart->Body = \opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString('Version: 1');
		...
		$oSubPart->Headers->AddByName(Enumerations\Header::CONTENT_TRANSFER_ENCODING->value, '7Bit');
		$oSubPart->Body = \opacit_mail\Mail\Base\ResourceRegistry::CreateMemoryResourceFromString($sEncrypted);
		$oPart->SubParts->append($oSubPart);
	}
