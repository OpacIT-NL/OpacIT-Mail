...
class ESEARCH extends Request
{
	...
	function __construct(\opacit_mail\Mail\Imap\ImapClient $oImapClient)
	{
		if (!$oImapClient->hasCapability('ESEARCH')) {
			$oImapClient->writeLogException(new \opacit_mail\Mail\RuntimeException('ESEARCH is not supported'), \LOG_ERR);
		}
...
/*		// RFC 6203
		if (false !== \stripos($this->sCriterias, 'FUZZY') && !$this->oImapClient->hasCapability('SEARCH=FUZZY')) {
			$this->oImapClient->writeLogException(new \opacit_mail\Mail\RuntimeException('SEARCH=FUZZY is not supported'), \LOG_ERR);
		}
*/
		...
		if ($aFolders) {
			if (!$this->oImapClient->hasCapability('MULTISEARCH')) {
				$this->oImapClient->writeLogException(new \opacit_mail\Mail\RuntimeException('MULTISEARCH is not supported'), \LOG_ERR);
			}
			$sCmd = 'ESEARCH';
			$aReques[] = 'IN';
			$aReques[] = $aFolders;
			...
			if (!$this->oImapClient->hasCapability('CONTEXT=SEARCH')) {
				foreach ($this->aReturn as $sReturn) {
					if (\preg_match('/PARTIAL|UPDATE|CONTEXT/i', $sReturn)) {
						$this->oImapClient->writeLogException(new \opacit_mail\Mail\RuntimeException('CONTEXT=SEARCH is not supported'), \LOG_ERR);
					}
				}
			}
