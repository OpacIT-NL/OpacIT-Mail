...
namespace opacit_mail\Mail\Imap\Traits;

use opacit_mail\Mail\Imap\Response;
use opacit_mail\Mail\Imap\Enumerations\ResponseType;
use opacit_mail\Mail\Imap\Exceptions\ResponseNotFoundException;
...
trait ResponseParser
{
	...
	protected function partialParseResponse() : Response
	{
		...
		if (ResponseType::UNKNOWN->value === $oResponse->ResponseType) {
	 ...
	 * The legacy fix for this isn't correct either.
		...
		bool $bRoot = false)
	{
		...
		while (true) {
			...
			{
				case ']':
					if ($bRoot && self::skipSquareBracketParse($oImapResponse)) {
						$bIsGotoDefault = true;
						break 2;
					}
				case ')':
...
//									$this->writeLog('{'.$iLiteralLen.'} '.$sLiteral, \LOG_INFO);
									...
									\opacit_mail\Engine\Log::notice('IMAP', 'Literal treated as atom and skipped');
								}
								unset($sLiteral);
							} else {
				...
				case '"':
					...
					while (true) {
						...
						case "\r":
						case "\n":
							\opacit_mail\Engine\Log::notice('IMAP', 'Invalid char in quoted string: "' . \substr($this->sResponseBuffer, $iPos, $iOffset + $iLength - $iPos) . '"');
				...
				default:
					...
					while ($iPos <= $iBufferEndIndex) {
						...
						switch (true)
						{
							case $bRoot && ('[' === $sCharDef || ']' === $sCharDef) && self::skipSquareBracketParse($oImapResponse):
								++$iPos;
								break;
							case '[' === $sCharDef:
						...
						if ($bRoot) {
							if (!isset($oImapResponse->Tag) && 1 === \count($aResponseList)) {
								...
								if ($this->getCurrentTag() === $oImapResponse->Tag) {
									$oImapResponse->ResponseType = ResponseType::TAGGED->value;
								}
							}
							...
							else if (ResponseType::CONTINUATION->value === $oImapResponse->ResponseType
...
//				\set_time_limit(10);
	...
	private function partialResponseLiteralCallbacks(?string $sParent, ?string $sLiteralAtomUpperCase, int $iLiteralLen) : bool
	{
		...
		$rImapLiteralStream = \opacit_mail\Mail\Base\StreamWrappers\Literal::CreateStream($this->ConnectionResource(), $iLiteralLen);
