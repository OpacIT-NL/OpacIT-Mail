...
use opacit_mail\Engine\Providers\AddressBook\AddressBookInterface;
use opacit_mail\Engine\Providers\AddressBook\Classes\Contact;
...
class NextcloudAddressBook implements AddressBookInterface
{
	...
	public function ContactSave(Contact $oContact): bool
	{
		...
		if (empty((string) $vCard->UID)) {
			$vCard->UID = \opacit_mail\Engine\UUID::generate();
		}
	...
	private function contactToVCard(array $ncContact): ?VCard
	{
		...
		$vCard = new VCard();
		$vCard->UID = $ncContact['UID'] ?? \opacit_mail\Engine\UUID::generate();
		$fn = $ncContact['FN'] ?? '';
		$vCard->FN = $fn;

		// opacit_mail displays the N (structured name) property.
	...
	{
		$props = [];
		$props['UID'] = (string) ($vCard->UID ?? \opacit_mail\Engine\UUID::generate());
