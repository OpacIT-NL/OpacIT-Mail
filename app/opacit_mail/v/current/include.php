...
if (!defined('OPACIT_MAIL_DEV')) {
	define('OPACIT_MAIL_DEV', '0.0.0' === APP_VERSION);
}
...
if (!defined('OPACIT_MAIL_LIBRARIES_PATH')) {
	define('OPACIT_MAIL_LIBRARIES_PATH', rtrim(realpath(__DIR__), '\\/').'/app/libraries/');

	if (false === set_include_path(OPACIT_MAIL_LIBRARIES_PATH . PATH_SEPARATOR . get_include_path())) {
		exit('set_include_path() failed. Probably due to Apache config using php_admin_value instead of php_value');
	}

	spl_autoload_extensions('.php');
	/** lowercase autoloader (for non-opacit_mail libraries) */
...
//		if ($file = stream_resolve_include_path(strtr($sClassName, '\\', DIRECTORY_SEPARATOR) . '.php')) {
	...
	/** opacit_mail\Engine lowercase-filename autoloader
	 *  Handles engine classes that use lowercase file/directory names
	 *  e.g. opacit_mail\Engine\Crypt → opacit_mail/Engine/crypt.php
	 *       opacit_mail\Engine\SASL\Plain → opacit_mail/Engine/sasl/Plain.php (lowercase dir, PascalCase file)
	 */
	spl_autoload_register(function($sClassName){
		if (str_starts_with($sClassName, 'opacit_mail\\Engine\\')) {
			...
			// Try lowercase directories but preserve filename case
			// e.g. opacit_mail\Engine\SASL\XOAuth2 → opacit_mail/Engine/sasl/XOAuth2.php
